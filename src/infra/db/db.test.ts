...
test("migrations: every file under db/migrations is applied to the test database (863 tables: public + restricted_fl)", { skip }, async () => {
  ...
  assert.equal(t!.c, 863n);   // the count a fresh `db/migrate.sh` run produces through 0243 (859 through 0242 plus 0243's four — 36.1: partner_users, partner_credentials, partner_sessions, partner_actions; 836 through 0231 plus 0190's five — 35.6: closing_orchestrations, closing_orchestration_steps, funding_snapshots, purchase_reconciliations, orchestration_daily_receipts — plus 0240's eleven — 35.12: environment_manifests, posture_controls, posture_checks, posture_findings, integration_switches, restore_drills, data_scans, parallel_runs, parallel_run_diffs and two more — plus 0200's seven — 35.8: work_screen_versions, work_items, work_item_events, work_actions, work_derivations, work_approvals, work_log_recon_runs; 0201–0203 add no table — measured on a scratch database at the 35.8 merge; 824 through 0222 plus 0180's six — 35.4: the close chain's tables — plus 0230's six — 35.11: ops_exceptions, exception_triages, ops_daily_reports, hosted_probe_runs, persisted_measurements, steward_notes; 0231 seeds rows only): 776 through 0137 (public + restricted_fl base tables; 0132 journey_progress is a view; 0134 recreates three tables as views) plus 35.5's seven — 0142's three (installment_schedule_runs, servicer_profiles, loan_servicing_configs; loan_installments is extended, not created), 0144's cashiering_unit_runs and 0145's three (lockbox_batches, lockbox_items, ach_return_files; ach_entries is extended) — 0146's five (35.3: cycle_registry, cycle_runs, jobs, job_events, cycle_receipts), 0150's eight (35.1's seam tables; its entity_latest_scoped is a view) and 0170's five (35.7: role_grants, role_queue_snapshots, role_handovers, breakglass_uses, api_principals); 0138–0141, 0143 (seed rows only), 0151 and 0171 create no table; plus 0210's eight (35.9: case_timelines, case_milestone_expectations, breach_action_registry, breach_actions, docket_reactions, firm_dispatches, claim_candidates, default_case_daily_runs); plus 0220's five (35.10: refinance_closeouts, refinance_closeout_steps, prior_loan_retirements, partner_retirement_notifications, refinance_closeout_daily_receipts; 0221/0222 add no table); plus 35.2's ten — 0161's two (document_blobs, document_holds), 0162's three (document_integrity_runs, document_integrity_findings, document_access_log), 0163's three (esign_envelopes, esign_envelope_documents, esign_signature_events) and 0164's two (mail_manifests, mail_manifest_pieces); 0160, 0165–0167 add no table; measured on a scratch database at the 35.2 merge; the absolute number moves with every §35 migration that adds a table
});
