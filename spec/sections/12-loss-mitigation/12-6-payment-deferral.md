...
#### State machine
`screened_eligible` → `offered|solicited` → `accepted` → `awaiting_contractual_payment` (if required) → `pending_smdu_entry` → `completed` (SMDU case submitted with campaign ID; effective) → `agreement_sent` → `documented` (custodian/recording done) → `closed`; `screened_ineligible` → Flex Mod path (12.8); `offered` → `declined|expired`. Guards: all 12 eligibility criteria as assertions; `assertEscrowAnalysisFresh` (≤30 days); `assertContractualPaymentIfRequired`; `assertEntryByMonthEnd`; `assertLarBeforeCompletion` (5.x ack — only when `contractual_payment_required`); `assertUpbLpiMatchesInvestorReporting` (IRM 4-01: pre-deferral UPB/LPI in the servicing solutions system equal the last reported values, otherwise the case is not processed).
...
| `SM_DEFERRAL_SOLICIT_ACCEPT_WINDOW` | deadline (borrower) | solicitation sent | send date | through the last day of the solicitation month (policy; ≥14 days) | acceptance | expire → `payment_deferral.solicitation.expired{no_response=true, qrpc, delinquency_days, acceptance_date}` (12.8 `FNMA_D23206_POSTDEFERRAL_SOLICIT_EXPIRY_FLEX_15`: Flex Mod solicitation within 15 days when no QRPC and ≥90 days delinquent — D2-3.2-06); re-screen next month |
...
| `FNMA_F122_DEFERRAL_LAR_BEFORE_EOM_1BD` | deadline | `payment_deferral.accepted{contractual_payment_required=true}` (6 months delinquent at evaluation, cumulative cap exceeded, or disaster deferral at 12 months — F-1-22 / IRM 4-01: the LAR rule applies only where a full contractual payment is required) | last day of completion month | −1 `business_days_fannie_et` | contractual-payment LAR/event accepted (5.x `FNMA_IRM_DEFERRAL_LAR_BEFORE_EOM_1BD`) | sev-2; slips to processing month |
...
| `FNMA_D23204_RECORDED_ORIGINAL_5BD` | deadline | `erecording.recorded_document.received` | receipt | 5 `business_days_fannie_et` (Guide glossary business days) | custodian delivery of original | sev-2 |
...
- **Investor reporting (5.x):** contractual payments reported (LAR 96 / payment events) before completion when a full contractual payment is required (F-1-22); otherwise the pre-deferral UPB/LPI reconciliation against the investor reporting system (IRM 4-01); loan-data change for the NIB/deferred amount per the Investor Reporting Manual **[PARTIALLY VERIFIED]**; delinquency status cleared.
...
- Event `payment_deferral.solicitation.expired{deferral_id, no_response=true, qrpc, delinquency_days, acceptance_date, basis}` — emitted when `SM_DEFERRAL_SOLICIT_ACCEPT_WINDOW` lapses with no response; 12.8's `FNMA_D23206_POSTDEFERRAL_SOLICIT_EXPIRY_FLEX_15` keys on it (D2-3.2-06).
...
- **Guardrails:** ineligibility determinations go to `lossmit_reviewer` before any notice (adverse determination); NIB math is code; cannot complete without the pre-deferral UPB/LPI matching the investor reporting system and, where a full contractual payment is required, its LAR/event ack and the payment; cannot include late charges or escrow shortage in the NIB.
...
- **12.6-T1:** Given the 4-payment example, when accepted 2026-09-22, then NIB = $7,370.68, late charges $252.00 waived, new payment $2,131.17, SMDU entry by 2026-09-30, effective 2026-10-01, agreement sent by completion + 5 days, custodian by 2026-10-26.
