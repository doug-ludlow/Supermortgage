...
const events = (type: string) => db.query<{ sequence: string; type: string; payload: Json; occurred_at: string }>(`SELECT sequence::text AS sequence, type, payload, occurred_at FROM loan_events WHERE loan_id = $1 AND type = $2 ORDER BY loan_events.sequence`, [S.loanId, type]);
