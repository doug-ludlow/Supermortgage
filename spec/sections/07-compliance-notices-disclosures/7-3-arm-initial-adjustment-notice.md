...
- `jurisdiction_rules.hfa_contact` (optional: state housing finance authority name/phone/URL by state, printed as additional information when populated — (xi) requires only the Bureau web site to access HFA contact information) — Supermortgage compliance; **[data to be compiled and verified if populated]**.
...
- `jurisdiction_rules.hfa_contact jsonb` {name, phone, url} (optional; (xi) is satisfied by the Bureau HFA-contact web site).
...
4. **Content assembly:** (ii) schedule sentence ("every 6 months thereafter"), (iv) index/source, (v) caps and floor, (vi) balance/term, (viii) "no prepayment penalty," (ix) toll-free number, (x) alternatives list verbatim, (xi) CFPB URL, HUD (800) 569-4287 and the Bureau web site for State housing finance authority contact information (§1026.20(d)(2)(xi); configured `counseling.hfa_bureau_url`); the state HFA contact from `jurisdiction_rules` by property state is optional additional information when populated.
...
| 7.3-T8 | Given a Texas property, then the (xi) block carries the CFPB/HUD counselor-list web site, the HUD toll-free number and the Bureau web site through which State housing finance authority contact information is accessed (§1026.20(d)(2)(xi)); naming the Texas HFA itself from `jurisdiction_rules` is optional additional information. |
...
3. Source of the optional state HFA contact list ((xi) requires only the Bureau web site) — compile in-house or license? **Default: optional; if populated, compile from HUD/NCSHA listings and verify annually [UNVERIFIED data set].**
