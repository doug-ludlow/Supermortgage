...
export function applyServicingRequestTimerOverrides(reg: TimerRegistry): void {
  ...
  o("REGX_1024_35I_CREDIT_SUPPRESS_60", { trigger: "`case.noe.opened{payment_related=true}`", satisfied: "`credit_reporting.suppression.expired`",
    why: "§4.1 timer table: `case.noe.opened` (any payment-related assertion: b1–b3, b5, b11) → 60 `calendar_days`; satisfied by 'expiry, or `case.noe.exception_determined` for the excepted assertions (§1024.35(g)(1) lifts (i))' — both are `credit_reporting.suppression.expired`: ops.expireCreditSuppressions emits it when `ends_at` passes (§1024.35(i)(1)), and case.noe.determine emits it with reason `regx_1024_35_g1_exception` when the (g)(1) determination ends the row (cases.liftSuppressionOnException: every payment-related assertion excepted; a carved-out identifiable assertion keeps the row — (g)(1)(ii) — so the gate stays armed until expiry)." });
  ...
  o("FNMA_A4_2_1_04_EMAIL_48H", { trigger: "`communication.inbound.received{channel=email}`", satisfied: "`communication.outbound.sent{channel=email, reply=true}`", why: "§4.3/4.5 timer table: inbound email → 'reply sent' within 48 hours — a policy target: the A4-2.1-04 standard is 'on average, emails from borrowers must be responded to within 48 hours of receipt', measured monthly in FNMA_A4_2_1_04_CALL_METRICS_MONTHLY (a per-message miss is an ops alert, not itself a Guide breach)." });
  o("FNMA_A4_2_1_04_CALL_METRICS_MONTHLY", { trigger: "`period.month_end`", satisfied: "`report.produced{report=a4_2_1_04_call_metrics}`", why: "§4.3 timer table: month-end → the monthly metrics report (ASA ≤ 60s, blockage ≤ 1%, abandonment ≤ 5%, e-mail response average ≤ 48 hours — ops.callMetrics) (A4-2.1-04)." });
  ...
  o("FNMA_D1_4_1_02_FNMA_LEGAL_60", { anchor: "`fnma_received_on`", satisfied: "`fnma.due_on_transfer.resolved{outcome∈{non_objection, expired}}`", why: "§4.4 timer table: `due_on_transfer.unenforceable.notified` (only for loans acquired by Fannie Mae after June 1, 2007 — sii.due_on_transfer.notify refuses earlier acquisitions) → Fannie Mae non-objection or expiry within 60 `calendar_days` of 'Fannie Mae receipt of the notice (delivery evidence)' — the payload's `fnma_received_on` (the send date for the e-mail package), not the servicer's `notice_date` (D1-4.1-02: 'not responded within 60 days of its receipt of the notice'); sii.due_on_transfer.resolve." });
  // ---- 4.5 complaints ------------------------------------------------------
