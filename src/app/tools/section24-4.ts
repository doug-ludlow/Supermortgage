...
import { appEvent, AGENT_24_4, RULE_SET_VERSION_24_4, GATES_24_4, orderTitleFeeGate, placeTitleOrder, parseCommitment, classifyException, computeRequiredEndorsements, missingEndorsements, endorsementFamily, endorsementClears, openCurative, clearCurative, transitionTitleOrder, clearanceCheck, commitmentDatedownGate,
  vetSettlementAgent, verifyWireInstructions, releaseWireBlock, cplBeforeFundingGate, requestPayoff, parsePayoffStatement, computePayoffAtDate, payoffStaleness, decideEscrowTreatment, helocRatios, checkSubordinateTerms, reviewTrust, reviewPOA, trustPoaReviewGate, evaluateAolPath, recordAolEvaluated, tx50a6TitleCheck, cemaNewMoney, classifyTaxCertificate, evaluateGates as evalGates, dateOf,
...
export const TOOLS_24_4: readonly ToolDef[] = defineTools("24.4", "title-closing", [
  ...
  { name: "requestPayoff", kind: "act", handler: compute((i, ctx, rt) => {
    ...
    if (i.op === "cancel") {
      need(i, "liability_id");
      // 35.10 T10 / unwind: the demand is cancelled (rescission, a cancelled funding, a withdrawal); nothing else changes
      const prevRow = rt.store.get("payoff_demands", `${app}:${str(i, "liability_id")}`)?.data; if (!prevRow) throw new RangeError(`no payoff_demands row ${app}:${str(i, "liability_id")}`);
      const cancelled = persist(rt, ctx, "payoff_demands", `${app}:${str(i, "liability_id")}`, { ...prevRow, status: "cancelled", cancelled_at: at(i, ctx), cancel_reason: str(i, "reason") || null }).data;
      appEvent(ctx.events, c, "payoff.demand.cancelled", { liability_id: str(i, "liability_id"), reason: str(i, "reason") || null, servicing_loan_id: (prevRow.servicing_loan_id as string | null) ?? null });
      return { status: cancelled.status, liability_id: str(i, "liability_id") };
    }
    need(i, "liability_id", "existing_servicer_party_id", "requested_good_through", "state", "written_authorization_document_id");
    const r = requestPayoff(ctx.events, c, { application_id: app, liability_id: str(i, "liability_id"), same_servicer: flag(i, "same_servicer"), servicing_loan_id: optStr(i, "servicing_loan_id"), existing_servicer_party_id: str(i, "existing_servicer_party_id"), requested_at: requestedAt, requested_on: optDate(i, "requested_on") ?? dateOf(requestedAt), request_channel: optStr(i, "request_channel") ?? "email", written_authorization_document_id: str(i, "written_authorization_document_id"), requested_good_through: dateIn(i, "requested_good_through"), state: str(i, "state"), refresh: flag(i, "refresh"), ...(str(i, "requester_type") === "refinancing_lender" ? { requester_type: "refinancing_lender" as const } : {}) });
    const prev = rt.store.get("payoff_demands", `${app}:${str(i, "liability_id")}`)?.data ?? {};
    // same servicer: 16.1's request row (payoffRequestIntake) is the servicing side's record — stored so 16.1 computePayoffQuote{request_id} finds it and does not re-intake (35.10 discrepancy 3)
    if (r.servicing_intake && r.servicing_event) rt.store.put("payoff_requests", String(r.servicing_intake.payload.request_id ?? `PR-${app}-${str(i, "liability_id")}`), { ...(r.servicing_row ?? {}), requested_by: "origination", application_id: app, request_event_id: r.servicing_event.id }, ctx.actor, ctx.now);
  ...
  { name: "parsePayoffStatement", kind: "act", handler: compute((i, ctx, rt) => {
    if (i.op === "paid") {
      // 35.10 rule 7: the final settlement statement's payoff line, paid to the servicer of record (the partner's wire, or the same-servicer internal transfer) — status paid, the posting date and the wire reference on the row
      need(i, "liability_id", "payoff_posted_on", "amount_cents", "evidence_document_id");
      const app = appOf(i, ctx); const c = ectx(i, ctx);
      const prevRow = rt.store.get("payoff_demands", `${app}:${str(i, "liability_id")}`)?.data; if (!prevRow) throw new RangeError(`no payoff_demands row ${app}:${str(i, "liability_id")}`);
      if (!["received", "refreshed", "funded", "paid"].includes(String(prevRow.status))) throw new RangeError(`payoff demand ${app}:${str(i, "liability_id")} is ${String(prevRow.status)}: only a received/refreshed statement's line can be paid`);
      const paid = persist(rt, ctx, "payoff_demands", `${app}:${str(i, "liability_id")}`, { ...prevRow, status: "paid", payoff_posted_on: str(i, "payoff_posted_on"), paid_amount_cents: cents(i.amount_cents), wire_reference: optStr(i, "wire_reference") ?? null, evidence_document_id: str(i, "evidence_document_id") }).data;
      appEvent(ctx.events, c, "payoff.demand.paid", { liability_id: str(i, "liability_id"), payoff_posted_on: str(i, "payoff_posted_on"), amount_cents: cents(i.amount_cents).toString(), wire_reference: optStr(i, "wire_reference") ?? null, evidence_document_id: str(i, "evidence_document_id"), servicing_loan_id: (prevRow.servicing_loan_id as string | null) ?? null, same_servicer: prevRow.same_servicer === true, statement_total_cents: String(prevRow.total_cents ?? "") });
      return { status: paid.status, payoff_posted_on: paid.payoff_posted_on, wire_reference: paid.wire_reference, amount_cents: cents(i.amount_cents), statement_total_cents: cents(prevRow.total_cents) };
    }
    ...
    const app = appOf(i, ctx); const c = ectx(i, ctx);
    const parsed = parsePayoffStatement({ principal_cents: cents(i.principal_cents), rate_pct: str(i, "rate_pct"), interest_paid_through: dateIn(i, "interest_paid_through"), per_diem_cents: cents(i.per_diem_cents), good_through_date: dateIn(i, "good_through_date"), fees_cents: optCents(i, "fees_cents") ?? 0n, recording_fee_cents: optCents(i, "recording_fee_cents") ?? 0n, escrow_shortage_cents: optCents(i, "escrow_shortage_cents") ?? 0n, credits_cents: optCents(i, "credits_cents") ?? 0n, statement_date: dateIn(i, "statement_date"), short_payoff: flag(i, "short_payoff"), stated_total_cents: optCents(i, "stated_total_cents"), stated_interest_cents: optCents(i, "stated_interest_cents") } as PayoffStatementInput);
    const prev = rt.store.get("payoff_demands", `${app}:${str(i, "liability_id")}`)?.data ?? {};
  ...
  { name: "decideEscrowTreatment", kind: "act", handler: compute((i, ctx, rt) => {
    ...
    const prev = rt.store.get("payoff_demands", `${app}:${str(i, "liability_id")}`)?.data ?? {};
    persist(rt, ctx, "payoff_demands", `${app}:${str(i, "liability_id")}`, { ...prev, application_id: app, liability_id: str(i, "liability_id"), same_servicer: i.same_servicer === undefined ? true : flag(i, "same_servicer"), servicing_loan_id: str(i, "servicing_loan_id"), escrow_treatment: r.escrow_treatment, consent_id: optStr(i, "consent_id"), refund_due_on: r.refund_due_on });
    return r;
  ...
  { name: "evaluateGates", kind: "act", handler: compute((i, ctx, rt) => {
    ...
    if (aol) recordAolEvaluated(ctx.events, ectx(i, ctx), aol, optStr(i, "order_id"));
