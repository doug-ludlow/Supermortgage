...
test("23.2-T1: Given findings Tue Oct 6, 2026 (refinance fixture) with 14 verification messages, 1 MI message (\"no MI\" at 70% LTV → none), 1 value-acceptance offer and 2 observations, when interpreted, then 14 PTD conditions are opened by 10:00 the same day (`SM_DU_CONDITIONS_SLA_4H` satisfied), `value_acceptance_offer = value_acceptance`, `sfc_required = {127}` (801 is added only when value acceptance is exercised at delivery), and no borrower-facing text contains a DU message ID.", async () => {
  ...
  assert.equal(interp.policy_outcome, "proceed"); assert.equal(interp.ptd_conditions_opened, 14); assert.equal(interp.value_acceptance_offer, "value_acceptance"); assert.deepEqual(interp.sfc_required, ["127"]);
  ...
  assert.deepEqual(conds.filter((c) => c.stage === "post_closing").map((c) => c.template_code).sort(), ["COND_DELIVERY_SFC_127"]);
  // B5-4.1-04: 003/007 are the Texas 50(a)(6) companions of SFC 304, never a general LCOR code — the Arizona LCOR carries neither; 801 joins only once value acceptance is exercised at delivery (rule 9)
  const az = { transaction_type: "limited_cash_out" as const, score_model: "classic_fico" as const, homeready: false, counseling_credit: false, value_acceptance_exercised: false, va_pd_exercised: false, high_balance: false, community_seconds: false, temporary_buydown: false, inter_vivos_trust: false, texas_50a6: false };
  assert.deepEqual(sfcAssembly(az), ["127"]); assert.deepEqual(sfcAssembly({ ...az, value_acceptance_exercised: true }), ["127", "801"]); assert.deepEqual(sfcAssembly({ ...az, transaction_type: "cash_out" }), ["127"]);
  assert.deepEqual(sfcAssembly({ ...az, texas_50a6: true }), ["007", "127", "304"]); assert.deepEqual(sfcAssembly({ ...az, texas_50a6: true, transaction_type: "cash_out" }), ["003", "127", "304"]); assert.deepEqual(sfcAssembly({ ...az, texas_50a6: true, transaction_type: "purchase" }), ["127", "304"]);
  ...
  assert.equal(h.ofType("condition.opened").length, 15); assert.equal(h.ofType("condition.opened")[0]!.applicationId, REFI);   // 14 PTD + the SFC 127 delivery condition (no 007 on a non-Texas LCOR)
