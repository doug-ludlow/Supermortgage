...
import { FAKE_SERVICER_CONTACT as SERVICER_CONTACT } from "../operations-runtime/servicing-config.ts";
...
test("32.8-T1: Given due Oct 1 with 15-day grace, then the badge is \"Payment due\" Oct 1–15 with the grace end shown, \"Past due\" from Oct 16 with the assessed late charge, and \"Current\" on posting.", { skip }, async () => {
  ...
  let rec = await record(A, loanId); assert.equal(status(rec).badge, "Payment due", JSON.stringify({ status: status(rec), seen: await db.query(`SELECT e.sequence::text AS seq, e.type, e.occurred_at, e.loan_id, e.application_id, e.payload FROM loan_events e, loans l WHERE l.id = $1 AND (e.application_id = l.origination_application_id OR e.loan_id = l.id) AND e.type IN ('installment.due_date_reached', 'payment.posted', 'payment.reversed') ORDER BY e.sequence`, [loanId]), posted: (await events(loanId, "payment.posted")).map((e) => [e.occurred_at, e.payload["payment_id"], e.payload["installments"], e.payload["received_on"]]), reversed: (await events(loanId, "payment.reversed")).map((e) => e.payload["payment_id"]) })); assert.equal(status(rec).one_liner, "account.payment_due"); assert.deepEqual(status(rec).one_liner_tokens, { date: "2027-10-01", grace_end: "2027-10-18" });
...
test("32.8-T4: Given `ach.return.received{R01}`, then `AUTODRAFT-RETURN-v1` renders, one retry is scheduled in 3–5 banking days, and a second R01 moves the enrollment to `suspended_returns` with a re-activation `ChoiceCard`.", { skip }, async () => {
  ...
  assert.equal((await entity("payments", pay1))!["status"], "posted"); assert.equal((await loanCashState(runtime, loanId, D("2028-01-02"))).state.installments.find((x) => x.due_date === "2028-01-01")!.status, "satisfied", JSON.stringify({ payment: await entity("payments", pay1), settle: s1.events.map((e) => [e.type, e.payload]), posted: (await events(loanId, "payment.posted")).slice(-2).map((e) => [e.occurred_at, e.payload]), terms: await entity("loan_terms", loanId) }, (_k, v) => (typeof v === "bigint" ? v.toString() : v)));
