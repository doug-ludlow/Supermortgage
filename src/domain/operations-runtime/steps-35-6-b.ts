...
async function calendar(ctx: StepContext, closing: ClosingFacts): Promise<{ funding_type: string; scheduled_funding_date: string; earliest_funding_date: string; rescission_expires_on: string | null }> {
  const rec = ctx.rec;
...
const closingScheduled: StepDef = {
  ...
  actions: async (ctx) => {
    ...
    if (!cd) {
      ...
      const prepaid = cents(interest["prepaid_interest_cents"]); const perDiem = cents(interest["per_diem_cents"]); const prepaidDays = Number(interest["prepaid_days"] ?? 0);
      ...
      const fees: Row[] = [...creditor.value.map((f) => ({ ...f })), ...quote.fees.map((f) => ({ fee_code: f.fee_code, description: f.fee_code === "title_lender_policy" ? "Title — Lender's policy" : f.fee_code === "settlement_fee" ? "Title — Settlement fee" : "Recording fees", amount_cents: f.amount_cents, section: f.fee_code === "recording" ? "E_taxes_gov" : "C_can_shop", tolerance_class: "ten_percent", source_id: saSource })),
        { fee_code: "prepaid_interest", description: `Prepaid interest ($${(Number(perDiem) / 100).toFixed(2)} per day, ${prepaidDays} days from ${cal.scheduled_funding_date})`, amount_cents: String(prepaid), section: "F_prepaids", tolerance_class: "unlimited", source_id: creditorSource },
      ...
      const rendered = await ctx.run<Row>({ process: "25.2", name: "renderCd", actor: DISCLOSURE, input: { disclosure_id: disclosureId, cd_version: version, transaction_type: rec.app.transaction_type === "purchase" ? "purchase" : "refinance", state: closing.state, required_consumer_ids: parties.borrower_ids,
        loan: { loan_amount_cents: String(terms.loan_amount_cents.value), rate_pct: terms.note_rate_pct.value, term_months: terms.term_months, pi_cents: String(note["pi_cents"] ?? terms.pi_cents.value), product: "Fixed Rate", loan_type: "Conventional", purpose: rec.app.transaction_type === "purchase" ? "Purchase" : "Refinance", prepayment_penalty: false, balloon: false, arm: false, loan_id_number: rec.app.id, mic_number: null, first_payment_date: String(note["first_payment_date"]), maturity_date: String(note["maturity_date"]) },
...
const cdDelivered: StepDef = {
  ...
  actions: async (ctx) => {
    ...
    const gate = { final_cd_delivered: true, approval_ptd_cleared: true, trust_poa_gate_open: true, compliance_pass_cd_gate_open: rec.has("compliance.gate.opened", (p) => p["gate"] === "cd"), lock_status: "active", lock_expires_on: terms.lock_expires_on, closing_date: closing.scheduled_note_date };
    const gateSources = { final_cd_delivered: src("entity", `disclosures:${cd.id}:${cd.version}`, "25.2"), approval_ptd_cleared: src("event", `clear_to_close.issued:${ctc.id}`, "23.3"), compliance_pass_cd_gate_open: src("event", `compliance.gate.opened:${rec.last("compliance.gate.opened", (p) => p["gate"] === "cd")?.id ?? ""}`, "25.1"), lock: src("event", `lock.executed:${terms.lock.id}`, "21.4"), cash_to_close: src("event", `funds_to_close.reconciled:${ftc.reconciled!.id}`, "22.4") };
    ...
    if (!setRow.data["snapshot_id"]) {
      ...
      const snapshot: Row = { application_id: rec.app.id, cd_version: Number(cd.data["cd_version"] ?? 1), du_submission_number: String(rec.payload("du.findings.received")?.["submission_number"] ?? "1"), lock_id: String(terms.lock.payload["lock_id"]), partner: { legal_name: parties.partner_legal_name, nmlsr_id: parties.partner_nmlsr_id, mers_org_id: parties.partner_mers_org_id ?? "" }, mlo_of_record: { name: parties.mlo_name, nmlsr_id: parties.mlo_nmlsr_id }, servicer: { name: "Supermortgage LLC", payment_address: "PO Box 1, Phoenix AZ 85001" },
        ...
        loan_amount_cents: String(terms.loan_amount_cents.value), note_rate_pct: terms.note_rate_pct.value, term_months: terms.term_months, product: "fixed", note_date: closing.scheduled_note_date, scheduled_disbursement_date: cal.scheduled_funding_date, scheduled_closing_date: closing.scheduled_note_date, escrowed: !!escrow, rescindable: closing.rescindable,
        enote_default: closing.note_form === "enote", partner_emortgage_approved: closing.note_form === "enote", ron_authorized_state: closing.closing_type === "ron", settlement_agent_eclosing_eligible: closing.closing_type !== "wet", borrower_declined_electronic: closing.closing_type === "wet", ...(min ? { min } : {}) };
    ...
    await ctx.run({ process: "26.1", name: "releaseToSettlementAgent", actor: CLOSER, input: { set_id: setId, released_to_party_id: closing.settlement_agent_party_id, facts: { qc_pass_gate_open: true, template_version_gate_open: true } }, detail: { sources: { qc: src("event", `closing.document_qc.passed:${qc.id}`, "26.1"), settlement_agent: src("entity", `closings:${closing.row.id}:${closing.row.version}`, "26.2") } } });
    return {};
  },
};
...
function recordSnapshot(rec: OrchRecord, closing: ClosingFacts, now: string, terms: ReturnType<typeof loanTerms>, apr: EntityRecord, cd: EntityRecord): Row {
  return { application_id: rec.app.id, as_of: civil(rec, now), property_state: closing.state, lien_position: "first", loan_amount_cents: terms.loan_amount_cents.value, note_rate_pct: terms.note_rate_pct.value, term_months: terms.term_months, rate_set_date: civil(rec, terms.lock.occurredAt), apr: { actual: aprBig(apr.data), disclosed_apr: String((cd.data["figures"] as Row | undefined)?.["apr_pct"] ?? apr.data["apr_disclosed_str"]), disclosed_finance_charge_cents: cents(apr.data["finance_charge_cents"]), transaction: { irregular_first_period: true } }, escrow_established: true };
}
...
async function pollRon(ctx: StepContext): Promise<{ done: boolean; sealed: boolean }> {
  ...
  for (const e of feed.pending(closing.closing_id, timeline, now)) {
    ...
    if (!already) {
      ...
      if (e.op === "seal") {
        if (closing.note_form === "enote") { const copy = feed.authoritativeCopy(min, (Number(loanTerms(rec).loan_amount_cents.value) / 100).toFixed(2), loanTerms(rec).note_rate_pct.value); await ctx.run({ process: "26.2", name: "validateAuthoritativeCopy", actor: CLOSER, input: { op: "seal", closing_id: closing.closing_id, seal_hash: sha256(copy), signing_completed_at: e.detail["signing_completed_at"], authoritative_copy_ref: `EV-${rec.app.id.slice(0, 8)}`, tamper_sealed_at: e.detail["tamper_sealed_at"] }, detail }); }
...
const documentsReleased: StepDef = {
  ...
  actions: async (ctx) => {
    ...
    if (!rec.has("closing.pre_session_checks.passed")) {
      ...
      const facts = { ctc: { ctc_issued: !!ctc, checklist_passed: !!ctc, decision_status: "active" }, le: { earliest_consummation_date: S(le?.["earliest_consummation_date"]) }, cd: { earliest_consummation_date: S(wp?.["earliest_consummation_date"]), receipts_complete: !!wp?.["earliest_consummation_date"] }, signing_package: rec.borrowerIds().map((c) => ({ consumer_id: c, copies: 2, channel: closing.closing_type === "wet" || !consents[c] ? "in_person" : "esign", esign_consent_id: consents[c]?.consent_id ?? null, material_disclosures_in_package: true, receipt_capture: true })), fraud_hold: { fraud_hold: !!fraud }, compliance_consummate_gate_open: rec.has("compliance.gate.opened", (p) => p["gate"] === "consummation" || p["gate"] === "consummate"), vvoe_within_10bd: vvoe ? vvoe.payload["within_window"] !== false : false, mi_commitment_valid: rec.entities("mi_certificates").length === 0 || rec.entities("mi_certificates", (d) => d["status"] === "active").length > 0, lock_valid_through_closing: terms.lock_expires_on >= closing.scheduled_note_date, documents_released: true,
...
const consummated: StepDef = {
  ...
  actions: async (ctx) => {
    ...
    if (closing.note_form === "enote") {
      ...
      if (!enote?.data["authoritative_copy_validated_at"]) { const terms = loanTerms(rec); const copy = ctx.fakes.ron.authoritativeCopy(S(enote?.data["min"]), (Number(terms.loan_amount_cents.value) / 100).toFixed(2), terms.note_rate_pct.value); await ctx.run({ process: "26.2", name: "validateAuthoritativeCopy", actor: CLOSER, input: { closing_id: closing.closing_id, authoritative_copy: copy }, detail: { sources: { seal: src("entity", `enotes:${enote?.id ?? ""}`, "26.2"), copy: src("platform", "FAKE RON platform authoritative copy", "35.6") } } }); rec = await ctx.refresh(); }
...
const executionReviewed: StepDef = {
  ...
  actions: async (ctx) => {
    ...
    if (!rec.has("warehouse.advance.approved")) {
      ...
      const fid = S(rec.entities("warehouse_facilities").at(-1)?.id) ?? "WHF-1";
...
const wireReleased: StepDef = {
  ...
  actions: async (ctx) => {
    ...
    if (!rec.has("warehouse.advance.funded")) {
      if (!rec.has("warehouse.wire.package.prepared") && !rec.entities("escalations").length && !(await ctx.rt.db.query(`SELECT 1 FROM escalations WHERE application_id = $1 AND owner_role = 'funding_approver' AND completed_at IS NULL AND payload->'package'->>'advance_id' = $2`, [rec.app.id, advanceId])).length) {
        const fid = S(rec.entities("warehouse_facilities").at(-1)?.id) ?? "WHF-1";
        await ctx.run({ process: "27.1", name: "prepareWire", actor: WAREHOUSE, input: { advance_id: advanceId, facility_id: fid, partner_haircut_reserve_cents: String(rec.entities("warehouse_facilities").at(-1)?.data["partner_haircut_reserve_cents"] ?? "25000000"), partner_contribution_cents: String(rec.payload("warehouse.advance.approved")?.["partner_contribution_cents"] ?? "0") }, detail: { sources: { approved: src("event", `warehouse.advance.approved:${rec.last("warehouse.advance.approved")?.id ?? ""}`, "27.1"), accepted: src("event", `funding.wire.accepted:${accepted.id}`, "26.3") } } });
      }
      return { wait: { status: "waiting_human", waiting_on: "funding_approver", clocked: true } };
    }
...
export const unwindStep: StepDef = {
  ...
  actions: async (ctx): Promise<StepOutcome> => {
    ...
    if (!rec.entities("funding_unwinds", (d) => d["funding_id"] === fundingId).length && !cancelled) {
      const trigger = exercised ? (rec.has("loan.funded") ? "rescission_exercised_post_disbursement" : "rescission_exercised_pre_disbursement") : "borrower_withdrew";
      await ctx.run({ process: "26.3", name: "openUnwind", actor: FUNDER, input: { funding_id: fundingId, op: "open", unwind_id: `UNW-${rec.app.id.slice(0, 8)}`, trigger, ...(exercised ? { exercise: { exercise_id: String(exercised.payload["exercise_id"]), refund_due_at: exercised.payload["refund_due_at"] ?? null } } : {}), enote: closing?.note_form === "enote", security_instrument_recorded: rec.has("recording.confirmed"), prior_lien_paid: false, at: now }, detail: { sources: { trigger: exercised ? src("event", `rescission.exercised:${exercised.id}`, "25.3") : src("event", `orchestration.unwind.requested:${rec.last("orchestration.unwind.requested")?.id ?? ""}`, "35.6") } } });
      rec = await ctx.refresh();
    }
    // the eNote's registration is reversed on the eRegistry (26.2; 26.4's MIN reversal when a 26.4 SOR row carries the MIN)
    if (closing?.note_form === "enote" && rec.has("enote.registered") && !rec.has("enote.registration.reversed") && !rec.has("mers.min.reversed")) {
      const min = S(rec.entities("enotes", (d) => d["closing_id"] === closing.closing_id).at(-1)?.data["min"]);
      const sor = min ? rec.entities("mers_registrations", (d) => d["min"] === min).at(-1) : undefined;
      if (sor) await ctx.run({ process: "26.4", name: "registerMin", actor: POST_CLOSING, input: { op: "reverse", min, reason: exercised ? "rescission_exercised" : "never_funded", submitted_at: now }, detail: { sources: { registration: src("entity", `mers_registrations:${sor.id}:${sor.version}`, "26.4") } } });
      else await ctx.run({ process: "26.2", name: "reverseRegistration", actor: CLOSER, input: { closing_id: closing.closing_id, reason: exercised ? "rescission_exercised" : "never_funded" }, detail: { sources: { registered: src("event", `enote.registered:${rec.last("enote.registered")?.id ?? ""}`, "26.2") } } });
    }
