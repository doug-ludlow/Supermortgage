...
- Baseline, read-only projection sources this process renders (owned by the sections in the Blueprint row; no table is re-declared): `applicant_demographics`, `application_borrowers`, `application_income`, `application_liabilities`, `application_properties`, `credit_authorizations`, `declarations`, `du_bankruptcy_filings`, `du_casefiles`, `du_declarations`, `du_residences`, `jurisdiction_rules`, `purchase_contracts`, `verifications`.
...
- **On webhook:** identity events satisfy `SM_IDENTITY_IAL2_GATE` (22.6). Extracted name/DOB/address written to `application_borrowers` with `source=stripe_identity`, then `ConfirmCard{fields: legal name, date of birth, current address, how you live there, time at this address}` — **Confirm** or **Edit** (a different mailing address is an edit). The residence basis — **Own** · **Rent** (with the monthly rent) · **Living rent-free** — and the months at the address are asked on every file (23.5 discrepancy 3: DU needs the current residence with its basis on every casefile); the tap writes the borrower's Current `du_residences` row through 23.5 `writeResidence` in the confirm's own transaction (command `application.confirmField{path=identity}`). The prior-residence ask (SQ-06: `ConfirmCard{prior address, how you lived there, months there}` → a Prior `du_residences` row) stays conditional on under two years at the current address — the stated months, or the credit report when it later shows less.
...
- **Tests:** 32.3-T4 (hard pull refused below L3 — `credit.authorize{hard_pull}` returns gate error); 32.3-T5 (extracted fields count as submitted only on Confirm — 21.1 rule 1; the residence basis on the same card); 32.3-T32 (the Current residence with its basis, the Prior residence under two years).

###### E6 · Consents
Decided 2026-09-13 (32.17 rule 20, least conservative): the three consents are **one statement under the goal card** (E3; and under 15's Show me my rate) — `props.statement`, `statement_version = consents-on-goal-2026-09` — and the tap on that card is the affirmation of all three. The flow writes the rows on `application.received` with `method = single_tap`, the statement's hash as the E-SIGN and TCPA text version and the credit sentence's own hash as the authorization's; no `ConsentCard`, no checkbox, no typed name. The assistant says the statement once in plain words before asking the goal and never asks for a separate yes. A borrower on a joint application still authorizes credit after joint intent on their own card (32.5 §7). What each row is, as before:
...
- **"None"** writes all thirteen as `false` in `declarations` with the list version hash as evidence, and the fourteen typed URLA section 5 answers to the borrower's `du_declarations` row through 23.5 `assertDeclarations` as the borrower's own actor (the thirteen listed items `No`; 5a.A — will you live in this home as your primary residence, and have you owned another home in the past three years — is asked on every file as the `ChoiceCard` that precedes the list, because a `No` written from a tap that never asked it would misstate occupancy; 23.5 rule 4 — and 5a.E — will the property carry a lien that could take priority over the first mortgage, a PACE / clean-energy lien — on its own `ChoiceCard` after it and before the list, for the same reason: neither is on the list, and neither is ever derived from another item's answer). **"Something applies"** → SQ-05: the fourteen questions one `ChoiceCard` at a time (Yes / No), each Yes opening its follow-up — 5a.A: the property usage and how title was held; bankruptcy: the chapter(s) — one tap each, "another?" between them — and a free-text explanation (`ExplanationCard`, optional, kept verbatim); undisclosed borrowed funds: the amount — and the last card's tap runs `application.answerDeclarations` once with all fourteen answers, their follow-ups, the chapters and the explanation. A "yes" on bankruptcy/foreclosure/short sale opens the waiting-period explanation from B3-5.3-07 in the assistant, never as a decline. The assistant never takes a declaration in words (32.16: `application.answerDeclarations` is never proposed; the reply points at the card).
- **Tests:** 32.3-T14 (thirteen explicit values written; list version hash logged; the fourteen typed answers asserted by the borrower's own actor); 32.3-T31 (the fourteen-question sequence, its follow-ups, one command).
...
| SQ-06 Residence history | < 2 years at current address (stated months, or the credit report) | prior address, basis and months → a Prior `du_residences` row |
...
| 32.3-T4 | Given L1 only, when the goal is tapped, then the hard-pull authorization is written on that tap — `credit.authorization.captured` twice over, 32.2's `hard_pull` and 20.3's own `hard_application` under one `authorization_id`, the goal card named on it — with no ConsentCard, no typed name and no identity gate in the way (32.17 rules 18 and 20), and the same goal tapped again writes nothing more. |
| 32.3-T5 | Given Stripe extracted "Jane Q. Public, 1990-04-01, 14 Elm St", when the borrower taps Edit on the address and confirms "22 Elm St", then `application_borrowers.current_address = "22 Elm St"` with `source = borrower`, and name/DOB carry `source = stripe_identity`, all with `confirmed_at`; the same card asks how she lives there — Own, Rent (with the monthly rent) or Living rent-free — and the months at the address, and the tap writes her Current `du_residences` row through 23.5 `writeResidence` in the confirm's own transaction. |
| 32.3-T6 | Given an in-app voice call, when the borrower says "yes, e-delivery is fine", then no `consents{kind=esign}` row becomes `active`: the row the goal's tap wrote stays `pending_verification` and only the e-mailed code activates it — a spoken yes is never a consent (20.3 T8). |
