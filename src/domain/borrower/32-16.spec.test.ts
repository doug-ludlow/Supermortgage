...
test("32.16-T31: Given the first turn with the goal card pending, then the model is prompted to lead with the Journey's top need (`next_in_words` comes from the Journey, the prompt version is `32.16-p4`) and the reply names that need in its own words.", { skip }, async () => {
  assert.equal(PROMPT_VERSION, "32.16-p4"); assert.match(SYSTEM_PROMPT, /the situation carries a journey/); assert.match(SYSTEM_PROMPT, /make a suggestion when there is an easier way/); assert.match(SYSTEM_PROMPT, /Be warm|warm, plain, quick/);
  ...
  const row = (await db.query<{ prompt_version: string }>(`SELECT prompt_version FROM agent_turns WHERE party_id = $1 ORDER BY created_at DESC LIMIT 1`, [a.party_id]))[0]!; assert.equal(row.prompt_version, "32.16-p4");
});
