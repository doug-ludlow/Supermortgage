...
async function nonprodRules(q: Queryable, environment: string, nowIso: string): Promise<{ findings: ScanFinding[]; tables: number; rows: number; coverage: string | null }> {
  ...
  // rule 6: a documents row is real data when it lives in a real object store (`gs://`, `s3://`, `http(s)://`) that is neither the FAKE store
  // (`metadata.blob_store = "fake-blob"`) nor a nonprod bucket; an in-process placeholder URI (35.8's `work://`, 35.9's `mem://`, 35.11's
  // `memory://`, 20.3's `notice://`, 32.2's `letter://`, the DU port's `du://`, `evidence://`, `report://`, `fake-queue://`, `fixture://`, `file://` — every
  // writer that has not yet moved onto 35.2's store) names no bucket at all, so it is outside the count either way
  const realBlobs = await n(q, `SELECT count(*)::text AS n FROM documents WHERE coalesce(metadata->>'blob_store', '') <> 'fake-blob' AND storage_uri ~ '^(gs|s3|https?)://' AND storage_uri NOT LIKE '%nonprod%' AND storage_uri NOT LIKE '%FAKE%'`);
