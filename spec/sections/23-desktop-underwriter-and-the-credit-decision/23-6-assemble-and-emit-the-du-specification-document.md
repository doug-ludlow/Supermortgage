...
4. **Conditionality at emission.** A data point marked required, or conditional with its condition true for this loan, that has no value is a refusal naming the XPath and the condition statement — not an empty element and not an omission.
...
- Sample DI-C09 (three borrowers, an asset with two owners, a liability with two obligors, an asset securing a liability, income items to employers) must round-trip exactly — it is the first golden file to make pass.
...
| 23.6-T5 | Given a required data point with no value, when emitted, then the emission is refused with `DU_REQUIRED_MISSING` naming the XPath; given a conditional data point whose condition statement is false for this loan, then its absence is accepted. |
