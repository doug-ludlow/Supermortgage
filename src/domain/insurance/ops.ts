...
import { lpiCoverage, firstNoticeContent, selectTrack, fpiClocks, type InsuranceType, type FpiTrack } from "./fpi.ts";
...
import { nextInspectionWindow, pfpipTaskDue, inspectionWindow } from "./inspection.ts";
...
export function vacancyConfirmed(f: { inspected_on: PlainDate; certification_signed: boolean; pfpip: boolean }): { occupancy: "vacant"; interior_schedule: { from: PlainDate; to: PlainDate }; carrier_notify_by: PlainDate; preservation_case: "opened"; pfpip_update_by: PlainDate | null } {
...
export function vacantRegistration(f: { trigger_on: PlainDate; rule: { within_days: number; renewal_months: number | null }; fee_cents: Cents }): { file_by: PlainDate; renew_on: PlainDate | null; fee_claim_cents: Cents; fee_basis: "actual_cost" } {
  const c = registrationClocks(f.trigger_on, f.rule); if (!c) throw new RangeError("no registration rule");
  return { file_by: c.file_by, renew_on: c.renew_on, fee_claim_cents: f.fee_cents, fee_basis: "actual_cost" };
}
