...
- **Thread** (left, under **Guide**) — the conversation. Messages from the `intake` agent (pre-funding) or `borrower-comms` agent (post-funding) and from the borrower. *Amended by docs/ux/17 §2.1 (DELTA-26):* no card renders in the thread — a card's one home is the rail (right), and the thread carries a one-line **reference chip** that focuses and expands it there, plus the **confirm chip** of a proposed value (17 §3.4); no sender label, badge or timestamp row on every line (provenance is an aria/hover detail); the automation disclosure is the footer of every screen (17 §1 principle 8), never a line in the log. *Amended by docs/ux/18:* the thread is Guide's navigation, not the only door. Workspace Home (shortcuts, Approvals, loan glance) is the post-auth default; the platform still puts the card where the borrower can tap it, and Guide remains one control away.
...
### 1.5 Navigation
- Route `/` → **Workspace Home** for the authenticated party (docs/ux/18). Unauthenticated → account / sign-in (`17` §2.0). A party with multiple applications/loans (a purchase in progress and a serviced loan; both loans across a refinance) gets a switcher in the header keyed by `application_id` / `loan_id`; Workspace Home and the Record follow the selection; the Guide thread is per party and shows all threads merged with the loan label on each message.
- Route `/guide` → Guide (Michelle): the conversation — thread + rail + action bar (`17` §2). Also reachable as a drawer from Workspace Home. The agent turn is not removed.
- Routes for external returns: `/return/{vendor}/{card_instance_id}` (Stripe, Plaid, Truv, RON), `/d/{deep_link_token}` (SMS/email deep links — §6.5), `/doc/{document_id}` (document viewer, authenticated). A `?card=` deep link lands on Workspace Home with that card expanded in Approvals (same `card_instances` row the rail would show).
- **Loan glance** is always visible on Workspace Home when a loan or application subject exists (status badge, key Numbers, autopay on/off when servicing applies). Numbers are not buried behind a default-collapsed “Your record” section on Home; the Guide rail may still collapse that section.
- Workspace Home is a dashboard: a projection of `borrower_record` and pending cards, not a second system. **No settings tree.** Preferences (contact permissions, e-delivery, language, autopay) are cards the borrower can request ("change my autopay day") and that the platform offers when relevant — or, later, a Workspace settings grouping of those same cards (18 W7), never an invented preference store.
