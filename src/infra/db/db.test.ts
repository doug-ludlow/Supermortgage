...
test("migrations: every file under db/migrations is applied to the test database (797 tables: public + restricted_fl)", { skip }, async () => {
  ...
  assert.equal(t!.c, 797n);   // the count a fresh `db/migrate.sh` run produces through 0210: 784 through 0150 (0150 adds 35.1's eight seam tables) plus 0170's five (35.7: role_grants, role_queue_snapshots, role_handovers, breakglass_uses, api_principals) plus 0210's eight (35.9: case_timelines, case_milestone_expectations, breach_action_registry, breach_actions, docket_reactions, firm_dispatches, claim_candidates, default_case_daily_runs); 0137–0141, 0151–0169 and 0171 create no table; the absolute number moves with every §35 migration that adds a table
});
