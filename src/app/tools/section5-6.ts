...
export const TOOLS_5_6: readonly ToolDef[] = defineTools("5.6", "investor-reporting", [
  ...
  { name: "recordDecision", kind: "write", handler: (i: ToolInput, ctx: CommandContext, rt: ToolRuntime) => {
      ...
      switch (kind) {
        case "": break;
        case "demand_received": { const d = ingestRepurchaseDemand(e, { repurchase_id, loan_id, received_on: date(i, "received_on"), demand_kind: (str(i, "demand_kind") || "repurchase") as DemandKind, amount_cents: cents(i.amount_cents), demand_document_id: str(i, "demand_document_id"), loan_liquidated: flag(i, "loan_liquidated"), lpi: str(i, "lpi") ? date(i, "lpi") : null });
          // rule 1 (A1-3-02 mandatory 24-month repurchase): the officer owns the demand from receipt — the repurchase must be reported in the month of the 24th past-due installment
          if (d.escalation === "officer") rt.escalations.open({ kind: "officer", severity: "sev1", loanId: loan_id, payload: { reason: "5.6 rule 1: A1-3-02 mandatory repurchase demand (22 months past due) — repurchase no later than 24 months past due, reported as activity in the month containing the 24th past-due installment's due date", repurchase_id, type: d.type, reporting_period: d.schedule!.reporting_period, due_date_24th: d.schedule!.due_date_24th, months_past_due_at_demand: d.schedule!.months_past_due_at_demand, pay_by: d.pay_by } }, ctx.actor);
          effect = d; break; }
