...
export const TOOLS_26_3: readonly ToolDef[] = defineTools("26.3", "funder", [
  ...
  { name: "evaluateFundingConditions", kind: "act", handler: compute((i, ctx, rt) => {
      ...
      const full: FundingConditions = evaluateFundingConditions(str(i, "funding_id"), { ...facts, as_of: facts.as_of || ctx.now, waivers });
      // op: pre_signing (wet states, the day before the session — 26.3 rule 6 / 35.6 open question 1): the subset every item of which can pass before signing; the post-signing items
      // (POST_SIGNING_FC) stay pending and are not blocking here — `passed` is the subset's, and the event names the subset
      const preSigning = i.op === "pre_signing";
      const c: FundingConditions = preSigning ? { ...full, checklist_id: `${full.checklist_id}:pre_signing`, passed: full.pre_signing_subset_passed, blocking_codes: full.blocking_codes.filter((code) => !POST_SIGNING_FC.includes(code)), pending_codes: full.pending_codes.filter((code) => !POST_SIGNING_FC.includes(code)) } : full;
      rt.store.put("funding_conditions", c.checklist_id, { ...(c as unknown as Record<string, unknown>), ...(preSigning ? { subset: "pre_signing" } : {}) }, ctx.actor, ctx.now);
      const ev = recordConditionsEvaluated(ctx.events, application_id, c, at(i, ctx), preSigning ? "pre_signing" : null);
