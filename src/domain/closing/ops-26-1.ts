...
/** B8-7-01: Maine uses the MERS Mortgage Assignment (Form 3749), executed at closing and recorded promptly after the mortgage; the Maine mortgage itself is NOT modified to name MERS as original mortgagee ("Loans in which the Maine security instrument has been modified to name MERS as the original mortgagee of record, solely as nominee for the seller/servicer, are ineligible for delivery to Fannie Mae"). */
export const MERS_ASSIGNMENT_STATES: readonly string[] = ["ME"];
/** Form 3200 signature block; E-2-07 d) Signature Area: the "[Sign Original Only]" text of the paper forms is omitted from eNote forms. */
export const NOTE_SIGNATURE_BLOCK = "WITNESS THE HAND(S) AND SEAL(S) OF THE UNDERSIGNED.";
export const SIGN_ORIGINAL_ONLY = "[Sign Original Only]";
/** The authorized change every `family=enote` template records (`document_templates.authorized_changes_applied`). */
export const ENOTE_SIGNATURE_AREA_CHANGE = "E-2-07 d) Signature Area: '[Sign Original Only]' omitted from the eNote form";
/** The MOM (MERS-as-original-mortgagee) paragraph of the 07/2021 security instruments — present everywhere except Maine (B8-7-01). */
export const MOM_NOMINEE_RE = /MERS is a separate corporation that is acting solely as a nominee for Lender/;
...
export const DEFAULT_TEMPLATE_LIBRARY: readonly DocumentTemplate[] = [
  ...
  T({ template_id: "3200e:2021-07", form_number: "3200e", family: "enote", smart_doc_profile: "v1_0_2_cat1_closing_dtd_2_3_1", authorized_changes_applied: [ENOTE_SIGNATURE_AREA_CHANGE] }),
  ...
  T({ template_id: "3441e:2021-07", form_number: "3441e", family: "enote", product_scope: ["arm_sofr"], smart_doc_profile: "v1_0_2_cat1_closing_dtd_2_3_1", authorized_changes_applied: [ENOTE_SIGNATURE_AREA_CHANGE] }),
...
export function selectDocumentSet(s: ClosingSnapshotInput, library: readonly DocumentTemplate[] = DEFAULT_TEMPLATE_LIBRARY): DocumentSetSelection {
  ...
  docs.push({ kind: "security_instrument", form_number: security_instrument_form, ...tpl(security_instrument_form), notarized: true, recordable: true, ...(MERS_ASSIGNMENT_STATES.includes(s.state) ? { note: "B8-7-01 Maine: unmodified mortgage — no MERS-as-original-mortgagee (MOM) language; a Maine security instrument modified to name MERS is ineligible for delivery to Fannie Mae; MERS takes by the Form 3749 assignment carrying the MIN" } : {}) });
...
export interface RenderedDocument {
  ...
  /** the MOM (MERS-as-nominee) paragraph is on the instrument — required everywhere except Maine, where it makes the loan ineligible (B8-7-01). */ readonly mom_language_present: boolean;
}
/** Parsed back from the rendered text (older stored rows carry no flag). */
export const hasMomLanguage = (d: Pick<RenderedDocument, "text"> & { mom_language_present?: boolean }): boolean => d.mom_language_present ?? MOM_NOMINEE_RE.test(d.text);
/** The 07/2021 security-instrument MOM paragraph (authorized change; B8-7-01: "show MERS as the nominee for the seller/servicer, define and name the originating seller/servicer"). */
export const momNomineeParagraph = (s: ClosingSnapshotInput): string => `"MERS" is Mortgage Electronic Registration Systems, Inc. MERS is a separate corporation that is acting solely as a nominee for Lender and Lender's successors and assigns. MERS is the ${DEED_OF_TRUST_STATES.includes(s.state) ? "beneficiary" : "mortgagee"} under this Security Instrument. "Lender" is ${lender(s)}.`;
/** B1-1-01: a set is POA-executed when the note or security instrument is signed by an attorney-in-fact. */
export const poaExecutedSet = (s: Pick<ClosingSnapshotInput, "poa" | "borrowers">): boolean => !!s.poa || s.borrowers.some((b) => !!b.attorney_in_fact);
...
export function renderDocument(sel: SelectedDocument, s: ClosingSnapshotInput, terms: NoteTerms, min: string, snapshot_hash: string, selection: DocumentSetSelection, extra: { buydown?: BuydownAgreement | null; cema?: CemaPackage | null; instructions?: ClosingInstructionsContent | null } = {}): RenderedDocument {
  ...
  switch (sel.kind) {
    case "note": case "enote": {
      ...
      lines.push(e ? NOTE_SIGNATURE_BLOCK : `${NOTE_SIGNATURE_BLOCK} ${SIGN_ORIGINAL_ONLY}`);   // E-2-07 d): "[Sign Original Only]" appears on the paper form only
    ...
    case "security_instrument": {
      ...
      if (MERS_ASSIGNMENT_STATES.includes(s.state)) lines.push(`"Lender" is ${lender(s)}. Lender is the mortgagee under this Security Instrument. (Maine: this Mortgage is not modified to name MERS as the original mortgagee of record — B8-7-01; MERS becomes mortgagee of record by the Form 3749 assignment recorded promptly after this Mortgage.)`);
      else lines.push(momNomineeParagraph(s));
      ...
      lines.push(`${s.state}—Single Family—Fannie Mae/Freddie Mac UNIFORM INSTRUMENT ${selection.riders.includes("3158") ? "(MERS Rider)" : MERS_ASSIGNMENT_STATES.includes(s.state) ? "(non-MOM; MERS by Form 3749 assignment)" : "(MERS)"} Form ${sel.form_number} ${sel.revision_date ?? ""}`);
      break;
    }
  ...
  return { document_id: `DOC-${sha256(`${s.application_id}|${sel.kind}|${sel.form_number}|${data_hash}`).slice(0, 16)}`, kind: sel.kind, form_number: sel.form_number, template_id: sel.template_id, template_revision_date: sel.revision_date, data_hash, snapshot_hash, render_hash: sha256(text), text,
    signers, notarized, witness_count: sel.recordable ? jr.witness_count : 0, recordable: sel.recordable, retention_class: RETENTION_LOAN_FILE, execution_status: "unsigned", nmlsr_block: nmlsr ? nmlsrBlock(s) : null, min_present, mom_language_present: MOM_NOMINEE_RE.test(text) };
}
...
/** B3-4.1-02 IPC cap for principal residences and second homes: 3% above 90% CLTV; 6% for 75.01%–90%; 9% at 75% or less — exactly 90% is in the 6% band (worked example 3: $457,777.78 × 6% = $27,466.67). */
...
/** B1-1-01: the only circumstances in which a POA may execute both the initial and the final Form 1003 (24.4 `poa_reviews` records which applies). */
export type Poa1003Exception = "military_service_outside_us" | "attorney_in_fact_personal_capacity" | "applicable_law";
...
export interface QcUpstream {
  ...
  readonly title: { vesting_text: string; legal_description: string }; readonly urla_1003: { org_nmlsr_id: string; mlo_nmlsr_id: string; loan_amount_cents: Cents; note_rate_pct: string; term_months: number; /** B1-1-01: the initial 1003 bears the borrower's personal signature (required when the note/security instrument and the final 1003 are POA-executed). */ initial_1003_personally_signed?: boolean | null; /** the B1-1-01 exception recorded in 24.4 `poa_reviews` that lets a POA sign both 1003s. */ poa_1003_exception?: Poa1003Exception | null };
...
export function runDocumentQc(s: ClosingSnapshotInput, terms: NoteTerms, selection: DocumentSetSelection, docs: readonly RenderedDocument[], up: QcUpstream, waivers: readonly { rule_code: QcRuleCode; waived_by_role: string; reason: string }[] = []): { checks: QcCheck[]; passed: boolean; hard_failures: QcRuleCode[]; soft_failures: QcRuleCode[] } {
  ...
  const termsOk = up.urla_1003.loan_amount_cents === terms.principal_cents && pct3(up.urla_1003.note_rate_pct) === terms.note_rate_pct && up.urla_1003.term_months === terms.term_months;
  const poaExecuted = poaExecutedSet(s); const initialOk = !poaExecuted || up.urla_1003.initial_1003_personally_signed === true || !!up.urla_1003.poa_1003_exception;
  checks.push(C("DQC_1003_FINAL_TERMS", termsOk && initialOk, `${formatCents(terms.principal_cents)} ${terms.note_rate_pct}% ${terms.term_months}m${poaExecuted ? "; initial 1003 personally signed by the borrower or a B1-1-01 exception recorded in 24.4 poa_reviews" : ""}`, `${formatCents(up.urla_1003.loan_amount_cents)} ${up.urla_1003.note_rate_pct}% ${up.urla_1003.term_months}m${poaExecuted ? `; initial 1003 ${up.urla_1003.initial_1003_personally_signed ? "personally signed" : up.urla_1003.poa_1003_exception ? `POA-signed under exception ${up.urla_1003.poa_1003_exception}` : "POA-signed, no exception recorded"}` : ""}`, !termsOk ? "B1-1-01: final 1003 must reflect the final loan terms" : "B1-1-01: when the note or security instrument and the final Form 1003 are executed under a POA, the initial Form 1003 must be personally signed by the borrower unless the borrower is on military service outside the United States or aboard a U.S. vessel, the attorney-in-fact signs the security instrument in a personal capacity for their own ownership interest, or the lender determines applicable law requires it (24.4 poa_reviews)", ids(urla)));
  ...
  const nonMom = MERS_ASSIGNMENT_STATES.includes(s.state); const siMom = si ? hasMomLanguage(si) : null; const momOk = siMom === null ? true : nonMom ? !siMom : siMom;
  checks.push(C("DQC_MIN_CHECK_DIGIT", isValidMin(min) && recordables.every((d) => d.min_present) && momOk, `valid 18-digit MIN on the security instrument and each rider; MOM (MERS-as-nominee) paragraph ${nonMom ? "absent from the security instrument (Maine — B8-7-01)" : "present on the security instrument"}`, `${min || "none"}; on ${recordables.filter((d) => d.min_present).length}/${recordables.length} recordable instruments; MOM paragraph ${siMom === null ? "n/a" : siMom ? "present" : "absent"}`, nonMom && siMom ? "B8-7-01: loans in which the Maine security instrument has been modified to name MERS as the original mortgagee of record, solely as nominee for the seller/servicer, are ineligible for delivery to Fannie Mae" : !momOk ? "B8-7-01: the security instrument must show MERS as the nominee for the seller/servicer (MOM paragraph missing)" : "MIN check digit invalid or MIN missing from a recordable instrument", ids(si)));
  if (selection.enote) {
    ...
    // E-2-07 d) Signature Area: the eNote omits "[Sign Original Only]" and every eNote template records that authorized change
    const signOriginal = !!note && note.text.includes(SIGN_ORIGINAL_ONLY); const enoteTemplates = up.templates.filter((t) => t.family === "enote"); const changeRecorded = enoteTemplates.every((t) => t.authorized_changes_applied.some((c) => c.includes(SIGN_ORIGINAL_ONLY)));
    checks.push(C("DQC_ENOTE_ARC_VIEW", !!enote && enote.ok && enote.arcs.length === Object.keys(enote.data).length && enote.tamper_seal_algorithm === "SHA-256" && enote.profile === "v1_0_2_cat1_closing_dtd_2_3_1" && !signOriginal && changeRecorded, "every DATA point has an ARC to VIEW; SHA-256 seal; v1.0.2 Category One; E-2-07 d) '[Sign Original Only]' omitted and recorded in authorized_changes_applied", `${enote ? (enote.ok ? `${enote.arcs.length} ARCs; ${enote.tamper_seal_algorithm}` : enote.reason) : "no SMART Doc (MIN invalid or note missing)"}; [Sign Original Only] ${signOriginal ? "present" : "omitted"}; change ${changeRecorded ? "recorded" : "not recorded"} on ${enoteTemplates.length} eNote template(s)`, signOriginal || !changeRecorded ? "E-2-07 d) Signature Area: the text '[Sign Original Only]' which appears on the paper forms should be omitted from eNote forms (recorded in the template's authorized_changes_applied)" : "SMART Doc ARC/VIEW validation failed", ids(note)));
  }
...
export function draftClosingInstructions(s: ClosingSnapshotInput, selection: DocumentSetSelection, docs: readonly RenderedDocument[], i: { set_id: string; version: number; settlement_agent_party_id: string; wire_verification_id: string; wire_verification_match_result: string; platform?: string | null; session_id?: string | null; ron_provider?: string | null; notary_commission_state?: string | null; state_requires_wet_lender_signature?: boolean }): ClosingInstructionsContent {
  ...
  const recording = [`Record the ${securityInstrumentName(s.state)} (Form ${selection.security_instrument_form}) first, with every rider attached, showing MIN ${s.min ?? ""}; then any affidavit (${s.tx_50a6 ? "Form 3185" : "none"}).`, ...(mersRider ? ["MERS Rider (Form 3158) attached; a post-closing assignment to MERS is prohibited in this state (B8-7-01)."] : []), ...(maine ? ["Maine: the Mortgage names the Lender as mortgagee of record and carries no MERS-as-original-mortgagee language (B8-7-01: a Maine mortgage modified to name MERS is ineligible for delivery to Fannie Mae).", "Maine: record the MERS Mortgage Assignment (Form 3749), executed at closing by the Lender's signing officer, promptly after the Mortgage."] : []), "eRecord through the Lender's eRecording vendor where the county accepts it; paper fallback with tracking otherwise (26.2)."];
