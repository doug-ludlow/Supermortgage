...
1. **One directory for every person.** A homegrown party and a partner-book party are the same row kind (`parties{borrower}`); the directory shows the origin (`front door`, `video door`, `partner book: <partner>`), the first session, and the doors used since. A person with no session yet (invited, never signed in) is listed as `invited` with the invitation and reminder dates. The browse of every person — newest first, with counts, filters and paging — is 34.5's list (`directory.list`, `directory.counts`); the search here is unchanged. The "not yet an account" rows (a lead that never authenticated, an interview application with no linked party, a boarded borrower with no party) are 34.5's and are not persons under this rule.
...
4. **Search never leaks.** Results carry names and masked contact only; the query is logged as a hash; fewer than three characters is refused; more than 50 matches asks for a narrower query. The three-character floor and the fifty-result cap apply to the search; the list pages by a cursor of 50 under 34.5's `LIST_VOLUME` guardrail, and the count of people is a separate read that returns no row.
...
6. **The directory writes nothing to a borrower's record.** The actions the account page dispatches are named: re-send an invitation (33.1 `account.invite{kind: reminder}`, one per party by an operator, with a reason, under 33.1's amended edge case — `REMINDER_CAP_1` on the second), unmask (`compliance` or `officer`, rule 2), export (`compliance`, rule 5) and the notice look (34.5 `notice.read`); each is the owning section's command, dispatched with the staff actor (34.1 rule 3), and appears in the stream as its own event. Opening a case or an escalation on a subject is the owning section's or 35.8's command (`work.item.open`), dispatched from the page when it exists — this process has no tool that opens one.
...
- A video-door party that has not identified (`Borrower (video)`, `contact = {provisional: video}`) → counted in 34.5's "video visits, not identified", never listed and never searchable as a person, whether its session is open or closed (32.17: nothing of it is shown to anyone else); it carries no subject in any list, and its placeholder application is no subject of any row. It becomes a row the moment `video.identify` has run.
