...
import {
  ...
  mandatoryPairOff, mandatoryExtensionFee, extensionCapCheck, hblCap, pairOffSizing, postPairOffMinimum, autoExtensionCap, dailyLimitCheck, duApproveWindow, reconcileDraft, keyDataChangeDue, closedStatusDue, loanAgeDueOn, expectedPurchaseReadyDate, COMMITTING_FEE_EXPENSE, PARTNER_REIMBURSABLE_FROM_SM,
...
test("29.1-T11: Given `execution.mandatory_enabled=false`, when 29.2 requests a $5,000,000 mandatory commitment, then `prepareMandatoryPackage` is refused; with the flag true and an approved `officer_mandatory_authorization`, the operator package shows `tolerance_low_cents = 487,500,000`, `tolerance_high_cents = 512,500,000`, `over_delivery_cap_cents = 125,000,000`, five PTRs 5.625–6.125%, `expires_on = 2026-12-07`.", async () => {
  ...
  // C2-2-01 (VERIFIED): high-balance loans ≤ 10% of the aggregate UPB of a standard fixed-rate mandatory commitment — the column defaults to 10; $500,000 on $5,000,000; non-TBA products (ARMs) carry no concentration limit
  assert.equal(c.hbl_cap_pct, "10.00"); assert.equal(pkg.hbl_cap_pct, "10.00"); assert.equal(pkg.max_allowable_hbl_delivery_cents, "50000000"); assert.equal(DEFAULT_POLICY.hbl_cap_pct, "10");
  assert.deepEqual(hblCap(500_000_000n, "10"), { hbl_cap_pct: "10.00", max_allowable_hbl_delivery_cents: 50_000_000n }); assert.deepEqual(hblCap(500_000_000n, null), { hbl_cap_pct: null, max_allowable_hbl_delivery_cents: null });
...
test("29.1-T12: Given a mandatory commitment (price 101.250) with $4,600,000 purchased on Fri Dec 4, 2026 and a live price of 100.750, then the pair-off is sized at the full undelivered $400,000 (a standalone $275,000 pair-off is refused as short of the C2-2-01 post-pair-off minimum — revised commitment − $50 — unless the other $125,000 is extended with it, C2-1.1-04) and yields `cash_back_cents = 200,000`; with a live price of 101.750 it yields `fee_cents = 200,000`; a 10-day extension of the $400,000 remaining balance at the lowest PTR 5.625% costs `fee_cents = 62,500`.", async () => {
  ...
  // by Fri Dec 4 the partner has purchased $4,600,000 against it — $400,000 undelivered ($275,000 short of the $4,875,000 lower band, but a pair-off is measured from the original commitment amount, not the low tolerance: C2-1.1-02)
  ...
  // a standalone $275,000 pair-off is refused: once requested, the minimum delivery is the revised commitment − $50 = $4,725,000 − $50 = $4,724,950 against $4,600,000 delivered (C2-2-01)
  const short = await h.refused(h.run("preparePairOffPackage", { commitment_id: c.commitment_id, amount_cents: 27_500_000n, market_price: "100.750", at: et("2026-12-04", "14:30"), authorization_escalation_id: poAuth, reason: "shortfall to the lower tolerance band" }), "FNMA_C2_2_01_POST_PAIR_OFF_MINIMUM");
  assert.match(short.message, /\$4,724,950\.00/); assert.match(short.message, /whole undelivered \$400,000\.00/); assert.match(short.message, /extend the other \$125,000\.00 with it/);
  assert.deepEqual(postPairOffMinimum(500_000_000n, 27_500_000n), { revised_commitment_cents: 472_500_000n, minimum_delivery_cents: 472_495_000n });
  const alone = pairOffSizing({ original_amount_cents: 500_000_000n, purchased_cents: 460_000_000n, paired_off_cents: 0n, requested_cents: 27_500_000n }); assert.equal(alone.allowed, false); assert.equal(alone.rule, "C2-2-01"); assert.equal(alone.undelivered_cents, 40_000_000n); assert.equal(alone.post_pair_off_minimum_cents, 472_495_000n);
  assert.equal(h.svc.pairOffs.length, 0);   // nothing was prepared
  // … unless the other $125,000 is extended with it — the two sum to the whole $400,000 (C2-1.1-04); a mismatched sum is refused
  const combo = await h.run("preparePairOffPackage", { commitment_id: c.commitment_id, amount_cents: 27_500_000n, extension_remainder_cents: 12_500_000n, market_price: "100.750", at: et("2026-12-04", "14:30"), authorization_escalation_id: poAuth, reason: "pair-off plus an extension of the rest" });
  assert.equal(combo.amount_cents, 27_500_000n); assert.equal((combo.package as Record<string, unknown>).extension_remainder_cents, "12500000"); assert.equal((combo.package as Record<string, unknown>).undelivered_cents, "40000000"); assert.equal(combo.cash_back_cents, 137_500n);
  await h.refused(h.run("preparePairOffPackage", { commitment_id: c.commitment_id, amount_cents: 27_500_000n, extension_remainder_cents: 10_000_000n, market_price: "100.750", at: et("2026-12-04", "14:30"), authorization_escalation_id: poAuth }), "FNMA_C2_1_1_04_PAIR_OFF_EXTENSION_SUM");
  assert.equal(pairOffSizing({ original_amount_cents: 500_000_000n, purchased_cents: 460_000_000n, paired_off_cents: 0n, requested_cents: 27_500_000n, extension_remainder_cents: 12_500_000n }).allowed, true);
  // the pair-off sized at the full undelivered $400,000 at 2:30 p.m. ET (before the 5:00 p.m. ET pair-off cutoff on the expiration date): live 100.750 → cash back $2,000.00 (C2-1.1-04)
  const cb = await h.run("preparePairOffPackage", { commitment_id: c.commitment_id, market_price: "100.750", at: et("2026-12-04", "14:30"), authorization_escalation_id: poAuth, reason: "undelivered balance at expiry" });
  assert.equal(cb.amount_cents, 40_000_000n); assert.equal(cb.cash_back_cents, 200_000n); assert.equal(cb.fee_cents, 0n); assert.equal((cb.package as Record<string, unknown>).cash_back_display, "$2,000.00"); assert.equal((cb.package as Record<string, unknown>).request_by, et("2026-12-07", "17:00"));
  assert.equal((cb.package as Record<string, unknown>).post_pair_off_minimum_cents, "459995000"); assert.equal((cb.package as Record<string, unknown>).revised_commitment_cents, "460000000"); assert.match(String((cb.package as Record<string, unknown>).measured_from), /original commitment amount, not the low delivery tolerance/); assert.match(String((cb.package as Record<string, unknown>).pair_off_cutoff), /5:00 p\.m\. ET on the expiration date/);
  // with a live price of 101.750 it is a fee of $2,000.00 (below the $2,500 officer threshold)
  const fee = await h.run("preparePairOffPackage", { commitment_id: c.commitment_id, market_price: "101.750", at: et("2026-12-04", "14:31"), authorization_escalation_id: poAuth, reason: "undelivered balance at expiry" });
  assert.equal(fee.amount_cents, 40_000_000n); assert.equal(fee.fee_cents, 200_000n); assert.equal(fee.cash_back_cents, 0n); assert.equal((fee.package as Record<string, unknown>).fee_display, "$2,000.00"); assert.equal((fee.package as Record<string, unknown>).officer_approval_required, false);
  assert.deepEqual(mandatoryPairOff(40_000_000n, "101.250", "100.750"), { fee_cents: 0n, cash_back_cents: 200_000n, price_delta: "-0.500" }); assert.deepEqual(mandatoryPairOff(40_000_000n, "101.250", "101.750"), { fee_cents: 200_000n, cash_back_cents: 0n, price_delta: "0.500" });
  // the alternative: a 10-day extension of the $400,000 remaining balance at the lowest PTR 5.625% costs $625.00 (Fannie Mae accepts extension requests until 10:00 p.m. ET, C2-1.1-04; SM policy targets 5:00 p.m. ET)
  ...
  // executing the $400,000 pair-off leaves nothing undelivered: the revised commitment is $4,600,000, its minimum delivery $4,599,950 (C2-2-01) and the $4,600,000 purchased fulfils it; the cash back reconciles to committing_cash_back_receivable
  ...
  assert.equal(done.status, "executed"); assert.equal(done.amount_cents, 40_000_000n); assert.equal(done.cash_back_cents, 200_000n); const fin = h.commitment(String(c.commitment_id));
  assert.equal(fin.remaining_balance_cents, 0n); assert.equal(fin.paired_off_cents, 40_000_000n); assert.equal(fin.purchased_cents, 460_000_000n); assert.equal(fin.tolerance_low_cents, 459_995_000n); assert.equal(fin.status, "fulfilled");
  const po = h.ofType("commitment.paired_off")[0]!; assert.equal(po.payload.amount_cents, "40000000"); assert.equal(po.payload.cash_back_cents, "200000"); assert.equal(po.payload.revised_commitment_cents, "460000000"); assert.equal(po.payload.post_pair_off_minimum_cents, "459995000");
  const ful = h.ofType("commitment.fulfilled")[0]!; assert.equal(ful.payload.within_tolerance, true); assert.equal(ful.payload.after_pair_off, true); assert.equal(ful.payload.purchased_cents, "460000000"); assert.equal(h.timer("FNMA_C2_1_1_03_MAND_COMMITMENT_EXPIRY")!.status, "satisfied"); assert.equal(h.timer("FNMA_PEWL_MAND_TOLERANCE_GATE")!.status, "satisfied");
  assert.deepEqual(evaluateGate("29.1.mandToleranceGate", { original_amount_cents: 500_000_000n, purchased_cents: 460_000_000n, paired_off_cents: 40_000_000n }), { open: true }); assert.equal(evaluateGate("29.1.mandToleranceGate", { original_amount_cents: 500_000_000n, purchased_cents: 460_000_000n, paired_off_cents: 27_500_000n }).open, false);
  const rec = await h.run("reconcileFeeDrafts", { drafts: [{ draft_id: "FD-2026-12-07-0002", notification_date: "2026-12-07", draft_date: "2026-12-08", commitment_id_fnma: "MAND-2026-0001", fee_type: "pair_off", amount_cents: -200_000n }] });
  assert.equal(rec.matched, 1); assert.equal(h.ledger.sets()[0]!.lines[0]!.account.account, "committing_cash_back_receivable"); assert.equal(h.ledger.sets()[0]!.lines[0]!.amountCents, 200_000n);
});
...
test("29.1 worked figures: the refinance and purchase fixtures, the pair-off, extension and mandatory arithmetic reproduce the spec's dollar amounts", () => {
  ...
  // Worked example 4 — mandatory: tolerance $125,000 → $4,875,000–$5,125,000; over-delivery cap $1,250,000; the undelivered $400,000 (measured from the original amount, C2-1.1-02) paired off at 100.750 vs 101.250 = −$2,000.00 (cash back) / at 101.750 = $2,000.00 (fee); 10-day extension of $400,000 at 5.625% = $625.00
  ...
  assert.equal(mandatoryPairOff(40_000_000n, "101.250", "100.750").cash_back_cents, 200_000n); assert.equal(mandatoryPairOff(40_000_000n, "101.250", "101.750").fee_cents, 200_000n); assert.equal(formatCents(200_000n, { symbol: true }), "$2,000.00");
  ...
  // C2-2-01: once a partial pair-off is requested the minimum delivery is $50 below the revised commitment — $275,000 alone would leave $4,600,000 below $4,724,950; the whole $400,000 (or $275,000 + $125,000 extended with it, C2-1.1-04) is required; nothing is left inside the band
  assert.equal(500_000_000n - 460_000_000n, 40_000_000n); assert.deepEqual(postPairOffMinimum(500_000_000n, 40_000_000n), { revised_commitment_cents: 460_000_000n, minimum_delivery_cents: 459_995_000n });
  assert.equal(pairOffSizing({ original_amount_cents: 500_000_000n, purchased_cents: 460_000_000n, paired_off_cents: 0n, requested_cents: 27_500_000n }).allowed, false); assert.equal(pairOffSizing({ original_amount_cents: 500_000_000n, purchased_cents: 460_000_000n, paired_off_cents: 0n }).amount_cents, 40_000_000n);
  assert.equal(pairOffSizing({ original_amount_cents: 500_000_000n, purchased_cents: 460_000_000n, paired_off_cents: 0n, requested_cents: 27_500_000n, extension_remainder_cents: 12_500_000n }).allowed, true);
  assert.equal(hblCap(500_000_000n, "10").max_allowable_hbl_delivery_cents, 50_000_000n);   // C2-2-01: high-balance ≤ 10% of the aggregate UPB
