...
**LL-2026-05**: "Escrow Setup event for each applicable escrow item category type" for newly acquired loans (mandatory Dec. 1, 2026), then same-day deposit/disbursement events and monthly attestation. **Investor Reporting Manual (Apr. 8, 2026)** and research/00b F2/F3: the Form 496 data source is not named in the bundle — F-1-03 requires Form 496 to "reconcile each P&I custodial account, and reflect the composition of the cash book balance of the account at the close of the reporting period" (confirmed), LSDU is the IRM's web application for submitting loan-level reports and receiving investor reporting system reports (IRM 1-01, confirmed), and neither "Cash Position" nor a Fannie Mae Connect "Remittance Detail" report occurs in the IRM or the Servicing Guide (research/00b F2/F3-derived, **[UNVERIFIED]**); A/A shortage/surplus (Form 472 Schedule 3) persists until the automatic P&I draft phase.
...
- **`fnma-lsdu`** Loan Position/Cash Position (API where available; else LSDU CSV via `human_portal_task`) — read **[UNVERIFIED — research/00b: "Cash Position" is not an IRM term]**.
- **`fnma-servicing-events`** — Escrow Setup, and subsequently deposit/disbursement events (6.x/3.x).
- **Fannie Mae Connect** "Remittance Detail – P&I Report" for Form 496 (6.3) **[UNVERIFIED — research/00b: no such report is named in the IRM or the Servicing Guide]**.
