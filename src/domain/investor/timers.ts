...
export function applyInvestorTimerOverrides(reg: TimerRegistry): void {
  ...
  o("FNMA_IRM_LAR83_5BD_2000", { trigger: "`loan_terms.*{reason!=reamortization}`", anchorField: "calculation_date", offset: "+5 business_days_fannie_et, 20:00 ET", satisfied: "`investor_events.submitted{event_type=rate_payment.change}`",
    why: "§5.1 timer table: `loan_terms.rate_changed` scheduled (also `loan_terms.payment_changed` / `.converted_to_fixed`, — IRM 3-03 attaches this clock to *scheduled* rate/payment changes only, so §2.4's `loan_terms.activated{reason=reamortization}` is excluded here and reports its LAR 83 on 2.4's own unscheduled next-BD row) → 5 `business_days_fannie_et` 20:00 ET after the scheduled interest-rate calculation date (rate effective date − look-back), carried as `calculation_date`; satisfied when the `rate_payment.change` event is submitted." });
  ...
  o("FNMA_F121_DQ_EXCEPTIONS_BD4", { trigger: "`period.month_end`", anchorField: "bd2_following", offset: "+2 calendar_days, 12:00 ET (rolled to the next fannie business day)",
    why: "§5.7 timer table (verification amendment, F-1-21 'on the second calendar day following the second business day of the month'): anchor = the following month's BD2 (`bd2_following`, `periodAnchors()`), + 2 calendar days 12:00 ET, rolled to the next `fannie_et` business day when that day is not one — Sept 2026 file: BD2 Fri Oct 2 + 2 = Sun Oct 4 → Mon Oct 5 12:00 ET (the calendar's Oct 5), not BD4 Tue Oct 6. The code stays `…_BD4` as an identifier." });
