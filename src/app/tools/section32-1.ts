...
export const TOOLS_32_1: readonly ToolDef[] = defineTools("32.1", "intake", [
  { name: "send_card", kind: "write", handler: compute((i, ctx, rt) => {
      ...
      ctx.events.append({ type: "card.sent", ...(loan_id ? { loanId: loan_id } : {}), ...(application_id ? { applicationId: application_id } : {}), aggregate: { kind: "card_instance", id: card_instance_id }, actor: ctx.actor, payload: { card_instance_id, party_id, kind, copy_key, command_ref, created_by, expires_at, message_id } });
