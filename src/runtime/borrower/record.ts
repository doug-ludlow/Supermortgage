...
export class BorrowerRecordReader {
  ...
  private async documents(party: { id: string }, subject: Subject, byKind: (k: string) => Entity[], events: Ev[], cards: readonly CardInstanceRow[] = []): Promise<Record<string, unknown>[]> {
    ...
    const noticeDocumentIds = new Set<string>();
    if (subject.loan_id || subject.application_id) {
      // the Notice Registry's rows (recipients are the subject's own parties by construction; visibility is by code — 02 §5 — and, when the row names its recipients, by party)
      const allNotices = await this.db.query<Record<string, unknown>>(`SELECT n.id, n.template_code, n.template_version, n.document_id, n.status, n.produced_at, n.sent_at, n.channel_decision, n.recipient_party_ids, (SELECT d.channel FROM notice_deliveries d WHERE d.notice_id = n.id ORDER BY d.attempt_no DESC LIMIT 1) AS channel, (SELECT d.mailed_at FROM notice_deliveries d WHERE d.notice_id = n.id AND d.mailed_at IS NOT NULL ORDER BY d.attempt_no DESC LIMIT 1) AS mailed_at
        FROM notices n WHERE $1::uuid IS NOT NULL AND n.loan_id = $1 ORDER BY n.created_at`, [subject.loan_id]);
      // 35.2: every rendered notice is a row now — a notice addressed to named parties is theirs (a confirmed successor who declined notices sees no statement, 32.12); a row with no party ids keeps the code-only visibility; the document behind any notices row is listed through that row only
      for (const n of allNotices) if (typeof n["document_id"] === "string") noticeDocumentIds.add(n["document_id"]);
      const notices = allNotices.filter((n) => { const ids = (n["recipient_party_ids"] as string[] | null) ?? []; return ids.length === 0 || ids.includes(party.id); });
    ...
    if (subject.application_id || subject.loan_id) {
      const docs = await this.db.query<Record<string, unknown>>(`SELECT id, kind, doc_class, subject_borrower_id, received_at, created_at, metadata, template_code FROM documents WHERE ($1::uuid IS NOT NULL AND application_id = $1) OR ($2::uuid IS NOT NULL AND loan_id = $2) ORDER BY created_at`, [subject.application_id, subject.loan_id]);
      ...
      for (const d of docs) { const cls = DOCUMENT_CLASSES.find((c) => c.code === d["doc_class"]); const family = cls?.family; const kind = String(d["kind"]);
        if (noticeDocumentIds.has(String(d["id"]))) continue;   // a notices row already carries it (or withholds it from this party)
