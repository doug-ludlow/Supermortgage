...
export function createConsoleServer(opts: ConsoleServerOptions): Server {
  ...
  const SECTION34_PREFIXES = ["/api/directory", "/api/partner-book/", "/api/controls", "/api/portal/", "/api/roles", "/api/principals", "/api/handover", "/api/work"];   // the trailing slash: the legacy /api/portal-tasks/* acts are not 34.5's
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      const ctl = matchControlsRoute(CONTROLS, method, path) ?? matchControlsRoute(ROLES, method, path) ?? matchControlsRoute(WORK, method, path);
      if (ctl) {
