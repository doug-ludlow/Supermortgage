...
export class PartnerAuth {
  ...
  private readonly keyGiven: Buffer | undefined; private keyCache: Buffer | undefined;
  /** The e-mail cipher key, resolved on first use — not at construction: the router is built for every server (production included, where STAFF_EMAIL_KEY is required), and a server that never opens a partner door must not fail for want of it (the staff door's console is optional; this one is always mounted). */
  get key(): Buffer { return (this.keyCache ??= this.keyGiven ?? staffEmailKey({ ...process.env, ENVIRONMENT: this.environment })); }
