...
import { plainDate as D, addDays, addMonths, daysInMonth, endOfMonth, parts, ymd, type PlainDate } from "../../../kernel/calendar/date.ts";
...
export type Dependency = { readonly cycle_code: string; readonly period?: "same" | "as_of_date" | "period_end" | "bd1_next" } | { readonly event: string; readonly key: string };
...
export const SELECTORS: Record<string, Selector> = {
  ...
  statements_due: async (db, w) => {
    ...
    const out: Unit[] = [];
    // Reg Z §1026.41(b): within 4 days of the courtesy period's close — the four days are the look-back (a day the planner missed still gets its statement; the period key is the cycle's due date, so a day planned twice adds nothing); the due day clamps to the month's last day for a first payment on the 29th–31st
    for (const r of rows) {
      const grace = r.grace ?? 15; const first = D(r.first_payment_date); const firstDay = Number(first.slice(8, 10));
      for (let k = 1; k <= 4; k++) {
        const courtesyEnd = addDays(w.as_of_date, -k); const due = addDays(courtesyEnd, -grace);
        const { y, m } = parts(due); const dueDay = Math.min(firstDay, daysInMonth(y, m));
        if (due < first || Number(due.slice(8, 10)) !== dueDay) continue;
  ...
  lar_day: async (db, w) => ((await db.query<{ c: string }>(`SELECT count(*)::text AS c FROM investor_events WHERE status = 'queued'`)).at(0)?.c !== "0" ? [{ period_key: w.as_of_date, unit_id: w.as_of_date }] : []),
  ...
  arm_windows_today: async (db, w) => (await hasTable(db, "arm_schedule")) ? (await db.query<{ loan_id: string; d: string }>(`SELECT loan_id, first_new_payment_due::text AS d FROM arm_schedule WHERE first_new_payment_due - 120 = $1::date ORDER BY loan_id`, [w.as_of_date])).map((r) => ({ period_key: r.d, unit_id: r.loan_id, loan_id: r.loan_id, input: { first_new_payment_due: r.d } })) : [],
  /** 10.x: every `mi_schedules` row whose 80% / 78% / midpoint date is today or passed unhandled */
  mi_dates_today: async (db, w) => (await db.query<{ loan_id: string }>(`SELECT DISTINCT s.loan_id FROM mi_schedules s JOIN loans l ON l.id = s.loan_id AND l.status = 'active' WHERE s.superseded_by IS NULL AND least(coalesce(s.derived_80_date, 'infinity'::date), coalesce(s.derived_78_date, 'infinity'::date), coalesce(s.derived_midpoint_date, 'infinity'::date)) <= $1::date AND NOT EXISTS (SELECT 1 FROM loan_events e WHERE e.loan_id = s.loan_id AND e.type LIKE 'mi.%' AND e.occurred_at >= least(coalesce(s.derived_80_date, 'infinity'::date), coalesce(s.derived_78_date, 'infinity'::date), coalesce(s.derived_midpoint_date, 'infinity'::date))::timestamptz) ORDER BY s.loan_id`, [w.as_of_date])).map((r) => ({ period_key: w.as_of_date, unit_id: r.loan_id, loan_id: r.loan_id })),
  /** 35.5: per lockbox per servicer business day (`lockbox_batches` is 35.5's) */
  lockboxes_daily: async (db, w) => (await hasTable(db, "lockboxes")) ? (await db.query<{ id: string }>(`SELECT id FROM lockboxes WHERE status = 'active' ORDER BY id`)).map((r) => ({ period_key: `${w.as_of_date}:${r.id}`, unit_id: r.id, input: { lockbox_id: r.id } })) : [],
  /** 27.1: every open warehouse advance */
  open_advances_daily: async (db, w) => (await hasTable(db, "warehouse_advances")) ? (await db.query<{ id: string }>(`SELECT id FROM warehouse_advances WHERE status = 'open' ORDER BY id`)).map((r) => ({ period_key: w.as_of_date, unit_id: r.id, input: { advance_id: r.id } })) : [],
  /** 14.x: every open bankruptcy case's loan */
  open_bk_loans_daily: async (db, w) => (await hasTable(db, "bankruptcy_cases")) ? (await db.query<{ loan_id: string }>(`SELECT DISTINCT loan_id FROM bankruptcy_cases WHERE status NOT IN ('closed', 'dismissed', 'discharged') AND loan_id IS NOT NULL ORDER BY loan_id`)).map((r) => ({ period_key: w.as_of_date, unit_id: r.loan_id, loan_id: r.loan_id })) : [],
  /** 35.9: every loan with an open Reg X EI window, case or claim candidate */
  default_case_loans_daily: async (db, w) => {
    const out = new Set<string>();
    for (const [t, col] of [["regx_ei_windows", "loan_id"], ["cases", "loan_id"], ["claim_candidates", "loan_id"]] as const) if (await hasTable(db, t)) for (const r of await db.query<{ id: string }>(`SELECT DISTINCT ${col} AS id FROM ${t} WHERE ${col} IS NOT NULL AND coalesce(status::text, 'open') NOT IN ('closed', 'completed', 'withdrawn', 'denied', 'paid')`)) out.add(r.id);
  ...
  open_parallel_runs_daily: async (db, w) => (await hasTable(db, "parallel_runs")) ? (await db.query<{ id: string }>(`SELECT id FROM parallel_runs WHERE status = 'open' ORDER BY id`)).map((r) => ({ period_key: w.as_of_date, unit_id: r.id, input: { parallel_run_id: r.id } })) : [],
};

// ---------------------------------------------------------------- the registry (rule 2's table, at first commit)
const dep = (cycle_code: string, period: "same" | "as_of_date" | "period_end" | "bd1_next" = "same"): Dependency => ({ cycle_code, period });
...
export const CYCLES: readonly CycleDef[] = [
  ...
  daily({ cycle_code: "mi_changes", owner_process: "10.3", owner_agent: "pmi", unit_scope: "loan", schedule: "daily: every mi_schedules row whose 80% / 78% / midpoint date is today or passed unhandled", selector: "mi_dates_today", runner: "mi_changes", receipt_event: "mi.schedule.run_completed", depends_on: [dep("cashiering_daily")], serves_timer: "HPA_4902C_MIDPOINT_TERMINATE_0", escalation_role: "ops_analyst", expected_by: "same_day 23:59 ET" }),
