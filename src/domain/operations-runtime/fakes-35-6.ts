...
export class FakeRonSessionFeed {
  ...
  timeline(closingId: string, scheduledAt: string, signers: readonly string[], documents: { enote: string; final_1003: string; security_instrument: string }, opts: { enote: boolean; min: string | null; partner_org_id: string | null; notary_party_id: string }): readonly RonSessionEvent[] {
    ...
    // the note is single-method (26.2): an eNote is e-signed in the session; a paper note (hybrid closing) is wet-signed at the table by the same signers in the same minutes
    signers.forEach((party, k) => out.push({ seq: ++seq, op: "sign", at: at(25 + k), detail: { closing_document_id: documents.enote, kind: opts.enote ? "enote" : "note", signer_party_id: party, signed_at: at(25 + k), signature_method: opts.enote ? "esign_ron" : "wet", required_note_signers: signers } }));
