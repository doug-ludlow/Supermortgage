...
- (i)(2) exemption: if at the time of the analysis the borrower is "more than 30 days overdue," or the servicer has brought a foreclosure action, or the borrower is in bankruptcy, the servicer is exempt from submitting the annual statement; if the servicer did not issue the annual statement under this exemption "and the loan subsequently is reinstated or otherwise becomes current, the servicer shall provide a history of the account since the last annual statement (which may be longer than 1 year) within 90 days of the date the account became current" — the anchor is the account-current date, not the end of a bankruptcy or foreclosure case; (i)(2) contains no request-based duty, so a borrower request while exempt is handled as an information request (§1024.36; policy) and does not replace or cancel the 90-day history once the loan becomes current.
...
- `loan.reinstated` — or the account otherwise becoming current (`loan.became_current`, or a `bankruptcy.case.closed` / `foreclosure.case.cancelled` that records the account as current with its `current_on` date) — where an (i)(2) exemption was applied → `escrow.statement.exemption_ended` → post-exemption account history; anchor = the date the account became current (§1024.17(i)(2)). A `bankruptcy.case.closed` / `foreclosure.case.cancelled` on a still-delinquent loan ends nothing (recorded as `escrow.statement.exemption_end.refused`; the exemption re-applies at the next analysis if > 30 days overdue).
- Borrower request while exempt → `rfi` case (4.2), handled as an information request (§1024.36; policy — (i)(2) contains no request-based duty); it does not replace or cancel the 90-day history once the loan becomes current: a request on a loan that is current is itself evidence the account became current (anchor = the known account-current date, else the request date), and the history statement — sent within 5 business days of the request as policy — answers it and satisfies the timer early.
...
#### State machine
`due` (created at analysis approval or at the short-year trigger) → `exempt_hold` (if (i)(2) applies; records reason) | `rendered` → `sent` → `delivered` | `returned_undeliverable` → `re_addressed` → `sent`. From `exempt_hold`: `exemption_ended` (the account became current) → `history_due` (90-day timer) → `rendered`; a borrower request while held is logged (`rfi`) and answered by the history once `history_due`; an annual statement the servicer nevertheless issues while the loan is still delinquent discharges the hold, since (i)(2) owes the history only "if the servicer does not issue an annual statement pursuant to this exemption". Terminal: `delivered`, `cancelled` (loan transferred/paid before due with the short-year variant issued instead). Actors: `escrow` agent; Notice Registry; print/mail return feed.
...
| `REGX_1024_17I2_POST_EXEMPTION_HISTORY_90` | deadline | `escrow.statement.exemption_ended` — emitted only when the loan "is reinstated or otherwise becomes current" (`loan.reinstated`, or the account otherwise becoming current) while an (i)(2) hold is open; `bankruptcy.case.closed` / `foreclosure.case.cancelled` on a still-delinquent loan do not emit it (the exemption re-applies at the next analysis if > 30 days overdue) | date the account became current (`exemption_ended_on`) | 90 calendar_days | `escrow.statement.sent` (post_exemption_history) | sev-2 |
...
6. **Post-exemption history**: covers `[last statement end, date the account became current]`, may exceed 12 months; delivered within 90 days of the date the account became current (§1024.17(i)(2)) — the anchor is the reinstatement / account-current date, never the end of a bankruptcy or foreclosure case on a still-delinquent loan; a fresh annual analysis (3.2 `reinstatement`) accompanies it and resets the computation year with a short-year statement if the year has drifted.
...
- Exemption applied, then the loan becomes current → `REGX_1024_17I2_POST_EXEMPTION_HISTORY_90` arms on the account-current date whether or not the borrower asks; a borrower request is logged as an `rfi` (§1024.36) and answered by the history statement (policy: send within 5 business days of the request, satisfying the timer early); a request while still delinquent is an RFI answered per 4.2 and arms nothing.
- Loan reinstated (or otherwise current) → exemption ends → 90-day history timer anchored on the account-current date; a foreclosure dismissal or bankruptcy closure on a still-delinquent loan ends nothing (the exemption re-applies at the next analysis if > 30 days overdue).
...
- Corrections: a statement with an error is superseded by a corrected statement (new `escrow_statements` row referencing the original); both retained; NoE handling per 4.1 (1024.35(b)(11); (b)(4) only where the error is a missed disbursement or refund).
