...
export async function readItem(q: Queryable, itemId: string): Promise<ItemRow | undefined> {
  const r = (await q.query<Row>(`SELECT id, batch_id, item_no, scanline, loan_number_read, amount_cents::text AS amount_cents, check_number, payer_name, to_char(scanned_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS scanned_at, after_cutoff, received_on::text AS received_on, matched_loan_id, match_method, payment_id, suspense_item_id, disposition, resolved_by FROM lockbox_items WHERE id = $1`, [itemId]))[0];
