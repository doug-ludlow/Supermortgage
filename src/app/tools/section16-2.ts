...
/** 16.2 rule 4: the loan's pool membership (`mbs_pool` or `pool_class = mbs`) gates the S/S BD1/BD2 exception; absent, the loan is treated as portfolio S/S and owes the full month (F-1-20). */
const mbsPoolOf = (i: ToolInput): boolean => flag(i, "mbs_pool") || str(i, "pool_class").toLowerCase() === "mbs";
...
export const TOOLS_16_2: readonly ToolDef[] = defineTools("16.2", AGENT, [
  ...
  { name: "postPayoff", kind: "write", moneyFields: ["amount_cents", "buckets"], handler: compute((i, ctx, rt) => {
      ...
      const nib = (b.nib_deferred ?? 0n) + (b.nib_forborne ?? 0n); const type = rtype(i); const processedOn = today(ctx); const mbsPool = mbsPoolOf(i);
      const share = str(i, "note_rate_pct") && str(i, "ptr_pct") && str(i, "lpi_due") ? fnmaPayoffShare({ type, upb_cents: b.principal, nib_cents: nib, note_rate_pct: str(i, "note_rate_pct"), ptr_pct: str(i, "ptr_pct"), lpi_due: date(i, "lpi_due"), payoff_on: payoffOn, participation_pct: str(i, "participation_pct") || "100", mbs_pool: mbsPool, processed_on: processedOn, reported_on: optDate(i, "reported_on") }) : null;
    ...
    guardrails: [never("NO_POST_BEFORE_CLEARED", "16.2 timer SM_PAYOFF_GOODFUNDS_GATE: no posting before payoff.funds.cleared per the good-funds policy", (i) => i.op !== "reverse" && !str(i, "funds_id"), "name the payoff_funds row the intake cleared (matchPayoffFunds) — a funds_cleared attestation is not evidence"),
      ...
      needsRole("ROLE_DENIED", "16.2 rule 6 / 35.10 T10: a reversal of a settlement inside the finality window is an `officer` command — the agent may only propose it (NO_MONEY_FIELD)", (i) => i.op === "reverse", ["officer"], "a settlement reversal is the officer's act; the agent proposes it through an escalation"),
  ...
  { name: "computeFnmaPayoffShare", kind: "write", handler: compute((i, ctx, rt) => { need(i, "type", "upb_cents", "note_rate_pct", "ptr_pct", "lpi_due", "payoff_on");
      ...
      const share = fnmaPayoffShare({ type, upb_cents: cents(i.upb_cents), nib_cents: cents(i.nib_cents), note_rate_pct: str(i, "note_rate_pct"), ptr_pct: str(i, "ptr_pct"), lpi_due: date(i, "lpi_due"), payoff_on: date(i, "payoff_on"), participation_pct: str(i, "participation_pct") || "100", mbs_pool: mbsPoolOf(i), processed_on: optDate(i, "processed_on") ?? today(ctx), reported_on: optDate(i, "reported_on") });
  ...
  { name: "buildCrsBatch", kind: "act", handler: compute((i, ctx, rt) => {
      ...
      const task = rt.escalations.open({ kind: "human_portal_task", loanId: ctx.loanId, payload: { role: b.upload_task.role, batch_id: rec.id, package: b.upload_task.package, cut_off: zonedEpochMs(b.batch_on, "16:00", ET) }, ownerRole: "fnma_portal_operator" }, ctx.actor);   // the CRS batch id rides in the payload: escalations.batch_id is 1.x's transfer batch (FK), never a remittance batch
  ...
  { name: "createHousekeepingTasks", kind: "write", handler: compute(async (i, ctx, rt) => {
      ...
      if (i.op === "complete") {
        ...
        if (task === "paid_in_full_letter") {
          // NTC_PAYOFF_PAID_IN_FULL from the settlement row and the escrow-refund calculator (payoff date, amounts applied, §1024.34(b)(1)/§1024.17(i)(4)(iii) dates), never the caller's figures
