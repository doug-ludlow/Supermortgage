...
test("16.2-T7: Given a returned payoff check on Wed 10/28 (before the 11/03 close), then a correcting removal event is projected, the ledger reopens as of 10/16, the refund is stop-paid and the MI/insurance/tax tasks are reversed; given the return on Wed 11/04, then `fnma_liquidated_in_error` is set and an `officer` escalation opens.", async () => {
  ...
  const rev = await h.run("postPayoff", { op: "reverse", loan_id: "L-1", settlement_id: r.posted.settlement_id, returned_at: "2026-10-28T14:00:00.000Z", cause: "returned_item", refund_disbursement_id: "disb-refund-1" }, OFFICER);
  ...
  const lrev = await late.run("postPayoff", { op: "reverse", loan_id: "L-1", settlement_id: l.posted.settlement_id, returned_at: "2026-10-28T14:00:00.000Z", cause: "returned_item" }, OFFICER);
  ...
  const post = await p.run("postPayoff", { op: "reverse", loan_id: "L-1", settlement_id: r2.posted.settlement_id, returned_at: "2026-11-04T14:00:00.000Z", cause: "returned_item" }, OFFICER);
  ...
  await assert.rejects(p.run("postPayoff", { op: "reverse", loan_id: "L-1", settlement_id: r2.posted.settlement_id, returned_at: "2026-11-04T14:00:00.000Z", finality_at: "2026-11-03T22:00:00.000Z", cause: "returned_item", reopen_ledger: true }, OFFICER), (e: unknown) => e instanceof CommandRefused && e.code === "NO_REVERSAL_AFTER_CLOSE");
