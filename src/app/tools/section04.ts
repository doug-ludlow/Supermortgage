...
import { handleUtterance, callbackRequest, nyExtension, nyNoeDeadline, noeCommunicationCheck, documentRequest, earlyCorrection, earlyResponse, privilegeRouting, fraudSignals, texasCure, eiNoticeAssignment, noeRfiProcedureBlock, AI_MONETARY_LIMIT_CENTS, type ContactTeamBlock, type DocumentRequester } from "../../domain/servicing-requests/ops.ts";
...
import { noeOpenedPayload, noeRespondedPayload, suppressionRow, liftSuppressionOnException, repostSet, feeReversalSet, rfiOpenedPayload, rfiRespondedPayload, complaintOpenedPayload, complaintResponseCheck, siiPolicyDue, type NoeOpenInput, type NoeOpenedPayload, type RfiOpenInput, type RfiOpenedPayload, type ComplaintOpenInput, type ComplaintOpenedPayload, type SuppressionRow, type SuppressionLift } from "../../domain/servicing-requests/cases.ts";
import { ownerIdentity, FNMA_OWNER_BLOCK, redactionCheck, searchLogComplete, ownAccountRecord, type Ownership, type TrustStructure } from "../../domain/servicing-requests/rfi.ts";
import { DOCUMENT_MATRIX, evaluateDocuments, fnmaExemptTransaction, type TransferType } from "../../domain/servicing-requests/successor.ts";
import { onPermanentPayment, caSpocDue, contactPointRequirement, type Episode } from "../../domain/servicing-requests/continuity.ts";
...
const p43: ToolDef[] = defineTools("4.3", "borrower-comms", [
  // §1024.40(b)(1) facts plus the (b)(4) procedure block (how to submit a written notice of error / information request — the exclusive address of record), so the personnel read both from the system of record (4.3 rule 5; 4.3-T8)
  { name: "lossmit_facts.get", kind: "read", handler: compute((i, _c, rt) => { const f = rt.store.get("lossmit_facts", str(i, "loan_id"))?.data; if (!f) return null; const addr = rt.store.get("designated_addresses", "noe_rfi_exclusive")?.data; return { ...f, noe_rfi_procedure: noeRfiProcedureBlock({ exclusive_address: typeof addr?.address === "string" ? addr.address : null, online_channel: typeof addr?.online_channel === "string" ? addr.online_channel : null }) }; }) },
...
const noeCommands: ToolDef[] = defineTools("4.1", "case", [
  ...
  // §1024.35(g)(1) lifts (d), (e) and (i) for the excepted assertions: the §1024.35(i) suppression row ends on the determination date once every payment-related assertion is excepted (expiry emitted with reason `regx_1024_35_g1_exception`), and stays — narrowed — while a carved-out identifiable assertion is under investigation ((g)(1)(ii); 4.1-T7/T8/T9).
  { name: "case.noe.determine", kind: "write", handler: compute((i, ctx, rt) => {
      ...
      let lift: SuppressionLift | null = null;
      if (det === "exception") {
        caseEvent(ctx, "case.noe.exception_determined", caseId, { assertion_id: str(i, "assertion_id"), exception_basis: basis, determination_date: today(ctx) });
        const sup = rt.store.get("credit_reporting_suppressions", caseId);
        if (sup && !sup.data.lifted_on) {
          const excepted = caseEvents(ctx, "case.noe.exception_determined", caseId).map((e) => String(e.payload.assertion_id));
          lift = liftSuppressionOnException(sup.data as unknown as SuppressionRow, noeRecord(ctx, caseId)!, excepted, today(ctx));
          rt.store.put("credit_reporting_suppressions", caseId, lift.row, ctx.actor, ctx.now);
          if (lift.ended) ctx.events.append({ type: "credit_reporting.suppression.expired", loanId: ctx.loanId, aggregate: { kind: "case", id: caseId }, actor: ctx.actor, payload: { case_id: caseId, ends_at: lift.row.ends_at, reason: "regx_1024_35_g1_exception", lifted_assertion_ids: lift.row.lifted_assertion_ids } });
        }
      }
      return { determination: det, exception_basis: basis, exception_notice_due: det === "exception" ? federalDays(today(ctx), 5) : null, suppression: lift ? { ended: lift.ended, scope: lift.row.scope, ends_at: lift.row.ends_at, remaining_assertion_ids: lift.remaining_assertion_ids } : null };
    }),
  ...
  // §1024.35(e)(5) (4.1 Outputs): copies to a confirmed-successor/agent requester omit other borrowers'/successors' location/contact and personal financial data — the contents go through the redaction check (rfi.redactionCheck, the 4.2 rule-5 table) before the copies are sent.
  { name: "case.noe.documents.request", kind: "write", handler: compute((i, ctx, rt) => {
      ...
      const requester = (str(i, "requester_role") || "borrower") as DocumentRequester;
      const r = documentRequest(requestedOn, docs, requester);
      const check = i.document_contents === undefined ? null : redactionCheck(requester, i.document_contents);
      const omissions = { rules: r.omissions, check_passed: check ? check.passed : null, redaction_log: (i.redaction_log as unknown[] | undefined) ?? [] };
      const rec = rt.store.put("document_copy_requests", `${caseId}:${requestedOn}`, { case_id: caseId, requested_at: ctx.now, requested_via: str(i, "requested_via") || "oral", requester_role: requester, omissions_applied: omissions, due_at: r.copies_due, provided: r.provided, withheld: r.withheld }, ctx.actor, ctx.now);
      caseEvent(ctx, "case.noe.document_copies.requested", caseId, { requested_on: requestedOn, requested_via: str(i, "requested_via") || "oral", requester_role: requester, omissions: r.omissions, e5_redaction_check_passed: check ? check.passed : null, copies_due: r.copies_due, provided: r.provided.map((d) => d.id), withheld: r.withheld.map((d) => d.id) });
      rt.store.put("cases", caseId, { status: "docs_requested" }, ctx.actor, ctx.now);
      return { ...r, request_id: rec.id, e5_redaction_check_passed: check ? check.passed : null };
    }), guardrails: [NOE_EXISTS,
      guard("E5_REDACTION_BEFORE_SEND", "§1024.35(e)(5); 4.1 Outputs: the (e)(4) copies to a confirmed-successor or agent requester omit location/contact and personal financial information about other borrowers/successors (other than loan terms, status and payment history) — the contents pass the redaction check before the copies are sent", (i) => {
        const requester = str(i, "requester_role") || "borrower";
        if (i.document_contents === undefined) return requester === "borrower" ? undefined : `requester is a ${requester}: the document contents (document_contents) go through the §1024.35(e)(5) redaction check before the copies are sent`;
        const r = redactionCheck(requester, i.document_contents); return r.passed ? undefined : `§1024.35(e)(5) redaction check failed for a ${requester}: ${r.violations.map((v) => `${v.field} (${v.rule})`).join(", ")}`;
      })] },
...
const rfiCommands: ToolDef[] = defineTools("4.2", "case", [
  ...
  { name: "rfi.respond", kind: "act", handler: compute((i, ctx, rt) => {
    ...
    guardrails: [RFI_EXISTS,
      ...
      // 4.2 rule 2 / A4-1-03: the trust identifier on express request is the pool number (standard MBS), the designated trust name (structured deal) or the Fannie Mae contact number for the related Trust identifier (PFP pool)
      never("OWNER_BLOCK_VERSIONED", "4.2 guardrails: cannot send an owner-identity answer that deviates from the versioned Fannie Mae block (A4-1-03)", (i) => typeof i.owner_block === "string" && i.owner_block !== ownerIdentity((str(i, "ownership") || "fnma_portfolio") as Ownership, flag(i, "asked_for_trust_name"), str(i, "pool_number") || undefined, (str(i, "trust_structure") || "standard_mbs") as TrustStructure, str(i, "trust_name") || undefined), `the owner block must be the ${FNMA_OWNER_BLOCK.version} reference text for the loan's ownership type`),
...
const continuityCommands: ToolDef[] = defineTools("4.3", "borrower-comms", [
  ...
  // A non-principal-residence loan (`principal_residence: false`) is `not_required` under §1024.40 (§1024.30(c)(2)) but still assigned the standard team under the Fannie Mae A4-1-01 / A4-2.1-01 contact-point requirement — without the Reg X named-human requirement (4.3 edge cases; 4.3-T3).
  { name: "continuity.assign", kind: "write", handler: compute((i, ctx, rt) => {
      const id = episodeId(i, ctx); const mode = str(i, "mode") || "ai_first_named_human"; const req = contactPointRequirement(i.principal_residence !== false);
      const rec = rt.store.put("continuity_episodes", id, { loan_id: ctx.loanId, status: "assigned", assigned_at: ctx.now, assignment_mode: mode, mode, team: str(i, "team") || "default", team_name: str(i, "team_name") || str(i, "team") || "default", named_human: str(i, "named_human") || null, direct_number: str(i, "direct_number"), hours: str(i, "hours") || null, consecutive_on_time: 0, principal_residence: i.principal_residence !== false, regx_required: req.regx_1024_40 === "required", basis: req.basis, ...(typeof i.assignment_due_at === "string" ? { assignment_due_at: i.assignment_due_at } : {}) }, ctx.actor, ctx.now);
      ctx.events.append({ type: "continuity.assigned", loanId: ctx.loanId, aggregate: { kind: "continuity_episode", id }, actor: ctx.actor, payload: { episode_id: id, team: rec.data.team_name, mode, named_human: rec.data.named_human, direct_number: rec.data.direct_number, assigned_on: today(ctx), auto_assigned: false, regx_required: req.regx_1024_40 === "required", basis: req.basis } });
      return { episode_id: id, version: rec.version, mode, basis: req.basis, regx_status: req.regx_1024_40 };
    }),
    guardrails: [never("NAMED_HUMAN_REQUIRED", "4.3 state machine: `pending_assignment → assigned` requires `named_human_id` unless `ai_only` is enabled for the jurisdiction (default off — 4.3-Q1); a non-principal-residence loan is assigned under the Fannie Mae A4-1-01/A4-2.1-01 contact-point requirement without the Reg X named human (4.3 edge cases)", (i) => (str(i, "mode") || "ai_first_named_human") !== "ai_only" && !str(i, "named_human") && contactPointRequirement(i.principal_residence !== false).named_human_required, "an assignment names the human of record"),
...
const siiCommands: ToolDef[] = defineTools("4.4", "case", [
  ...
  // The Reg X determination (§1024.31 types + ownership interest) and the D1-4.1-02 exempt-transaction evaluation are separate (4.4 rule 4 / rule 6; 4.4-T5): an arm's-length buyer who assumes in writing and occupies with the transferor is `not_successor` yet an exempt transferee — no due-on-sale enforcement, assumption path offered.
  { name: "sii.determine", kind: "write", handler: compute((i, ctx, rt) => {
      ...
      const transferType = str(i, "transfer_type") || String(rt.store.get("sii_cases", caseId)?.data.transfer_type ?? "");
      const fnma = fnmaExemptTransaction({ transfer_type: transferType, assumes_in_writing: flag(i, "assumes_in_writing"), co_occupies_with_transferor: flag(i, "co_occupies_with_transferor"), ...(i.occupies !== undefined ? { occupies: flag(i, "occupies") } : {}), ...(i.months_since_closing !== undefined ? { months_since_closing: num(i, "months_since_closing") } : {}), purchased_by_fnma_on: typeof i.purchased_by_fnma_on === "string" ? D(i.purchased_by_fnma_on) : null, release_of_liability_requested: flag(i, "release_of_liability_requested"), modification: flag(i, "modification") });
      rt.store.put("sii_cases", caseId, { determination: det, determined_at: ctx.now, reason: str(i, "reason") || null, officer_approval_id: str(i, "officer_approval_id") || null, status: det, regx_sii: fnma.regx_sii, fnma_exempt_item: fnma.item, fnma_occupancy_required: fnma.occupancy_required }, ctx.actor, ctx.now);
      caseEvent(ctx, "case.sii.determined", caseId, { determination: det, reason: str(i, "reason") || null, officer_approval_id: str(i, "officer_approval_id") || null, regx_sii: fnma.regx_sii, fnma_exempt_item: fnma.item });
      ...
      return { determination: det, notice: det === "confirmed" ? "NTC_REGX_38B1VI_SII_CONFIRMED" : det === "not_successor" ? "NTC_REGX_38B1VI_SII_NOT_SUCCESSOR" : "NTC_REGX_38B1VI_SII_ADDL_DOCS", obligor: false, fnma_exempt_transaction: fnma, assumption_path_offered: fnma.exempt, ...(det === "confirmed" && pending ? { lossmit_application_received_on: today(ctx) } : {}) };
    }),
  ...
  // D1-4.1-02: the Fannie Mae Legal (F-4-02) package for an unenforceable due-on-transfer clause — an authority that exists only for loans acquired by Fannie Mae after June 1, 2007; the 60 days of FNMA_D1_4_1_02_FNMA_LEGAL_60 run from Fannie Mae's receipt of the notice (`fnma_received_on`, delivery evidence — the send date for the e-mail package), not from the servicer's send date.
  { name: "sii.due_on_transfer.notify", kind: "write", handler: compute((i, ctx, rt) => {
      const caseId = str(i, "case_id"); const sent = today(ctx); const received = D(str(i, "fnma_received_on") || sent); const acquired = str(i, "fnma_acquired_on") || null; const evidence = str(i, "delivery_evidence") || "email_package";
      rt.store.put("sii_cases", caseId, { fnma_legal_notified_on: sent, fnma_legal_received_on: received, fnma_legal_delivery_evidence: evidence, fnma_acquired_on: acquired }, ctx.actor, ctx.now);
      caseEvent(ctx, "due_on_transfer.unenforceable.notified", caseId, { notice_date: sent, fnma_received_on: received, delivery_evidence: evidence, fnma_acquired_on: acquired, basis: str(i, "basis") });
      return { notice_date: sent, fnma_received_on: received, wait_until: addDays(received, 60) };
    }),
    guardrails: [SII_EXISTS,
      never("FNMA_LEGAL_POST_2007_ONLY", "D1-4.1-02: the servicer is authorized to approve a transfer on an unenforceable due-on-transfer clause only 'for a mortgage loan acquired by Fannie Mae after June 1, 2007'", (i) => typeof i.fnma_acquired_on === "string" && i.fnma_acquired_on <= "2007-06-01", "the loan was acquired by Fannie Mae on or before June 1, 2007 — no unenforceability authority; enforce or seek Fannie Mae's specific approval"),
      guard("FNMA_RECEIPT_BEFORE_SEND", "FNMA_D1_4_1_02_FNMA_LEGAL_60: the 60 days run from Fannie Mae's receipt of the notice (delivery evidence), which cannot precede the send", (i, ctx) => (typeof i.fnma_received_on === "string" && i.fnma_received_on < today(ctx) ? `fnma_received_on ${i.fnma_received_on} precedes the send date ${today(ctx)}` : undefined))] },
