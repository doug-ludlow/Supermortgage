...
import { writeEmployer, resolveBorrowerEdge } from "../../domain/underwriting/du/writer.ts";
import {
  ...
  { name: "orderVerificationReport", kind: "write", handler: compute((i, ctx, rt) => {
      ...
      switch (op) {
        ...
        case "receive": {
          ...
          // 23.5 rule 2: an employment the report names (`employer_name`, `employer_ein` when the vendor gave one) is an `employers` row, created or matched under
          // EMPLOYER_IDENTITY_RULES in this command's transaction (writer.ts writeEmployer); the income item 32.2's income Confirm inserts then names it. A harness without a transaction keeps the record only.
          const employerName = optStr(i, "employer_name"); const employerEin = optStr(i, "employer_ein"); const employerRef = str(i, "borrower_id");
          const employer = employerName && (str(i, "kind") === "income" || str(i, "kind") === "employment") && hasTransaction(rt) ? { display_name: employerName, ein: employerEin, rule: "EMPLOYER_IDENTITY_RULES" } : null;
          if (employer) { const defer = rt.services["deferWrite"] as (fn: (q: Queryable) => Promise<void>) => void; defer(async (q) => { await writeEmployer(q, { applicationId: app, applicationBorrowerId: await resolveBorrowerEdge(q, app, employerRef), displayName: employerName!.slice(0, 150), ein: employerEin }); }); }
          return { verification_id: r.verification_id, kind: str(i, "kind"), report_reference_id: str(i, "report_reference_id"), du_submission_fields: { "DU:VerificationReportSupplierType": str(i, "supplier_code"), "DU:VerificationReportIdentifier": str(i, "report_reference_id") }, event: r.event.type, employer }; }
        case "du_validation": {
