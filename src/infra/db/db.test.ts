...
test("migrations: every file under db/migrations is applied to the test database (811 tables: public + restricted_fl)", { skip }, async () => {
  ...
  assert.equal(t!.c, 811n);   // the count a fresh `db/migrate.sh` run produces through 0171: 776 through 0137 (public + restricted_fl base tables; 0132 journey_progress is a view; 0134 recreates three tables as views) plus 35.5's seven — 0142's three (installment_schedule_runs, servicer_profiles, loan_servicing_configs; loan_installments is extended, not created), 0144's cashiering_unit_runs and 0145's three (lockbox_batches, lockbox_items, ach_return_files; ach_entries is extended) — 0146's five (35.3: cycle_registry, cycle_runs, jobs, job_events, cycle_receipts), 0150's eight (35.1's seam tables; its entity_latest_scoped is a view), 35.2's ten (0161 document_blobs, document_holds; 0162 document_integrity_runs, document_integrity_findings, document_access_log; 0163 esign_envelopes, esign_envelope_documents, esign_signature_events; 0164 mail_manifests, mail_manifest_pieces) and 0170's five (35.7: role_grants, role_queue_snapshots, role_handovers, breakglass_uses, api_principals); 0138–0141, 0143 (seed rows only), 0151, 0160, 0165, 0166 and 0171 create no table; measured on a scratch database at the 35.2 second merge; the absolute number moves with every §35 migration that adds a table
});
