 ...
 * disbursement_date, exception }`, `{ loan_purpose, note_date, application_date | as_of }`, `{ pass, factors }` or the raw
 * borrower's-interest inputs.
 */
import type { Evaluator } from "../../app/evaluator-kit.ts";
import { resolicitCooldownGateFacts, offerFrequencyCapGateFacts, cashoutNoteSeasoningGateFacts, cashoutToLcorGateFacts, titleSeasoningGateFacts, borrowerInterestGateFacts } from "./ops-20-1.ts";
...
export const EVALUATORS_20_1: Record<string, Evaluator> = {
  ...
  /** FNMA_B2_1_3_03_CASHOUT_NOTE_SEASONING_12M (worked example 2; T4): new note date ≥ existing first-mortgage note date + 12 months (subordinate liens paid off / co-owner buyout under a legal agreement excepted). */
  "20.1.cashoutNoteSeasoningGate": (f) => cashoutNoteSeasoningGateFacts(f),
  /** FNMA_B2_1_3_04_CASHOUT_TO_LCOR_30D (T4): an LCOR on an existing cash-out refinance opens on day 31 after that note date (an application dated 30 days or less after it is not eligible as LCOR). */
  "20.1.cashoutToLcorGate": (f) => cashoutToLcorGateFacts(f),
