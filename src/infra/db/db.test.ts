...
test("migrations: every file under db/migrations is applied to the test database (789 tables: public + restricted_fl)", { skip }, async () => {
  ...
  assert.equal(t!.c, 794n);   // the count a fresh `db/migrate.sh` run produces through 0171 plus 0142 (35.3's five cycle tables: cycle_registry, cycle_runs, jobs, job_events, cycle_receipts): 784 through 0150 (0150 adds 35.1's eight seam tables) plus 0170's five (35.7: role_grants, role_queue_snapshots, role_handovers, breakglass_uses, api_principals); 0137–0141, 0151–0169 and 0171 create no table; the absolute number moves with every §35 migration that adds a table
});
