...
| `FNMA_D23206_POSTFORB_FLEX_SOLICIT_15` | deadline | `workout_plan.expired{qrpc=false, deferral_eligible=false, flex_eligible=true, delinquency_days>=90}` (non-disaster path; the disaster reduced-criteria path has no 90-day condition) | `term_end` | 15 `calendar_days` | `notice.sent{NTC_FNMA_D23206_SOLICIT_STREAMLINED}` | sev-2 |
...
4. **Pre-expiry evaluation:** at term_end −30, run the 12.2 hierarchy pre-screen (QRPC-dependent): hardship resolved + can reinstate → reinstatement; resolved + affordable plan → repayment (12.5); resolved + cannot afford plan → deferral (12.6) if 2–6 months delinquent … else Flex Mod (12.8); unresolved → extension within caps. At expiry without QRPC the post-forbearance Flex Mod solicitation (`FNMA_D23206_POSTFORB_FLEX_SOLICIT_15`) arms only when the loan is **at least 90 days delinquent** and the borrower is otherwise eligible (D2-3.2-06) — a loan under 90 days delinquent at expiry gets no Flex solicitation clock (the disaster reduced-criteria path carries no 90-day condition).
