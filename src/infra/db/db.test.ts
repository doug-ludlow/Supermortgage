...
test("migrations: every file under db/migrations is applied to the test database (786 tables: public + restricted_fl)", { skip }, async () => {
  ...
  assert.equal(t!.c, 786n);   // 0160 adds columns only; 0161 adds 35.2's document_blobs and document_holds; 0162 adds document_integrity_runs, document_integrity_findings and document_access_log; 0163 adds esign_envelopes, esign_envelope_documents and esign_signature_events; 0164 adds mail_manifests and mail_manifest_pieces; 0165 adds a column only (35.1's 0150 adds eight more when it merges — 794 with both) (776 through 0141; 35.1's 0150 adds eight more when it merges — 786 with both). Before 35.2: the count a fresh `db/migrate.sh` run produces through 0136 (public + restricted_fl base tables; 0116 oidc_identities, 0117 lead_tokens, 0118 party_credentials, 0119 agent_turns, 0120 demo_clock, 0121 video_sessions; 0122 and 0123 add columns only; 0124 widens a check; 0125 partner_book_imports, partner_book_facts, partner_book_invitations, partner_book_reviews, readiness_checks; 0126 adds a column only; 0127 staff_users, staff_credentials, staff_sessions, staff_actions, staff_access_reviews; 0128 directory_unmasks, directory_exports; 0129 partner_book_daily_reports; 0130 evidence_packs; 0131 adds a column only) ; 0132 journey_progress is a view, not a base table; 0133 adds the twelve 23.5 tables: employers, du_assets, du_owned_properties, du_liabilities, du_expenses, du_asset_parties, du_liability_parties, du_expense_parties, du_joint_credit_report_links, du_declarations, du_bankruptcy_filings, du_residences; 0134 drops application_assets, application_liabilities and application_reo and recreates them as VIEWS over the du_* tables — a view is not a BASE TABLE, so three fewer; 0135 adds du_documents, 23.6's one table; 0136 adds du_preflight_results, 23.7's one table)
});
