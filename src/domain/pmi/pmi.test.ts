...
test("10.1-T2 2027-07-10 request with UPB $335,548.68 → 8388 bps, LTV_ABOVE_THRESHOLD, denial due 2027-08-09", () => {
...
test("10.1-T1 2029-07-10 request: UPB $319,950.95 → 7998 bps, clean history, AVM $455,000 → eligible effective 2029-07-10, code 51", () => {
...
test("10.1-T4 30-day late (2028-10-01 paid 2028-11-05) in window B → PAYMENT_HISTORY_30_12M; disaster-attributable → excluded", () => {
...
test("10.1-T5/T6/T12 value check, 2-unit 70% threshold, valuation validity", () => {
...
test("10.1-T9/T10/T11 curtailment effective date; original-value evidence; SMDU override", () => {
  const r = evaluateCancellation({
  ...
  assert.equal(decisionDue(d("2026-08-27"), d("2026-09-25")), d("2026-10-25"));   // 10.6-T4 re-anchor
});

// ---- 10.2 / 10.3 -----------------------------------------------------------------

test("10.2 worked scenarios A/B/C at scheduled_78_date 2035-07-01", () => {
  const n = 140;
  ...
  assert.equal(automaticTermination(ledger(n), d("2035-07-01"), false).status, "not_applicable_midpoint_only");   // 10.2-T5
});

test("10.2-T4 ARM reset at payment 61 to 7.50% → 78% moves from 2034-07-01 to 2035-09-01", () => {
...
test("10.2-T10 / 10.3-T4 Flex Mod: 78% at payment 218 (2047-03-01); midpoint termination 2049-02-01", () => {
...
test("10.3-T1 IO 10/20: 78% at payment 192 (2040-04-01) is after midpoint → trigger 2039-05-01; 10.3-T6 15-year → 2031-11-01", () => {
...
test("10.3-T2/T3 2-unit loan: midpoint 2036-03-01 → termination 2036-04-01; leap-year February test; late March paid 04-09 → 2036-05-01", () => {
...
test("10.2-T9 LPMI options notice; 10.3-T5 legacy boarding termination", () => {
  assert.equal(lpmiOptionsNoticeDue(d("2035-07-01")), d("2035-07-31"));
  assert.deepEqual(legacyBoardingTermination(d("2014-01-01"), d("2026-10-01"), true, true), { terminate_on: d("2026-10-01"), escalate_officer: true });
...
test("10.4-T1/T8 disclosure cadence: last sent 2026-03-15, escrow statement 2027-02-20 → attached, next due 2028-02-20; null last-sent → boarding + 60", () => {
...
test("10.5-T1 annual plan E=2031-08-01: estimate $1,411.73; insurer transfer due 09-03; borrower refund due 09-15", () => {
...
test("10.5-T2/T3/T5/T6/T7/T8 monthly estimate $114; advance at E+40; dispute pays higher at day 40; LPMI legs; look-back shortfall; offset refused; R03", () => {
...
test("10.6-T1/T2/T3 denial content and due dates", () => {
  ...
  assert.equal(denialDue({ received_on: null, evidence_satisfied_on: null, scheduled_termination_on: d("2035-07-01"), state: "TX" }), d("2035-07-31"));   // 10.6-T5
...
test("10.6-T7/T8/T9 dispute routing, human review, evaluation link", () => {
