...
export const fundsPosition = (f: Funding): FundsPosition => (f.status === "disbursed" || f.status === "unwinding" || f.status === "unwound" ? "disbursed" : ["wire_released", "wire_accepted", "funds_at_agent", "returned"].includes(f.status) || (f.status === "cancelled" && ["wire_accepted", "funds_at_agent"].includes(f.prior_status ?? "")) ? "released_not_disbursed" : "not_released");
