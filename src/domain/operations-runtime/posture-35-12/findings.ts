...
export async function resolveFinding(d: PostureDeps, i: ResolveInput): Promise<ResolveResult> {
  ...
  if (cause === "exception") {
    ...
    const [ctl] = await d.db.query<{ security_control_code: string | null }>(`SELECT security_control_code FROM posture_controls WHERE code = $1 ORDER BY version DESC LIMIT 1`, [finding.control_code]);
    if (!ctl?.security_control_code || ctl.security_control_code !== ex.control_code) refuse(409, "FINDING_CLOSES_BY_EVIDENCE", `exception ${ex.id} is for ${ex.control_code}; ${finding.control_code} maps to ${ctl?.security_control_code ?? "no 19.2 control"} — an exception excepts the control it names (35.12 rule 3)`, { finding_id: finding.finding_id, exception_id: ex.id, exception_control: ex.control_code, finding_control: finding.control_code, mapped_control: ctl?.security_control_code ?? null });
