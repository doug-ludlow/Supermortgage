...
export class ClosingDisclosureService {
  ...
  scheduleCorrectedCd(applicationId: string, i: { disclosure_id: string; cd_reason: Exclude<CdReason, "initial">; input: Omit<CdRenderInput, "application_id" | "disclosure_id" | "cd_version" | "cd_reason">; evaluation?: RedisclosureEvaluation | null; gate: { run_id: string; apr_verdict: "pass" | "fail"; blocked_channels?: readonly string[] }; deliveries: readonly { consumer_id: string; channel: CdDeliveryChannel; at: string; esign_consent_id?: string | null; mailing_proof_id?: string | null; evidence_document_id?: string | null }[]; pdf_document_id?: string; /** 21.5's changed circumstances this corrected CD carries (a revised LE was barred by §1026.19(e)(4)(ii)) — 21.5 reads them off the event */ cc_ids?: readonly string[] }): { row: CdRow; corrected_event: DomainEvent; tolerance_test_invoked: boolean } {
