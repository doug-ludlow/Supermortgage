...
function assertDisclosureThenGoal(t: { messages: Json[]; pinned_card: Json | null }): void {
  const agent = t.messages.filter((m) => m["sender"] === "agent");
...
test("32.16-T1: Given a borrower message that is not an affirmative, not a flow reply and not \"human\", then the reply is produced by the agent turn (an `agent_turns` row exists with `model_version`, `prompt_version`, `context_hash`) and no placeholder copy key is used.", { todo: true });
test("32.16-T2: Given any turn, then its context contains no DU message, credit report field, findings text, fraud/QC entity or vendor payload (contract test over `context.ts`).", { todo: true });
test("32.16-T3: Given the model's tool calls in a turn, then each is a 32.16 bus tool with an `agent_decisions` row, and a call outside the contract (e.g. `payment.makeOneTime`) is refused by the bus with `command.refused` and never executed.", { todo: true });
test("32.16-T4: Given \"eight thousand two hundred a month\" with the R3 income card pending, then `card_instances.props.proposal.fields[0] = {path: \"monthly_income\", value: \"820000\", source: \"borrower_stated_unconfirmed\"}`, the thread shows the confirm chip, `application_income` is unchanged, and Confirm resolves the card with `evidence.source = borrower_stated`.", { todo: true });
test("32.16-T5: Given a model turn containing \"your rate is 6.125%\", then the guard rejects it, the regenerated turn uses `{{numbers.rate}}`, and the rendered message shows the projection's rate.", { todo: true });
test("32.16-T6: Given `ai_intake_mode = assisted` and no `mlo.review.completed`, when the turn is classified `particular_terms_presented`, then it is refused, `logSafeActivity` records it, and the reply is the step's default copy.", { todo: true });
test("32.16-T7: Given a turn that references `consent.esign.title`, then the rendered text equals the template exactly.", { todo: true });
test("32.16-T8: Given \"what's escrow?\" mid-R3, then the reply carries `explain.escrow` and restates the R3 ask; `session.next` before and after the turn is the same card.", { todo: true });
test("32.16-T9: Given three consecutive rejected or edited proposals on one card, then `human.request` runs with the transcript reference and `PersonCard{human_agent}` follows `human.transfer.completed`.", { todo: true });
test("32.16-T10: Given the 18.1 kill switch tripped for `intake`, then the turn is bypassed and the placeholder copy returns for every party until reset.", { todo: true });
