 ...
 * A claim is one person for CLAIM_HOURS; a second person is refused CLAIMED_BY_OTHER{staff_user_id} (the id, never the
...
import { hoursAfter, actorId, isUuid, obj, s, WorkRefused, CLAIM_HOURS, type ItemStatus, type Row, type SourceKind } from "./types.ts";
...
export interface ItemDeps { readonly db: Queryable; readonly events: EventStore; readonly now: string; readonly actor: Actor; readonly deferWrite: (fn: (q: Queryable) => Promise<void>) => void; readonly sessionId?: string | null }
...
/** `work.item.claim` — the session holds `required_role`; one person for CLAIM_HOURS (rule 9). */
export async function claimItem(d: ItemDeps, held: readonly string[], itemId: string): Promise<WorkItem> {
  ...
  if (it.status === "claimed" && it.claimed_by && it.claimed_by !== me && it.claim_expires_at && Date.parse(it.claim_expires_at) > Date.parse(d.now)) throw new WorkRefused(409, "CLAIMED_BY_OTHER", `item ${itemId} is claimed`, { staff_user_id: it.claimed_by, claim_expires_at: it.claim_expires_at });
  if (it.status === "waiting_approval") throw new WorkRefused(409, "WAITING_APPROVAL", `item ${itemId} awaits an officer`, { item_id: itemId });
  const claimed_at = d.now; const claim_expires_at = hoursAfter(d.now, CLAIM_HOURS);
...
export async function releaseItem(d: ItemDeps, itemId: string): Promise<WorkItem> {
  ...
  const me = byOf(d.actor);
  if (it.claimed_by && it.claimed_by !== me && it.claim_expires_at && Date.parse(it.claim_expires_at) > Date.parse(d.now)) throw new WorkRefused(409, "CLAIMED_BY_OTHER", `item ${itemId} is claimed`, { staff_user_id: it.claimed_by });
...
export async function collectSources(rt: Runtime, q: Queryable, now: string, ports: Required<WorkPorts>): Promise<Source[]> {
  ...
  for (const r of await store.queue({ now })) { if (r.kind === "breached_timer" && /^SM_SWEEP_HEARTBEAT_DAILY breached/.test(r.title)) continue; out.push(itemOfConsoleRow(rt, r)); }
