...
export interface CycleDef {
  ...
  /** Who appends `receipt_event` at the election: 35.3 (`election`, the default) or the cycle's owner (`owner` — 35.1's `projection_verify`, 35.2's `document_integrity`, 35.12's `posture.check` and `parallel_run.reconcile`, 35.5's `lockbox_ingest` / `ach_file_build` / `ach_returns_ingest` whose tools emit their own receipt literal: "the election emits `cycle.run.completed` only", rule 2; `cycle_receipts.receipt_event_id` is then NULL). */
...
export const selectors = {
  ...
  /** 35.12: one global unit per open parallel run — 0240's `parallel_runs` is append-only (rule 7: opened, day_reconciled, extended, closed rows share a `parallel_run_id`), so a run is open while its latest opened / extended / closed row is not `closed` (absent table → no unit) */
  open_parallel_runs: { name: "open_parallel_runs", select: async (q, w) => (await rowsOf<{ id: string }>(q, `SELECT parallel_run_id::text AS id FROM (SELECT DISTINCT ON (parallel_run_id) parallel_run_id, action FROM parallel_runs WHERE action IN ('opened', 'extended', 'closed') ORDER BY parallel_run_id, created_at DESC, id DESC) r WHERE r.action <> 'closed' ORDER BY 1`)).map((r) => ({ unit_id: r.id, input: { parallel_run_id: r.id, as_of_date: w.as_of_date } })) } satisfies NamedSelector,
} as const;
...
export const CYCLE_ROWS: readonly CycleDef[] = [
  ...
  // 35.8 rule 10: the tool emits `work.log.recon.run_completed` itself (src/domain/operations-runtime/work-35-8/recon.ts — SM_WORK_LOG_RECON_DAILY's satisfier and re-trigger on the global subject), so the election appends `cycle.run.completed` only (rule 2, as 35.1's and 35.2's); the sweep's work pass (work-35-8/sweep.ts) runs the day's reconciliation only when no run exists for the day — the planner's unit is the day's run
  def({ cycle_code: "work_log_recon", owner_process: "35.8", owner_agent: "case", unit_scope: "global", schedule: "once per calendar day", period_grammar: "day", selector: selectors.global, receipt_event: "work.log.recon.run_completed", receipt_emitted_by: "owner", serves_timer: "SM_WORK_LOG_RECON_DAILY", expected_by_rule: "same_day 23:59 ET" }),
  ...
  // 35.12's three cycles (runners.ts postureCheckRunner, dataScanRunner, parallelRunReconcileRunner — each the owner's bus tool as `compliance-sentinel`); `posture.check` emits its own receipt (check.ts: `posture.check.run_completed` once per environment-day, `posture.check.run_repeated` after — T5's one clock and one receipt; the election appends `cycle.run.completed` only); the rows escalate to the spec's roles (ciso, compliance, officer)
  def({ cycle_code: "posture.check", owner_process: "35.12", owner_agent: "compliance-sentinel", unit_scope: "global", schedule: "daily 05:30 ET and on every manifest", period_grammar: "day", selector: selectors.global, receipt_event: "posture.check.run_completed", receipt_emitted_by: "owner", serves_timer: "SM_PROD_POSTURE_DAILY", escalation_role: "ciso", expected_by_rule: "same_day 23:59 ET" }),
  def({ cycle_code: "data.scan", owner_process: "35.12", owner_agent: "compliance-sentinel", unit_scope: "global", schedule: "daily 05:45 ET on nonprod", period_grammar: "day", selector: selectors.global, receipt_event: "posture.data_scan.run_completed", escalation_role: "compliance", expected_by_rule: "same_day 23:59 ET" }),
  def({ cycle_code: "parallel_run.reconcile", owner_process: "35.12", owner_agent: "compliance-sentinel", unit_scope: "global", schedule: "daily 21:00 ET while a parallel_runs row is open", period_grammar: "day", selector: selectors.open_parallel_runs, receipt_event: "parallel_run.day.reconciled", receipt_emitted_by: "owner", depends_on: onCashiering(), serves_timer: "SM_PROD_PARALLEL_RUN_DAILY", escalation_role: "officer", expected_by_rule: "21:00 ET" }),
