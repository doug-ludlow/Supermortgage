...
- *Exceptions/corrections:* Delinquency Exception Summary/Details reports; "by the 10th calendar day of the month in which the exception report was issued, make corrections"; "on the 11th calendar day … receive … an updated final exception report." F-1-21 posts the exception reports "on the second calendar day following the second business day of the month" (the next business day when that day is not one — the 2026 Delinquency Reporting Calendar shows Sept 4, Oct 5, Nov 5, Dec 4, 2026, i.e. BD2 + 2 calendar days, not BD4), final correction days CD10–12 (Sept 11, Oct 10, Nov 12, Dec 10) and final reports CD11–13 **[PARTIALLY VERIFIED — graphic calendar; load actual dates annually]**; reclass deselection windows CD11–15/16 (apply only to June 1, 2007–Dec 1, 2008 MBS pools with forbearance/repayment status codes — F-1-25).
...
| `FNMA_F121_DQ_EXCEPTIONS_BD4` | deadline (inbound) | BD2 (`fannie_et`) | BD2 + 2 calendar days, 12:00 ET (next `fannie_et` business day 12:00 ET when that day is not a business day; the published Delinquency Reporting Calendar date overrides when loaded — F-1-21: "on the second calendar day following the second business day of the month") | 0 | exception report parsed | sev-2 → `human_portal_task` pull |
...
8. **Corrections:** exception report (posted the second calendar day after BD2, rolled to the next `fannie_et` business day — F-1-21; the `FNMA_F121_DQ_EXCEPTIONS_BD4` code is kept as an identifier) parsed (critical vs non-critical); critical exceptions corrected and retransmitted by CD10; the CD11 final report is reconciled line-by-line to `delinquency_report_lines` and stored.
