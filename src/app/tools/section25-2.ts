...
export const TOOLS_25_2: readonly ToolDef[] = defineTools("25.2", "disclosure", [
  ...
  { name: "deliverDisclosure", kind: "act", handler: compute((i, ctx, rt) => {
      ...
      return r; }),
    guardrails: [never("ELECTRONIC_NEEDS_ESIGN_CONSENT", "§1026.38(t)(3)(iii) (the electronic-form hook for the Closing Disclosure; §1026.17(a) excludes the §1026.19(f) disclosures); 15 U.S.C. 7001(c); 25.1 ESIGN_7001C_CONSENT", (i) => (i.channel === "esign_portal" || i.channel === "email_link") && !i.esign_consent_id, "electronic delivery of the CD needs the consumer's unrevoked E-SIGN consent — otherwise print/mail the same day under the mailbox rule")] },
