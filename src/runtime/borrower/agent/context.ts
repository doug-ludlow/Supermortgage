...
export const PROMPT_VERSION = "32.16-p4";
...
Words never commit; cards do. When the borrower states a fact the current card asks for, call card_propose with the value transcribed as a string (money in cents, choices as the option id) and say what you heard so they can tap Confirm; never calculate, round or infer a value. Their name and e-mail: when the journey's top need is the name-and-e-mail card (no name on file yet), ask for their name first and nothing else; when they say it, call card_propose with legal_name as they said it; then ask for their e-mail address; when they say it, write it as an address (spoken "at" is @, "dot" is ., no spaces, lower case) and call card_propose with both legal_name and email together, then read the address back so they can tap Confirm or fix it on the card; say once that this address is how they get back into this conversation if the call drops. Consents, credit authorization, declarations and demographics are never taken in words: point to the card. Never say a fact is done because the borrower said it. Ask for nothing except through a card or a tool. When a need has no card yet and the journey says it is next, call card_request for it so it appears on the rail as you ask.
...
export function buildContext(i: ContextInput): AgentContext {
  ...
  const view: P = { lender: i.partnerName, channel: i.channel, assurance_level: i.level, agent: i.routed_to, safe_mode: i.safeMode, ...(i.humanRequested ? { person_requested: "the borrower asked for a person and a callback request has already been queued for this message: say so plainly in your own words — no one is live right now, a person will pick it up — and do not call human_transfer or queue it again" } : {}), ...(i.journey ? { journey: i.journey } : {}), next_in_words, ...(i.rules ? { process_rules: { process: i.rules.process, note: "the rules of the current step, for your understanding — never quote them, never state a figure from them", text: i.rules.text } } : {}), record: rec.view, pending_cards: cards.view, session_next: i.next, recent_messages: compactMessages(i.messages, i.channel), ...(lead.view ? { lead_facts: lead.view } : {}), tokens_available: Object.keys(tokens) };
  const situation = JSON.stringify(view, null, 1);
  const borrower = i.borrowerText || (i.messages.some((m) => m.sender === "borrower") ? "(no new message — the borrower is back; restate where things stand and the current ask)" : "(the borrower just created their account and has not said anything yet — introduce yourself as Michelle, greet them by first name when one is on file (with no name on file, ask their name first: that is the first need), say what the first step is and ask the goal in your own words; if lead facts are present, acknowledge them in your words instead of asking again)");
