...
export interface CommitmentPolicy {
  ...
  /** C2-2-01 (VERIFIED): high-balance loans may not exceed this percentage of the aggregate UPB of a standard fixed-rate mandatory commitment (default 10); `null` = no concentration limit (non-TBA-eligible products such as ARMs). */
  readonly hbl_cap_pct: string | null;
}
...
export const DEFAULT_POLICY: CommitmentPolicy = {
  ...
  after_hours_commit: false, mandatory_enabled: false, extended_close_hhmm: "22:00", uncommitted_position_policy_days: 5, expected_purchase_ready_bd: 5, hbl_cap_pct: "10",
};
...
/** C2-2-01 (VERIFIED): high-balance loans "do not comprise more than 10% of the aggregate unpaid principal balance" of a standard fixed-rate mandatory commitment — `max_allowable_hbl_delivery_cents = round_half_up(hbl_cap_pct/100 × original)`; $5,000,000 → $500,000. Non-TBA-eligible products (ARMs) carry no concentration limit (`null`; 29.2 rule 1 keeps them best efforts). */
export function hblCap(originalAmountCents: Cents, hblCapPct: string | null): { hbl_cap_pct: string | null; max_allowable_hbl_delivery_cents: Cents | null } {
  if (hblCapPct === null) return { hbl_cap_pct: null, max_allowable_hbl_delivery_cents: null };
  return { hbl_cap_pct: dec(hblCapPct).toFixed(2, "HALF_UP"), max_allowable_hbl_delivery_cents: centsToDecimal(positive(originalAmountCents, "original_amount_cents")).mul(dec(hblCapPct)).div(HUNDRED).toCents("HALF_UP") };
}
/** C2-2-01: once a partial pair-off is requested "the minimum required delivery amount will be reduced to $50 below the revised commitment amount" (`revised = original − paired_off`). */
export const POST_PAIR_OFF_ALLOWANCE_CENTS: Cents = 5_000n;
export function postPairOffMinimum(originalAmountCents: Cents, pairedOffCents: Cents): { revised_commitment_cents: Cents; minimum_delivery_cents: Cents } {
  const revised = originalAmountCents - pairedOffCents; return { revised_commitment_cents: revised, minimum_delivery_cents: revised - POST_PAIR_OFF_ALLOWANCE_CENTS };
}
/** Rule 12: a pair-off is "calculated from the original commitment amount, not the low delivery tolerance" (C2-1.1-02); a standalone request must cover the whole undelivered amount — short of `undelivered − $50` it is refused (C2-2-01) — unless an extension of the rest is requested with it, "the sum of the extended amount and the paired-off amount must equal the total remaining commitment amount" (C2-1.1-04). $4,600,000 purchased on $5,000,000 → undelivered $400,000: $275,000 alone is refused (minimum $4,724,950 > $4,600,000); $275,000 + $125,000 extended is allowed; the default request is the full $400,000. */
export function pairOffSizing(i: { original_amount_cents: Cents; purchased_cents: Cents; paired_off_cents: Cents; requested_cents?: Cents | null; extension_remainder_cents?: Cents }): { undelivered_cents: Cents; amount_cents: Cents; extension_remainder_cents: Cents; revised_commitment_cents: Cents; post_pair_off_minimum_cents: Cents; allowed: boolean; rule: "C2-1.1-02" | "C2-2-01" | "C2-1.1-04" | null; reason: string | null } {
  const $ = (c: Cents): string => formatCents(c, { symbol: true });
  const undelivered = i.original_amount_cents - i.purchased_cents - i.paired_off_cents;
  const amount = i.requested_cents ?? undelivered; const ext = i.extension_remainder_cents ?? 0n;
  const post = postPairOffMinimum(i.original_amount_cents, i.paired_off_cents + amount);
  const base = { undelivered_cents: undelivered, amount_cents: amount, extension_remainder_cents: ext, revised_commitment_cents: post.revised_commitment_cents, post_pair_off_minimum_cents: post.minimum_delivery_cents };
  if (amount <= 0n || amount > undelivered) return { ...base, allowed: false, rule: "C2-1.1-02", reason: `C2-1.1-02: a pair-off of ${$(amount)} must be between $0.01 and the undelivered ${$(undelivered)} (original ${$(i.original_amount_cents)} − purchased ${$(i.purchased_cents)} − paired off ${$(i.paired_off_cents)}), measured from the original commitment amount` };
  if (ext < 0n) return { ...base, allowed: false, rule: "C2-1.1-04", reason: "C2-1.1-04: the extension requested with a pair-off cannot be negative" };
  if (ext > 0n) return amount + ext === undelivered ? { ...base, allowed: true, rule: null, reason: null } : { ...base, allowed: false, rule: "C2-1.1-04", reason: `C2-1.1-04: the sum of the extended amount and the paired-off amount must equal the total remaining commitment amount — ${$(amount)} + ${$(ext)} ≠ ${$(undelivered)}` };
  if (i.purchased_cents < post.minimum_delivery_cents) return { ...base, allowed: false, rule: "C2-2-01", reason: `C2-2-01: after a ${$(amount)} pair-off the minimum required delivery is ${$(post.minimum_delivery_cents)} (revised commitment ${$(post.revised_commitment_cents)} − $50) but only ${$(i.purchased_cents)} is delivered — pair off the whole undelivered ${$(undelivered)}, or extend the other ${$(undelivered - amount)} with it (C2-1.1-04)` };
  return { ...base, allowed: true, rule: null, reason: null };
}
/** Rule 12: mandatory pair-off / over-delivery `amount × (market − commitment)/100` — positive = fee, negative = cash back (C2-1.1-04). The undelivered $400,000 at 100.750 vs 101.250 → cash back $2,000.00; at 101.750 → fee $2,000.00. */
...
export const mandToleranceGate = (f: GateFacts): { open: boolean; reason?: string } => { const o = fc(f, "original_amount_cents"); if (o <= 0n) return { open: false, reason: "FNMA_PEWL_MAND_TOLERANCE_GATE: original_amount_cents is required" }; const t = mandatoryTolerance(o); const pu = fc(f, "purchased_cents"); const po = fc(f, "paired_off_cents"); const low = po > 0n ? postPairOffMinimum(o, po).minimum_delivery_cents : t.tolerance_low_cents; return pu >= low && pu <= t.tolerance_high_cents ? { open: true } : { open: false, reason: `FNMA_PEWL_MAND_TOLERANCE_GATE: purchased ${formatCents(pu, { symbol: true })} outside [${formatCents(low, { symbol: true })}, ${formatCents(t.tolerance_high_cents, { symbol: true })}]${po > 0n ? " (after a partial pair-off the minimum is the revised commitment − $50, C2-2-01)" : ""}` }; };
...
export class CommitmentService implements CommitmentPort {
  ...
  preparePairOffPackage(i: { commitment_id: string; market_price?: string; at?: string; amount_cents?: Cents; certain_non_delivery?: boolean; cure_probability?: string; reason?: string; payer?: FeePayer; officer_authorized?: boolean; channel?: "ui_operator" | "sales_desk"; extension_remainder_cents?: Cents }): CommitmentPairOff & { decision: "pair_off_now" | "carry"; package: Record<string, unknown> } {
    ...
    // Rule 12 (C2-1.1-02 / C2-2-01 / C2-1.1-04): a mandatory pair-off is measured from the original commitment amount and must cover the whole undelivered amount — alone, or with an extension of the rest requested with it
    const sizing = c.type === "mandatory" ? pairOffSizing({ original_amount_cents: c.original_amount_cents, purchased_cents: c.purchased_cents, paired_off_cents: c.paired_off_cents, requested_cents: i.amount_cents ?? null, ...(i.extension_remainder_cents !== undefined ? { extension_remainder_cents: i.extension_remainder_cents } : {}) }) : null;
    if (sizing && !sizing.allowed) throw new CommitmentRefused(sizing.rule === "C2-1.1-04" ? "FNMA_C2_1_1_04_PAIR_OFF_EXTENSION_SUM" : "FNMA_C2_2_01_POST_PAIR_OFF_MINIMUM", sizing.rule === "C2-1.1-04" ? "C2-1.1-04: the sum of the extended amount and the paired-off amount must equal the total remaining commitment amount" : "C2-2-01: once a partial pair-off is requested the minimum required delivery amount is reduced to $50 below the revised commitment amount; C2-1.1-02: pair-offs are calculated from the original commitment amount, not the low delivery tolerance", sizing.reason ?? "pair-off sizing refused");
    const amount = sizing ? sizing.amount_cents : (i.amount_cents ?? c.max_amount_cents);
    ...
    const pkg: Record<string, unknown> = { commitment_id_fnma: c.commitment_id_fnma, type: c.type, kind: "lender_requested", amount_cents: String(amount), commitment_price: c.commitment_price, market_price: dec(market).toFixed(3, "HALF_UP"), price_delta: fee.price_delta, fee_cents: String(fee.fee_cents), cash_back_cents: String(fee.cash_back_cents), fee_display: formatCents(fee.fee_cents, { symbol: true }), cash_back_display: formatCents(fee.cash_back_cents, { symbol: true }),
      reason: i.reason ?? null, ...(sizing ? { undelivered_cents: String(sizing.undelivered_cents), extension_remainder_cents: String(sizing.extension_remainder_cents), revised_commitment_cents: String(sizing.revised_commitment_cents), post_pair_off_minimum_cents: String(sizing.post_pair_off_minimum_cents), measured_from: "the original commitment amount, not the low delivery tolerance (C2-1.1-02)", pair_off_cutoff: "5:00 p.m. ET on the expiration date (C2-1.1-04; Fannie Mae accepts extension requests until 10:00 p.m. ET — SM policy 5:00 p.m. ET for both)" } : {}), decision, alternative: alt ? { days: alt.days, per_diem: alt.per_diem_display, carry_cents: String(alt.carry_cents), carry_display: formatCents(alt.carry_cents, { symbol: true }), auto_pair_off_on: cap!.cap_on, statement: alt.statement } : null, officer_approval_required: needsOfficer, threshold_cents: String(this.policy.pair_off_officer_threshold_cents), accept_window_seconds: UI_ACCEPT_WINDOW_SECONDS, payer: i.payer ?? "sm", request_by: c.expires_on ? etInstant(c.expires_on, isSifmaEarlyClose(c.expires_on) ? EARLY_CLOSE_HHMM : STANDARD_CLOSE_HHMM) : null };
  ...
  recordPairOff(i: { pair_off_id?: string; commitment_id: string; kind?: "lender_requested" | "automatic"; executed_at?: string; confirmation_document_id?: string | null; officer_approved?: boolean; channel?: "ui_operator" | "sales_desk"; fee_cents?: Cents; cash_back_cents?: Cents; amount_cents?: Cents; source?: "package" | "draft" }): CommitmentPairOff {
    ...
    // C2-2-01: after a partial pair-off the minimum delivery is the revised commitment − $50 — the low tolerance moves with it, and a commitment whose purchases already meet it is fulfilled (the pair-off requested before the 5:00 p.m. ET cutoff satisfies the expiry row)
    const post = c.type === "mandatory" ? postPairOffMinimum(c.original_amount_cents, c.paired_off_cents + done.amount_cents) : null;
    const fulfilled = post !== null && done.kind === "lender_requested" && c.purchased_cents > 0n && post.revised_commitment_cents > 0n && c.purchased_cents >= post.minimum_delivery_cents;
    const status: CommitmentStatus = c.type === "best_efforts" || (remaining <= 0n && !fulfilled) ? (done.kind === "automatic" ? "auto_paired_off" : "paired_off") : fulfilled ? "fulfilled" : c.status;
    const next = this.update(c.commitment_id, { paired_off_cents: c.paired_off_cents + done.amount_cents, remaining_balance_cents: remaining < 0n ? 0n : remaining, status, execution_channel: i.channel ?? "ui_operator", ...(post ? { tolerance_low_cents: post.minimum_delivery_cents } : {}) }, at);
    this.emit(next, "commitment.paired_off", { pair_off_id: done.pair_off_id, kind: done.kind, amount_cents: String(done.amount_cents), fee_cents: String(done.fee_cents), cash_back_cents: String(done.cash_back_cents), market_price: done.market_price, commitment_price: done.commitment_price, payer: done.payer, confirmation_document_id: done.confirmation_document_id, remaining_balance_cents: String(next.remaining_balance_cents), automatic: done.kind === "automatic", ...(post ? { revised_commitment_cents: String(post.revised_commitment_cents), post_pair_off_minimum_cents: String(post.minimum_delivery_cents) } : {}) }, at);
    if (fulfilled && post) this.emit(next, "commitment.fulfilled", { commitment_id_fnma: next.commitment_id_fnma, purchased_cents: String(next.purchased_cents), within_tolerance: true, after_pair_off: true, revised_commitment_cents: String(post.revised_commitment_cents), post_pair_off_minimum_cents: String(post.minimum_delivery_cents) }, at);
  ...
  prepareMandatoryPackage(i: { amount_cents: Cents; product_code: string; min_ptr: string; period_days: number; remittance_type?: RemittanceType; at?: string; flags?: Record<string, unknown>; officer_authorization?: { escalation_id: string; status: string } | null; hedge_request_id?: string | null; hbl_cap_pct?: string | null }): Commitment & { package: Record<string, unknown>; operator_escalation_id: string | null } {
    ...
    const ptrs = mandatoryPtrRange(i.min_ptr); const tol = mandatoryTolerance(i.amount_cents); const hbl = hblCap(i.amount_cents, i.hbl_cap_pct === undefined ? this.policy.hbl_cap_pct : i.hbl_cap_pct);   // C2-2-01: default 10% (column default), null for non-TBA products
    ...
    const c: Commitment = { ...this.newRow({ application_id: "", lock_id: "", lineage_id: "", lock_status: "n/a", mlo_approved: true, loan_amount_cents: i.amount_cents, note_rate: dec(i.min_ptr).add(Decimal.parse("0.25")).toFixed(4, "HALF_UP"), base_price: "0", product_code: i.product_code, lock_expires_on: expires.expires_on, rate_set_date: plainDate(etDate(at)), property_address: null, borrower_last_name: null, address_complete: true, du_casefile_id: null, underwriting_method: "other", remittance_type: i.remittance_type ?? this.policy.remittance_type },
      ...
      type: "mandatory", execution_channel: "ui_operator", application_id: null, lock_id: null, lineage_id: null, status: "authorized", ptr_range_low: ptrs.ptr_range_low, ptr_range_high: ptrs.ptr_range_high, pass_through_rate: ptrs.ptr_range_low, tolerance_low_cents: tol.tolerance_low_cents, tolerance_high_cents: tol.tolerance_high_cents, hbl_cap_pct: hbl.hbl_cap_pct, effective_on: plainDate(etDate(at)), commitment_period_days: daysBetween(plainDate(etDate(at)), expires.expires_on) };
    this.rows.push(c);
    const pkg: Record<string, unknown> = { execution_type: "mandatory", product_name: c.fnma_product_name, remittance_type: c.remittance_type, amount_cents: String(i.amount_cents), amount_display: formatCents(i.amount_cents, { symbol: true }), period_days: i.period_days, expires_on: expires.expires_on, rolled_from: addDays(plainDate(etDate(at)), i.period_days), ptrs: ptrs.ptrs, ptr_range: `${ptrs.ptr_range_low}–${ptrs.ptr_range_high}`, servicing_fee_bps: c.servicing_fee_bps,
      tolerance_low_cents: String(tol.tolerance_low_cents), tolerance_high_cents: String(tol.tolerance_high_cents), tolerance_cents: String(tol.tolerance_cents), over_delivery_cap_cents: String(tol.over_delivery_cap_cents), hbl_cap_pct: hbl.hbl_cap_pct, max_allowable_hbl_delivery_cents: hbl.max_allowable_hbl_delivery_cents === null ? null : String(hbl.max_allowable_hbl_delivery_cents), hbl_cap_basis: hbl.hbl_cap_pct === null ? "no concentration limit — non-TBA-eligible product (C2-2-01)" : "C2-2-01: high-balance loans ≤ hbl_cap_pct of the aggregate UPB of a standard fixed-rate mandatory commitment", officer_authorization_escalation_id: i.officer_authorization.escalation_id, hedge_request_id: i.hedge_request_id ?? null, accept_window_seconds: UI_ACCEPT_WINDOW_SECONDS, channel: "PE–WL UI (or Sales Desk recorded line)", request_by: etInstant(expires.expires_on, isSifmaEarlyClose(expires.expires_on) ? EARLY_CLOSE_HHMM : STANDARD_CLOSE_HHMM) };
