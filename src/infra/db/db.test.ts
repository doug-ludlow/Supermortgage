...
test("migrations: every file under db/migrations is applied to the test database (800 tables: public + restricted_fl)", { skip }, async () => {
  ...
  assert.equal(t!.c, 800n);   // the count a fresh `db/migrate.sh` run produces through 0240: 784 through 0150 (0150 adds 35.1's eight seam tables) plus 0170's five (35.7: role_grants, role_queue_snapshots, role_handovers, breakglass_uses, api_principals) plus 0240's eleven (35.12: environment_manifests, posture_controls, posture_checks, posture_findings, integration_switches, restore_drills, data_scans, parallel_runs, parallel_run_diffs, go_live_checklists, staff_oidc_identities); 0137–0141, 0151–0169 and 0171 create no table; the absolute number moves with every §35 migration that adds a table
});
