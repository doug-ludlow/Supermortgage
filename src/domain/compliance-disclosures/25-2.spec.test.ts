...
test("25.2 worked figures: recording fee +$12.00 (worked examples 6–7); loan calculations (o) on the refinance fixture from 25.1 worked example 1 under the 26.3 rounded-per-diem convention — prepaid interest $1,785.43, PFC $3,849.95, amount financed $556,150.05, finance charge $668,793.15, APR 6.159%, TIP 119.059%, P&I $3,402.62, total of payments $1,224,943.20; the (l)(7) escrow figures $525.00 / $1,875.00 / $6,300.00", () => {
  ...
  // loan calculations (o) come from `apr_calculations`, never typed: prepaid interest 19 × $93.97 = $1,785.43 → PFC $3,849.95 → amount financed $556,150.05 = $560,000.00 − $3,849.95; finance charge $668,793.15 = total of payments $1,224,943.20 + PFC − $560,000.00
  assert.equal(APR_V1.prepaid_interest_cents, 178_543n); assert.equal(APR_V1.prepaid_finance_charges_cents, 384_995n); assert.equal(APR_V1.amount_financed_cents, 55_615_005n); assert.equal(APR_V1.total_of_payments_cents, 122_494_320n); assert.equal(APR_V1.finance_charge_cents, 66_879_315n); assert.equal(APR_V1.pi_cents, 340_262n);
  assert.equal(APR_V1.apr_disclosed_str, "6.159"); assert.equal(APR_V1.tip_pct.toFixed(3), "119.059");
  // the superseded unrounded-per-diem figures ($1,785.48 → $556,150.00 / $668,793.20) differ by five cents: a CD that carried them would still be accurate under §1026.22 / §1026.38(o)(2), but they never appear on the CD (26.3-T2)
  const superseded = computeApr({ loan_amount_cents: 56_000_000n, note_rate_pct: "6.125", term_months: 360, term_start_date: D("2026-11-12"), first_payment_date: D("2027-01-01"), prepaid_finance_charges_cents: 385_000n, prepaid_interest_cents: 178_548n, method: "appendix_j_exact", checkpoint: "cd" });
  assert.equal(superseded.amount_financed_cents, 55_615_000n); assert.equal(superseded.finance_charge_cents, 66_879_320n); assert.equal(superseded.apr_disclosed_str, "6.159", "the five-cent difference does not move the disclosed APR");
  assert.equal(aprAccuracyTest({ disclosed_apr: superseded.apr_disclosed_str, actual_apr: APR_V1.apr_disclosed_str, transaction: { irregular_first_period: true }, disclosed_finance_charge_cents: superseded.finance_charge_cents, actual_finance_charge_cents: APR_V1.finance_charge_cents }).result, "pass", "overstated by $0.05 — accurate, but not the platform's figures");
