...
import { firstDeclarationsCard, HOME_ASKS, CREDIT_AUTHORIZATION_TEXT, CREDIT_AUTHORIZATION_VERSION, CREDIT_AUTHORIZATION_HASH } from "./3-entry.ts";
...
interface PropertyRow { readonly address_line1: string | null; readonly city: string | null; readonly state: string | null; readonly postal_code: string | null; readonly county: string | null; readonly property_type: string | null; readonly occupancy: string | null; readonly units: number | null; readonly estate_type?: string | null; readonly existing_clean_energy_lien?: boolean | null }
...
async function appContext(deps: FlowDeps, appId: string): Promise<AppCtx | null> {
  ...
  const [events, records, parties, props] = await Promise.all([deps.runtime.uow.events.byApplication(appId), deps.runtime.entities.load({ applicationId: appId }),
    ...
    deps.runtime.db.query<PropertyRow & P>(`SELECT address_line1, city, state, postal_code, county, property_type, NULL::text AS occupancy, units, estate_type, existing_clean_energy_lien FROM application_properties WHERE application_id = $1 ORDER BY is_subject DESC, created_at LIMIT 1`, [appId])]);
...
async function compressedCards(deps: FlowDeps, ctx: AppCtx): Promise<void> {
  ...
  for (const [k, party] of ctx.parties.entries()) {
    ...
    if (k === 0) {
      ...
      // 32.18 rule 7: the two facts only the borrower gives — the estate and a PACE / clean-energy lien on the home — are asked on every file, the compressed one too (3-entry's HOME_ASKS; the tap writes them to the subject application_properties row)
    ...
    // asked again per application: the declarations — 32.3 R5's own sequence (5a.A, 5a.E, the thirteen-item list, SQ-05 on "Something applies"; the next card raised by the borrower's tap in 3-entry's onCardResolved, the last tap asserting the fourteen answers as the borrower's own actor) — and the Reg C request
    const first = firstDeclarationsCard({ prefix: "declarations", key: abId, borrower_id });
    await sendCard(deps, subject, party, appCard({ ...first, props: { ...first.props, asked_again: true } }), ctx.now);
