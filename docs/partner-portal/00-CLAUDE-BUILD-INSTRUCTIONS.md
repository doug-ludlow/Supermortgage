...
> Status, 2026-09-17: Sessions 0–5 have run. 36.1–36.6 are built on `/v1/partner/*` (`src/runtime/partner-portal/`) and the audit measures section 36 at 40/40 units (`| 36 | 40/40 | 100.0 | 6/6 |` in `docs/audit/COVERAGE.md`). Session 5 built `apps/partner` on `/partners` (Next.js, the cookie proxy of §6, the screens of §7) and its walk, `src/domain/servicing-partner-portal/36-app.walk.test.ts` (a browser-driven `node:test`, run at landing and in CI): the seven checks of §8 Session 5 measured 7 pass / 0 fail on 2026-09-17 — sign-in as the seeded partner_admin, Home shows 12 monitored, Eligibility's three buckets sum to monitored-not-held, the fixture upload answers `already_loaded`, the loan page banner is "Monitored — Northlight Mortgage Servicing (FAKE partner) remains servicer", the Serviced tab is disabled with the 36.6 copy, a partner_ops user sees no upload control. Session 6 ran on 2026-09-17 (the seven-check walk held at the landing): `Dockerfile.partner` (+ `.dockerignore`), the `supermortgage-partner` Cloud Run service on its own `partner` service account (no secret), NEG and backend behind the load balancer's `/partners`, `/partners/*` rule (`infra/terraform`), the deploy workflow's `build-partner` job, the `/partners` smoke step and the partner walk (`apps/partner/tests/walk/partner-walk.mts`, seven outcomes after the borrower walk) — docs/DEPLOY.md "The partner portal". The deployed walk's first run follows that landing; it signs in as the seeded partner_admin, so a `seed_demo=true` dispatch precedes it. The Admin area shows a disable as DELTA-01 pending; no command was invented.
