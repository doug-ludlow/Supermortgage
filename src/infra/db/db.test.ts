...
test("migrations: every file under db/migrations is applied to the test database (747 tables: public + restricted_fl)", { skip }, async () => {
  ...
  assert.equal(t!.c, 747n);   // the count a fresh `db/migrate.sh` run produces through 0117 (public + restricted_fl base tables; 0116 oidc_identities, 0117 lead_tokens)
});
