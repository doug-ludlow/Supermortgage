...
export interface CreditTransferInput { readonly old_loan_id: string; readonly new_loan_id: string; readonly new_application_id: string; readonly consent: CreditConsent | null; readonly payoff_date: PlainDate; readonly settlement_date: PlainDate; readonly old_balance_after_final_disbursements_cents: Cents; readonly target_at_start_cents: Cents; readonly fnma_ti_account_id?: string; readonly actor?: Actor; }
...
export function postCreditTransfer(events: EventStore, ledger: Ledger, i: CreditTransferInput): EscrowCreditTransfer {
  ...
  const actor = i.actor ?? ESCROW_AGENT; const fnmaTi = i.fnma_ti_account_id ?? FNMA_TI_ACCOUNT_DEFAULT;
  ...
  const set2 = ledger.post({ effectiveDate: i.payoff_date, description: `T&I cash: Fannie Mae ${fnmaTi} → ${CUSTODIAL_TI_PREPURCHASE} (funds due borrower, F-1-03)`, lines: [
    { account: { scope: "custodial", custodialAccountId: CUSTODIAL_TI_PREPURCHASE, account: "custodial_ti_cash" }, amountCents: credited, ruleRef: "30.3 rule 8: Dr custodial_ti_prepurchase" },
