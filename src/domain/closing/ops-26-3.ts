...
export function transition(f: Funding, to: FundingStatus): Funding {
  ...
  // a wire in flight is not cancelled (recall it first); a wire the beneficiary bank has accepted may be — that is the cancellation the timer table's SM_O73_FUNDS_RETURN_2BD arms on (`funding.cancelled{after_wire_accepted=true}`: the funds come back within 2 federal business days, matched to the advance by returnWire)
  if (to === "cancelled") { if (idx(from) >= idx("wire_released") && !["held", "returned", "wire_accepted", "funds_at_agent"].includes(from)) throw new FundingRefused("CANCEL_AFTER_RELEASE", "26.3 state machine", `cannot cancel from ${from} without a return`); return { ...f, status: "cancelled", prior_status: from }; }
  if (to === "returned") { if (!["wire_released", "wire_accepted", "funds_at_agent"].includes(from) && !(from === "cancelled" && ["wire_accepted", "funds_at_agent"].includes(f.prior_status ?? ""))) throw new FundingRefused("RETURN_STATE", "26.3 state machine", `returned only from wire_released/wire_accepted/funds_at_agent, or from a cancellation after the wire was accepted (was ${from}${f.prior_status ? ` after ${f.prior_status}` : ""})`); return { ...f, status: "returned", prior_status: from }; }
...
/** Where the money is: a `cancelled` funding whose wire the beneficiary bank had accepted (SM_O73_FUNDS_RETURN_2BD's case) is still released-not-disbursed until `returnWire` matches the funds back to the advance. */
export const fundsPosition = (f: Funding): FundsPosition => (f.status === "disbursed" || f.status === "unwinding" || f.status === "unwound" ? "disbursed" : ["wire_released", "wire_accepted", "funds_at_agent", "returned"].includes(f.status) || (f.status === "cancelled" && ["wire_released", "wire_accepted", "funds_at_agent"].includes(f.prior_status ?? "")) ? "released_not_disbursed" : "not_released");
