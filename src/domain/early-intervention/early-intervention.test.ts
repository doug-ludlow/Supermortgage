...
test("11.5-T1..T6: hardship path, credit path with representative score and HTI, reserves strictness, 60-day reroute, FICO age, PCS 50 miles", () => {
  ...
  const b = ID.evaluate({ ...base, hardship_type: "reduction_in_income", credit: { scores: [601, 612, 620], fico_date: D("2026-10-20"), delinquencies_30_in_6m: 1, housing_expense_cents: cents("2000"), gross_income_cents: cents("4642.72") } });
  ...
  const b2 = ID.evaluate({ ...base, hardship_type: "reduction_in_income", credit: { scores: [601, 612, 620], fico_date: D("2026-10-20"), delinquencies_30_in_6m: 1, housing_expense_cents: cents("2000"), gross_income_cents: cents("6031.25") } }); assert.equal(b2.outcome, "ineligible");
  ...
  const old = ID.evaluate({ ...base, hardship_type: "reduction_in_income", credit: { scores: [600], fico_date: D("2026-07-21"), delinquencies_30_in_6m: 2, housing_expense_cents: 1n, gross_income_cents: 1n } }); assert.equal(old.outcome, "ineligible"); if (old.outcome === "ineligible") assert.deepEqual(old.failed, ["FNMA_D2101_FICO_AGE_90"]);
