...
| Timers | `FNMA_C1201_CURTAILMENT_APPLY_IMMEDIATE_0BD`, `FNMA_C1201_DELINQUENT_CURE_FIRST_GATE`, `FNMA_C1201_NIB_ORDER_GATE`, `FNMA_C1201_REAPPLY_ELIGIBILITY_GATE`, `FNMA_C4301_LAR83_REAMORT_NEXTBD_2000`, `FNMA_LL202605_EVENT_NEXTBD_0300`, `SM_CURTAILMENT_PAYOFF_ROUTE_GATE`, `SM_REAMORT_FORM181_DELIVERY_10BD` |
...
**Investor reporting hooks (Section 5.1/5.2, verified there):** the re-amortization is an *unscheduled* Transaction Type 83 (IRM 3-03: "a change to the loan's P&I due to a principal curtailment made by the borrower"); IRM 3-03 attaches its "prior to 8 PM eastern standard time on the 5th Business Day after the Scheduled Payment Calculation Date" clock only to the three *Scheduled* rows and states no deadline for the unscheduled row, so the governing clock is C-4.3-01's general rule for a transaction that is "not a removal transaction" — "by 8 p.m. eastern time on the next business day after the servicer processes the transaction in its system" (`FNMA_C4301_LAR83_REAMORT_NEXTBD_2000`; 5.1's `FNMA_IRM_LAR83_5BD_2000` remains the scheduled-change clock); a curtailment is a separate LL-2026-05 "Loan Curtailment" event (same-day processing, 3:00 a.m. ET next-BD deadline), reported on the legacy LAR 96 as principal in the period; IRM Ch. 4 rules: curtailments on deferral/mod loans apply to interest-bearing UPB first unless ≥ interest-bearing UPB (matches C-1.2-01); same-month MBS curtailments reduce the balance and report as principal; S/S loans pass curtailments through at the next remittance and MBS Express drafts unscheduled principal on BD4 after the month collected; a curtailment can never be reported as a negative in a later period — corrections are reversals plus re-reporting.
...
| `FNMA_C4301_LAR83_REAMORT_NEXTBD_2000` | deadline (IRM 3-03 sets no deadline for an *unscheduled* TT83 — its 5th-business-day 8 p.m. ET clock applies only to scheduled changes; C-4.3-01's general non-removal rule governs) | `loan_terms.activated{reason=reamortization}` (new `loan_terms` version booked at execution — 2026-10-20 in example H; 2.4 owns this code, 5.1's `FNMA_IRM_LAR83_5BD_2000` remains for scheduled changes) | `processed_at` | next business day 20:00 America/New_York (`business_days_fannie_et`, 1) | `rate_payment.change` (TT83, unscheduled) submitted | (5.1) sev-2 |
...
6. **Re-amortization (Form 181).** New P&I = `round_half_up(UPB × r / (1 − (1 + r)^−n))` with r = note rate ÷ 12, n = remaining term in months to the original maturity, UPB = current interest-bearing UPB; escrow unchanged; effective with the first installment due ≥ 30 days after execution **[policy]**; new `loan_terms` version; LAR 83/`rate_payment.change` submitted by 20:00 ET the next Fannie Mae business day after the new `loan_terms` version is booked (C-4.3-01 general non-removal rule, `FNMA_C4301_LAR83_REAMORT_NEXTBD_2000`; IRM 3-03's 5th-business-day clock applies only to *scheduled* changes and an unscheduled TT83 has no "scheduled calculation date") (5.1); not a modification for Flex Mod eligibility (12.8 reads `reamortizations`, not `modifications`); no fee to the borrower unless permitted by A2-3-05 and law **[policy: no fee]**.
...
10. **Worked example H — re-amortization.** After a 5,000,000¢ ($50,000.00) curtailment leaves UPB 19,854,677¢ with 346 months remaining at 6.500%: r = 0.065 ÷ 12 = 0.005416667 (decimal.js precision 20); (1+r)^−346 = 0.154261039; new P&I = 19,854,677 × 0.005416667 ÷ (1 − 0.154261039) = 107,546.17 ÷ 0.845738961 = 127,162.4… → **127,162¢ ($1,271.62)** vs. 158,017¢ — offered on Form 181, effective with the installment due 2026-12-01, LAR 83/`rate_payment.change` submitted by 20:00 ET the next Fannie Mae business day after the new `loan_terms` version is booked (executed and booked Tue 2026-10-20 → due Wed 2026-10-21 20:00 ET; C-4.3-01). Rounding: the P&I is rounded half-up to cents once, at the end.
...
#### Audit and evidence
Designation evidence (image/portal field/transcript reference), allocation plan with the F-1-09 ordering rule reference, IB/NIB split, decision records for redirects and reapplications (with the four C-1.2-01 eligibility tests), Form 181 executed copies and custodian/eVault delivery evidence, `loan_terms` version history, investor event ids/acks, and timer history for the 0-BD application and next-business-day LAR 83 (`FNMA_C4301_LAR83_REAMORT_NEXTBD_2000`) clocks.
...
- Section 5.1/5.2 (IRM Ch. 4 rules, LAR 83, MBS Express BD4), Servicing Changes Reference Guide v1.0 (curtailment as a separate event); IRM 3-03 (Reporting a Transaction Type 83: scheduled vs. unscheduled rows) and Servicing Guide C-4.3-01 (next-business-day 8 p.m. ET rule for non-removal transactions) — verified 2026-09-15 against the 2026-04-08 IRM and the 2026-08-12 Guide
