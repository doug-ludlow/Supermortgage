...
export interface CycleDef {
  ...
  /** Who appends `receipt_event` at the election: 35.3 (`election`, the default) or the cycle's owner (`owner` — 35.1's `projection_verify`, 35.2's `document_integrity`, 35.12's `parallel_run.reconcile`: "the election emits `cycle.run.completed` only", rule 2; `cycle_receipts.receipt_event_id` is then NULL). */
