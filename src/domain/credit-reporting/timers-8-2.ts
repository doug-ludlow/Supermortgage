 ...
 * evaluator, anchorField?, offset?, why })` per 8.2 registry row whose trigger/satisfied column names an event the
...
export function applySatisfiedOverrides_8_2(reg: TimerRegistry): void {
  ...
  void o; // no 8.2 overrides yet — add `o(code, { … , why })` rows here.
}
