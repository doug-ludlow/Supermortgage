...
async function typeIdentity(b: S4Borrower, x: S4Party): Promise<void> {
  ...
  // 32.3 E5 (amended): the residence basis and the months at the address are asked on every file — the typed card carries them too, and its tap writes the Current du_residences row
  assert.deepEqual((idc.props["fields"] as { path: string; value: string; source: string }[]).map((f) => [f.path, f.value, f.source]), [["legal_name", "", "borrower"], ["date_of_birth", "", "borrower"], ["current_address", "", "borrower"], ["residency_basis", "", "borrower"], ["monthly_rent_cents", "", "borrower"], ["months_at_address", "", "borrower"]], "the same ConfirmCard, empty, source=borrower, with the residence asks");
  assert.deepEqual(idc.props["required_paths"], ["legal_name", "date_of_birth", "current_address", "residency_basis", "months_at_address"]); assert.equal(idc.props["helper_copy_key"], "identity.residence.why");
  const short = await resolve(x.token, idc.card_instance_id, fieldsEvidence(idc, { legal_name: b.name, date_of_birth: b.dob, current_address: b.address })); assert.equal(short.status, 409, JSON.stringify(short.body)); assert.equal(short.body["code"], "CARD_FIELD_REQUIRED");
  const confirmed = await resolve(x.token, idc.card_instance_id, fieldsEvidence(idc, { legal_name: b.name, date_of_birth: b.dob, current_address: b.address, residency_basis: "own", months_at_address: "72" })); assert.equal(confirmed.status, 201, JSON.stringify(confirmed.body));
  assert.ok((confirmed.body["events"] as string[]).includes("du.graph.residence.written"), `the tap wrote the Current du_residences row in its own transaction: ${JSON.stringify(confirmed.body["events"])}`);
  const residences = await db.query<{ residency_type: string; residency_basis: string; duration_months: number }>(`SELECT r.residency_type, r.residency_basis, r.duration_months FROM du_residences r JOIN application_borrowers ab ON ab.id = r.application_borrower_id WHERE ab.application_id = $1 AND ab.party_id = $2`, [x.appId, x.partyId]);
  assert.deepEqual(residences, [{ residency_type: "Current", residency_basis: "Own", duration_months: 72 }], "one Current du_residences row for the typer (32.3 E5 / 23.5 writeResidence)");
...
test("32.14-T14: Given `terms_presented`, when the borrower taps Show me my rate, then `application.received` is logged, `REGB_1002_9_DECISION_30` is armed, and the three E6 consents ride that tap (32.17 rule 20): the E-SIGN row `pending_verification`, the TCPA row `active`, the hard-pull authorization captured at once at L2 naming the Show me my rate card — no ConsentCard, no typed name, no identity gate (32.17 rule 18).", { skip }, async () => {
  ...
  // 32.17 rule 20: the E6 consents rode the Show me my rate tap — no cards; the rows for Quinn, the hard-pull authorization at L2 naming the card
  assert.equal((await cardsOf(x.appId, x.partyId)).filter((c) => c.kind === "ConsentCard" && c.status === "pending" && c.copy_key !== "consent.credit.soft.title").length, 0, "no E6 ConsentCard");
  assert.equal(typeof proceed.props["statement"], "string", "the statement on Show me my rate");
  const rows = await db.query<{ kind: string; status: string }>(`SELECT kind::text AS kind, status FROM consents WHERE party_id = $1 AND kind IN ('esign', 'tcpa_sms') ORDER BY kind`, [x.partyId]);
  ...
  const hard = (await appEvents(x.appId, "credit.authorization.captured")).filter((e) => e.payload["kind"] === "hard_pull"); assert.equal(hard.length, 1, "the hard-pull authorization written on the tap, no identity gate");
  assert.equal(hard[0]!.payload["card_instance_id"], proceed.card_instance_id, "the Show me my rate card's tap is the affirmation");
});
