...
export interface SinkDeps { readonly runtime: Runtime; readonly events: EventStore; readonly actor: Actor; readonly now: () => string; readonly blobs: ObjectStorePort; readonly deferLate: (fn: (q: Queryable) => Promise<void>) => void; }
...
export class PgArtifactSink implements ArtifactSink {
  ...
  rendered(input: { documentId: string; templateCode: string; version: TemplateVersion; payload: Record<string, unknown>; loanId?: string; applicationId?: string; caseId?: string }, rendered: Rendered): { document_id: string; sha256: string; byte_size: number; page_count: number; blocks: Rendered["blocks"] } {
    ...
    if (!this.knownIds.has(id)) {
      ...
      if (storeReachable) ctx.events.append({ type: "document.stored", ...key, aggregate, actor: ctx.actor, payload: { document_id: id, storage_uri: `${blobs.vendorName}://${id}#1`, stored_generation: "1", sha256: pdf.sha256, stored_at: ctx.now } });
      ...
      this.deps.deferLate(async (q) => {
        ...
        if (storeReachable && r.storage_status !== "stored") throw new Error(`HASH_MISMATCH_ON_PUT: the object store's re-read of ${id} did not match the staged bytes (${r.drain?.error ?? "no drain"}); the render is refused and nothing is written`);
      });
    }
...
export function noticeServiceFor(runtime: Runtime, ctx: UowContext, actor: Actor, deferLate: (fn: (q: Queryable) => Promise<void>) => void): { notices: NoticeService | undefined; sink: PgArtifactSink | undefined } {
  if (!runtime.ports.printMail || !runtime.ports.edelivery) return { notices: undefined, sink: undefined };
  const sink = new PgArtifactSink({ runtime, events: ctx.events, actor, now: () => ctx.clock.now(), blobs: runtime.blobs, deferLate });
