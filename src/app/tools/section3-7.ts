...
export const TOOLS_3_7: readonly ToolDef[] = defineTools("3.7", "escrow", [
  ...
  { name: "releaseDisbursement", kind: "act",
    ...
    dualControl: { role: "officer", threshold: (i, probe) => isRelease(i) && releaseApproval({ amount_cents: cents(i.amount_cents), payee_instruction_changed_on: i.payee_instruction_changed_on ? D(str(i, "payee_instruction_changed_on")) : null, release_on: i.release_on ? D(str(i, "release_on")) : D(probe.now.slice(0, 10)), new_payee: flag(i, "new_payee") }).dual_approval_required },
