...
export function applyTransferTimerOverrides(reg: TimerRegistry): void {
  ...
  o("FNMA_LL_2026_01_FORBEARANCE_CUMULATIVE_12M", { evaluator: "1.7.forbearanceCumulativeWithin12Months", why: "§1.7 timer table: `assertGateOpen` on forbearance extension — two independent limbs: cumulative ≤ 12 months as measured from the start date of the initial forbearance plan AND the extension must not result in the loan becoming greater than 12 months delinquent, unless Fannie Mae approves an exception (Servicing Guide D2-3.2-01 (04/08/2026), which carries the LL-2026-01 rule; 12.4's FNMA_LL202601_FORB_CUMULATIVE_12M and FNMA_LL202601_FORB_DELQ_12M)." });
