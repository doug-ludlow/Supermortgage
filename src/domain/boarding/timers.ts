...
export function applyBoardingTimerOverrides(reg: TimerRegistry): void {
  ...
  // "for every escrow category": the boarding service tracks Fannie Mae's per-category acknowledgments and marks the one that completes the set (service.ts recordEscrowSetupAck).
  reg.override("LL_2026_05_ESCROW_SETUP_ACQUIRED_BD1", { satisfied: "`investor_events.acked{type=EscrowSetup, every_category=true}`",
    why: "§1.1 timer table: '`investor_events.acked{type=EscrowSetup}` for every escrow category' (LL-2026-05: an Escrow Setup event per applicable escrow item category type)." });
}
