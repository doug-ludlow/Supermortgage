# 12 — Message copy library

Every string the borrower reads from the assistant or on a card is keyed here; the UI never hard-codes sentences. Regulatory **notices keep their template text** (Notice Registry) — this library holds the assistant's accompanying lines and card labels only. Tokens per 01 §7.4. Grade-8 reading level. Where a channel variant is not given, SMS uses the first sentence plus the deep link; e-mail uses the full text plus the CAN-SPAM footer when marketing.
...
- `valuation.low.choice` — ChoiceCard — "The appraised value is {{money}}, below {{price|requested}}. Options:" — options per 06 §2.
...
- `offer.card` — OfferCard — structure per 09 §2 — options `Yes, let's do it` · `Not now` · `Never`.
