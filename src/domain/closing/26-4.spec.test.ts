...
test("26.4-T12: Given a Maine loan, then 26.1 renders Form 3749, the `signing_officer` executes it at closing, it is recorded promptly after the mortgage, `recorded_assignment_to_mers` is expected, and no other assignment (intervening, to Fannie Mae) exists on any non-CEMA loan in the population (NY CEMA loans carry their consolidated prior-lender assignments, which the query below excludes together with the Maine rows) — a query for `closing_documents.kind` containing \"assignment\" outside Maine and outside NY CEMA loans returns zero rows.", async () => {
  ...
  // E-2-01's NY CEMA row: the consolidated prior-lender assignments are custodial documents inside the Form 3172 package — the query excludes them together with the Maine rows; the same kind on a non-CEMA loan is still a defect
  h.rt.store.put("closing_documents", "DOC-NY-CEMA-ASSIGN", { application_id: "APP-NY-1", state: "NY", kind: "cema_prior_lender_assignment", cema: true }, AGENT, et("2026-11-16", "12:00"));
  const q2 = await h.run("computeMersAnchor", { op: "assignment_population" }); assert.deepEqual(q2.offending, []);
  assert.deepEqual(assignmentPopulationCheck([{ application_id: "APP-NY-1", state: "NY", kind: "cema_prior_lender_assignment", cema: true }]), []);
  assert.equal(assignmentPopulationCheck([{ application_id: "APP-NY-1", state: "NY", kind: "cema_prior_lender_assignment" }]).length, 1, "an assignment on a non-CEMA loan outside Maine is a population defect");
  assert.match(decideAssignments({ state: "NY" }).basis, /NY CEMA row's 'all required mortgage assignments'/);
