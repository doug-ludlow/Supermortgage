...
export function applySatisfiedOverrides_7_1(reg: TimerRegistry): void {
  ...
  o("REGZ_1026_41E6_CHARGEOFF_NOTICE_30", { trigger: "`loan.charged_off*`", anchorField: "notice_anchor_on",
    why: "§7.1 timer table: `loan.charged_off` → anchor 'the later of the charge-off date and the date of the most recent periodic statement sent (§1026.41(e)(6)(i)(B): \"within 30 days of charge-off or the most recent periodic statement\" — either anchor satisfies; a `statement.sent` after charge-off re-anchors)', +30 calendar_days. StatementCycleService.recordChargeOff carries the later date as `notice_anchor_on` (7.1-T7: approved Nov 3 with no later statement → notice by Dec 3); a periodic statement sent after charge-off appends `loan.charged_off.notice_reanchored{notice_anchor_on=statement date}` — covered by the `loan.charged_off*` trigger — which arms the fresh instance while the superseded one is cancelled (comment 41(e)(6)-2). Satisfied by `notice.sent{template=NTC_REGZ_41E6_CHARGEOFF_SUSPENSION}` (section override; NoticeService.send)." });
