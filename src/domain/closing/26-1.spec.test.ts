...
test("26.1-T2: Given an Arizona 1-unit primary refinance with `closing.enote_default=true` and partner eMortgage approval, when `selectDocumentSet` runs, then the set is `{3200e, 3003 (07/2021), final 1003, H-8 ×2 per consumer, closing instructions}` with no riders and `closing_type=ron`.", async () => {
  ...
  // E-2-07 d) Signature Area: the 3200e omits "[Sign Original Only]" (recorded in every family=enote template's authorized_changes_applied; DQC_ENOTE_ARC_VIEW checks it); the paper Form 3200 keeps it
  const az = refi(); const azT = computeNoteTerms({ principal_cents: az.loan_amount_cents, note_rate_pct: az.note_rate_pct, term_months: 360, scheduled_disbursement_date: az.scheduled_disbursement_date, state: "AZ" });
  const azDocs = sel.documents.filter((d) => d.kind !== "closing_instructions").map((d) => renderDocument(d, az, azT, MIN, "h", sel)); const enoteDoc = azDocs.find((d) => d.kind === "enote")!;
  assert.match(enoteDoc.text, /WITNESS THE HAND\(S\) AND SEAL\(S\) OF THE UNDERSIGNED\.\n/); assert.ok(!enoteDoc.text.includes(SIGN_ORIGINAL_ONLY), "eNote forms omit [Sign Original Only] (E-2-07 d)");
  assert.ok(DEFAULT_TEMPLATE_LIBRARY.filter((x) => x.family === "enote").every((x) => x.authorized_changes_applied.includes(ENOTE_SIGNATURE_AREA_CHANGE)), "every family=enote template records the E-2-07 d) authorized change");
  const paper = refi({ enote_default: false }); const paperSel = selectDocumentSet(paper); const paperNote = renderDocument(paperSel.documents[0]!, paper, azT, MIN, "h", paperSel);
  assert.equal(paperNote.kind, "note"); assert.ok(paperNote.text.includes(`WITNESS THE HAND(S) AND SEAL(S) OF THE UNDERSIGNED. ${SIGN_ORIGINAL_ONLY}`), "the paper Form 3200 keeps [Sign Original Only]");
  assert.equal(runDocumentQc(az, azT, sel, azDocs, upstream(az, sel)).checks.find((c) => c.rule_code === "DQC_ENOTE_ARC_VIEW")!.result, "pass");
  const ev = runDocumentQc(az, azT, sel, azDocs.map((d) => d.kind === "enote" ? { ...d, text: `${d.text} ${SIGN_ORIGINAL_ONLY}` } : d), upstream(az, sel)).checks.find((c) => c.rule_code === "DQC_ENOTE_ARC_VIEW")!; assert.equal(ev.result, "fail"); assert.match(ev.reason!, /E-2-07 d\) Signature Area/);
});
...
test("26.1-T9: Given a borrower who is trustee and settlor of a revocable trust and the credit applicant, then the note signature line reads \"individually and as Trustee of the … Trust under trust instrument dated …\" and the deed of trust carries the settlor acknowledgment paragraph.", () => {
  ...
  // rule (8) POA signers / B1-1-01: when the note or security instrument and the final 1003 are POA-executed, the initial 1003 must be personally signed by the borrower unless one of the three B1-1-01 exceptions is recorded in 24.4 poa_reviews — DQC_1003_FINAL_TERMS
  const poa = refi({ application_id: "APP-POA-1", vesting_text: "R. Borrower, a married person", borrowers: [{ party_id: "B1", legal_name: "R. Borrower", credit_used: true, on_title: true, capacities: ["borrower"], attorney_in_fact: "A. Agent" }], poa: { agent_name: "A. Agent", principal_party_id: "B1", poa_document_id: "DOC-POA-1" } });
  const poaSel = selectDocumentSet(poa); const poaT = computeNoteTerms({ principal_cents: poa.loan_amount_cents, note_rate_pct: poa.note_rate_pct, term_months: 360, scheduled_disbursement_date: poa.scheduled_disbursement_date, state: "AZ" });
  const poaDocs = poaSel.documents.filter((d) => d.kind !== "closing_instructions").map((d) => renderDocument(d, poa, poaT, MIN, "h", poaSel));
  assert.ok(poaSel.documents.some((d) => d.kind === "poa_copy")); assert.equal(poaSel.note_signers[0]!.capacity, "attorney_in_fact"); assert.match(poaDocs.find((d) => d.kind === "final_1003")!.signers[0]!.signature_line, /R\. Borrower, by A\. Agent, as attorney-in-fact/);
  const poaUp = upstream(poa, poaSel); const dqc1003 = (u: QcUpstream) => runDocumentQc(poa, poaT, poaSel, poaDocs, u).checks.find((c) => c.rule_code === "DQC_1003_FINAL_TERMS")!;
  const unsigned = dqc1003(poaUp); assert.equal(unsigned.result, "fail"); assert.match(unsigned.reason!, /initial Form 1003 must be personally signed by the borrower/); assert.match(unsigned.actual, /no exception recorded/);
  assert.equal(dqc1003({ ...poaUp, urla_1003: { ...poaUp.urla_1003, initial_1003_personally_signed: true } }).result, "pass", "initial 1003 personally signed by the borrower");
  for (const ex of ["military_service_outside_us", "attorney_in_fact_personal_capacity", "applicable_law"] as const) assert.equal(dqc1003({ ...poaUp, urla_1003: { ...poaUp.urla_1003, initial_1003_personally_signed: false, poa_1003_exception: ex } }).result, "pass", `B1-1-01 exception ${ex} recorded in 24.4 poa_reviews`);
});
...
test("26.1-T10: Given a 2-1 buydown at 6.500% on $412,000 funded by the seller, then the schedule computes $516.58 and $264.83 monthly subsidies, total $9,376.92, SFC 009, the note shows 6.500%/$2,604.12, and the agreement carries the \"not relieved of obligation\" clause.", () => {
  ...
  assert.equal(a.eligible, true); assert.equal(a.classification, "moderate"); assert.equal(a.sfc, "009"); assert.equal(a.total_subsidy_cents, 937_692n); assert.equal(a.ipc_cap_cents, 2_746_667n, "B3-4.1-02: exactly 90% CLTV is in the 75.01%–90% band → 6%: $457,777.78 × 6% = $27,466.67"); assert.equal(a.ipc_ok, true); assert.equal(ipcCapPct("90"), "6"); assert.equal(ipcCapPct("90.01"), "3"); assert.equal(ipcCapPct("75.01"), "6"); assert.equal(ipcCapPct("75"), "9"); assert.equal(a.custodial_account, "t_and_i_custodial");
...
test("26.1-T12: Given a Montana property, then Form 3158 is in the set and the closing instructions forbid a post-closing assignment to MERS; given Maine, the mortgage carries no MERS-as-original-mortgagee language (`runDocumentQc` fails a Maine set whose mortgage names MERS as nominee — B8-7-01: such loans \"are ineligible for delivery to Fannie Mae\") and Form 3749 is rendered for `signing_officer` execution.", () => {
  ...
  // B8-7-01: the Maine mortgage carries no MERS-as-original-mortgagee (MOM) language — a Maine security instrument modified to name MERS is ineligible for delivery; runDocumentQc fails such a set (DQC_MIN_CHECK_DIGIT)
  assert.match(meSel.documents.find((d) => d.kind === "security_instrument")!.note!, /no MERS-as-original-mortgagee/);
  const meDocs = meSel.documents.filter((d) => d.kind !== "closing_instructions").map((d) => renderDocument(d, me, t, MIN, "h", meSel)); const meSi = meDocs.find((d) => d.kind === "security_instrument")!;
  assert.equal(meSi.form_number, "3020"); assert.equal(meSi.mom_language_present, false); assert.doesNotMatch(meSi.text, /nominee for Lender/); assert.match(meSi.text, /Lender is the mortgagee under this Security Instrument/); assert.match(meSi.text, /non-MOM; MERS by Form 3749 assignment/); assert.equal(meSi.min_present, true);
  assert.match(meCi.text, /carries no MERS-as-original-mortgagee language \(B8-7-01/);
  const meQc = runDocumentQc(me, t, meSel, meDocs, upstream(me, meSel)); assert.equal(meQc.passed, true, meQc.hard_failures.join(",")); assert.equal(meQc.checks.find((c) => c.rule_code === "DQC_MIN_CHECK_DIGIT")!.result, "pass");
  const momMe = runDocumentQc(me, t, meSel, meDocs.map((d) => d.kind === "security_instrument" ? { ...d, text: `${d.text}\n${momNomineeParagraph(me)}`, mom_language_present: true } : d), upstream(me, meSel));
  const c = momMe.checks.find((x) => x.rule_code === "DQC_MIN_CHECK_DIGIT")!; assert.equal(c.result, "fail"); assert.match(c.reason!, /Maine security instrument has been modified to name MERS as the original mortgagee of record.*ineligible for delivery to Fannie Mae/); assert.deepEqual(momMe.hard_failures, ["DQC_MIN_CHECK_DIGIT"]); assert.equal(momMe.passed, false);
  // everywhere else the MOM paragraph is required: the Montana deed of trust carries it and a set rendered without it fails the same rule
  const mtSi = mtDocs.find((d) => d.kind === "security_instrument")!; assert.equal(mtSi.mom_language_present, true); assert.match(mtSi.text, /acting solely as a nominee for Lender/);
  assert.equal(runDocumentQc(mt, t, mtSel, mtDocs.map((d) => d.kind === "security_instrument" ? { ...d, text: d.text.replace(momNomineeParagraph(mt), ""), mom_language_present: false } : d), upstream(mt, mtSel)).checks.find((x) => x.rule_code === "DQC_MIN_CHECK_DIGIT")!.result, "fail");
...
test("26.1 worked figures: fixture note terms ($560,000.00 → P&I $3,402.62; $170.13 late charge; $2,858.33/$544.29 first split; −$1.00 residual), TX fee test ($6,450.00 ≤ $8,000.00; headroom $1,550.00), 2-1 buydown ($2,604.12 / $2,087.54 / $2,339.29; $516.58 + $264.83; $6,198.96 + $3,177.96 = $9,376.92; IPC cap $27,466.67) and the three authored notices", () => {
  ...
  const a = draftBuydownAgreement(s, s.buydown!, computeNoteTerms({ principal_cents: 41_200_000n, note_rate_pct: "6.500", term_months: 360, scheduled_disbursement_date: D("2026-11-18"), state: "OH" })); assert.equal(a.ipc_cap_cents, 2_746_667n, "$457,777.78 × 6% = $27,466.67 (B3-4.1-02: 75.01%–90% → 6%)"); assert.equal(a.ipc_counted_cents, 937_692n);
