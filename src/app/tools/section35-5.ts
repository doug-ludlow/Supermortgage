...
async function installmentsWrite(i: ToolInput, ctx: CommandContext): Promise<unknown> {
  ...
  const upb = has(i, "upb_cents") && ctx.actor.kind === "human" ? c(i["upb_cents"]) : c((await q.query<{ s: string | null }>(`SELECT sum(amount_cents)::text AS s FROM ledger_lines WHERE scope = 'loan' AND loan_id = $1 AND account = 'principal'`, [loanId]))[0]?.s ?? loan.original_upb_cents);
  ...
  try {
    ...
    void state;
    return { loan_id: loanId, run_id: plan.run_id, rows: plan.projection.rows.length, written: true, source, pi_cents: plan.pi_cents.toString(), first_due: plan.projection.rows[0]!.due_date, last_due: plan.projection.rows[plan.projection.rows.length - 1]!.due_date, total_interest_cents: plan.projection.total_interest_cents.toString(), total_principal_cents: plan.projection.total_principal_cents.toString(), maturity_variance_cents: plan.projection.maturity_variance_cents.toString(), hf005_difference_cents: plan.hf005.difference_cents.toString(), sha256: plan.projection.sha256, event_id: ev.id };
...
export async function installmentsReproject(i: ToolInput, ctx: CommandContext): Promise<unknown> {
  ...
  try {
    ...
    const r = await reprojectSchedule(q, ctx.events, ctx.actor, { loan_id: loanId, terms_id: termsId, effective_from: inputs.effective_from, note_rate_bps: inputs.note_rate_bps, pi_cents: inputs.pi_cents, escrow_cents: inputs.escrow_cents, upb_start_cents: has(i, "upb_start_cents") && hasRole(ctx.actor, ["officer"]) ? c(i["upb_start_cents"]) : null, source: inputs.source === "correction" ? "correction" : "reprojection", trigger_event_id: triggerId, maturity_date: D(String(terms.maturity_date ?? loan.maturity_date)) });
    return { loan_id: loanId, run_id: r.run_id, terms_id: termsId, terms_version: inputs.version, terms_source: inputs.source, effective_from: inputs.effective_from, rate_bps: inputs.note_rate_bps, pi_cents: inputs.pi_cents.toString(), rows_kept: r.rows_kept, rows_replaced: r.rows_replaced, sha256: r.sha256, reprojected: true, event_id: r.event.id, trigger_event_id: triggerId };
  } catch (e) {
...
export const TOOLS_35_5: readonly ToolDef[] = defineTools(CASHIERING_PROCESS, CASHIERING_AGENT_NAME, [
  { name: "installments.write", kind: "act", ruleSetVersion: RULE_SETS.schedule, guardrails: [NO_MONEY_FIELD], handler: compute(installmentsWrite),
    decision: (i, o, ctx) => { const r = out(o); return cashieringDecision({ subject: { kind: "installment_schedule_run", id: String(r["run_id"] ?? "") }, action: "installments.write", loan_id: str(i, "loan_id") || ctx.loanId, inputs: { source: r["source"] ?? str(i, "source") }, outputs: { rows: r["rows"], sha256: r["sha256"], maturity_variance_cents: r["maturity_variance_cents"] }, rule_set: RULE_SETS.schedule, rationale: r["written"] === true ? `schedule of ${String(r["rows"])} rows written from the boarded terms (rule 1)` : `no write: ${String(r["reason"])}` }); } },
  { name: "installments.reproject", kind: "act", ruleSetVersion: RULE_SETS.schedule, guardrails: [NO_MONEY_FIELD], handler: compute(installmentsReproject),
    decision: (i, o, ctx) => { const r = out(o); return cashieringDecision({ subject: { kind: "installment_schedule_run", id: String(r["run_id"] ?? "") }, action: "installments.reproject", loan_id: str(i, "loan_id") || ctx.loanId, inputs: { terms_id: r["terms_id"] ?? null, trigger_event_id: r["trigger_event_id"] ?? (str(i, "trigger_event_id") || null), effective_from: r["effective_from"] ?? null }, outputs: { rows_kept: r["rows_kept"], rows_replaced: r["rows_replaced"], sha256: r["sha256"] }, rule_set: RULE_SETS.schedule, rationale: r["reprojected"] === true ? `rows from ${String(r["effective_from"])} re-projected under ${String(r["terms_source"])} terms v${String(r["terms_version"] ?? "?")} (rule 3): ${String(r["rows_kept"])} kept, ${String(r["rows_replaced"])} replaced` : `no write: ${String(r["reason"])}` }); } },
