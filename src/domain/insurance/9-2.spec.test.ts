...
import { noticeChecklist, productionWindowOk, tierDeductible, premiumFromRate } from "./fpi.ts";
import { firstNoticeVariant, fpiCaseOpen, placementConfig } from "./ops.ts";
...
test("9.2-T5: Given a notice rendered with an extra marketing paragraph When checklist runs Then render fails ((c)(4)).", () => {
  const r = noticeChecklist([{ kind: "required" }, { kind: "account_number" }, { kind: "other" }]);
  assert.equal(r.ok, false); assert.deepEqual(r.violations, ["extra_content_(c)(4)"]);
...
test("9.2-T6: Given a windstorm-only gap When notice composed Then [Insurance Type]=\"windstorm\" and (v)(C) statement present.", () => {
  const v = firstNoticeVariant("insufficient", "wind");
  assert.equal(v.insurance_type, "windstorm"); assert.match(v.vC_statement!, /sufficient coverage for windstorm/); assert.equal(v.purchase_phrase, "will purchase");
  assert.equal(firstNoticeVariant("expired", "hazard").vC_statement, null);
});
test("9.2-T7: Given a notice produced 2026-10-01 and mailed 2026-10-09 (6 federal business days later; 2026-10-12 is Columbus Day but after) When mailing Then regeneration required per (d)(5) rule; produced 2026-10-05 \u2192 allowed.", () => {
  assert.equal(productionWindowOk(D("2026-10-01"), D("2026-10-09")), false);                          // 6 federal business days → regenerate
  assert.equal(productionWindowOk(D("2026-10-05"), D("2026-10-09")), true);
});
...
test("9.2-T9: Given flood required by FDPA lapses When case opened Then track `fdpa_flood`, no MS-3A.", () => {
  const c = fpiCaseOpen({ insurance_type: "flood", fdpa_required: true, opened_on: D("2027-02-04") });
  assert.equal(c.track, "fdpa_flood"); assert.equal(c.first_notice, "INS_FLOOD_FPI_NOTICE_45"); assert.equal(c.ms3a, false);
...
test("9.2-T10: Given LPI vendor offers a \"servicer expense reimbursement\" fee When placement configured Then rejected (B-6-01 commission exclusion).", () => {
  ...
  assert.equal(placementConfig({ vendor_id: "LPI-1", whitelist: ["LPI-1"], affiliate: true, fees: [] }).accepted, false);
});
