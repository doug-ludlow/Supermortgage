...
Everything `borrower-app` reads, writes and listens to. All names below are the build specs' names unless marked **UI-owned**. Claude Code implements the UI-owned tables in the `borrower-app` schema and the projections as read models in `api` over the existing tables; no other schema changes are permitted by this package.
...
| `status` | `badge` (01 §4 catalogue), `state_source` (the underlying state name and table), `one_liner` (copy key) | `applications.status`, `applications.disposition`, decision sub-status (O4.3), `locks.status`, `closings.status`, rescission state (O6.3), `fundings.status`, `loans.boarding_status`, servicing account states (08a) |
...
| `property` | `address`, `tbd: boolean`, `property_type`, `units`, `occupancy`, `valuation: {method, status, appointment_at?, value_used_cents?}`, `flood: {status}`, `hazard: {status}`, `project_review: {status}`, `hoa_dues_cents?` | `application_properties`, `valuation_orders`, `appraisals`, flood/hazard states (O5.5), `project_reviews` |
...
`{message_id, conversation_id, at, sender ∈ {borrower, agent, human, notice, system}, sender_label, channel ∈ {app, sms, email, voice, mail}, body_text?, card_instance_id?, subject, voice_turn: boolean, delivery: {sent, delivered, read}}`. Sources: **UI-owned** `conversations`/`messages` plus `lead_interactions` (O1.3) and `contacts` (4.x/11.x) written through the `borrower-comms` API. Notices appear as messages when `notices.sent_at` fires for a borrower-visible code.
...
### 1.6 UI-owned tables
- `conversations` `{conversation_id, party_id, created_at, locale, timezone}`
- `messages` `{message_id, conversation_id, at, sender, sender_ref, channel, body_text, card_instance_id?, subject_application_id?, subject_loan_id?, external_ref (twilio sid / email id)}` (append-only)
- `card_instances` `{card_instance_id, conversation_id, party_id, subject_*, kind, status, created_by, copy_key, props jsonb, evidence jsonb, command_ref?, expires_at, created_at, resolved_at}` (status transitions append to `card_instance_events`)
- `deep_links` `{token, party_id, target jsonb, expires_at, created_for_message_id}`
- `ui_events` (01 §9; append-only)
- `sessions` `{session_id, party_id, level ∈ {L1,L2,L3}, created_at, last_seen_at, passkey_id?, ip, user_agent}`
...
|---|---|---|---|---|
| `lead.start` | channel, utm, consumer_state | — | `lead.created`, `lead.interaction.started{channel, ai}` | O1.3 |
| `lead.acknowledgeAiDisclosure` | disclosure_version_id | — | `lead.disclosure.delivered`, `consent.ai_disclosure.acknowledged` | O1.3 |
| `party.authenticate` | method (otp_phone · otp_email · passkey), code | — | `lead.authenticated{level}` | O1.3 / UI |
| `party.startIdentity` / vendor webhook | vendor=stripe_identity | L1 | identity events → `SM_IDENTITY_IAL2_GATE` satisfied | O3.6 |
| `consent.capture` | kind, disclosure_version_id, method, text_hash, scope[] | method per kind (01 §3.5) | `consent.granted{kind}` → `consent.esign.pending|verified|active` (7.4) | 7.4, O1.3 |
| `credit.authorize` | kind ∈ {soft_pull, hard_pull}, text_hash, signature | L2 (soft; L1 when the consumer entered their own identity) / L1 (hard — no identity gate, 32.17 rule 18); `FCRA_1681B_A3_SOFT_PULL_PURPOSE_GATE` | `credit.authorization.captured{kind}`, then `credit.softpull.requested` / `credit.report.received` | O1.3, O3.2 |
| `application.confirmField` | path, value, source | ConfirmCard resolve | field write with `confirmed_at`; six-item detector runs → `application.received`, `application.trid_received` | O2.1, O2.2 |
| `application.setGoal` | transaction_type, occupancy, property (address or tbd), value_estimate?, loan_amount_sought? | — | `application.started` | O2.1 |
| `application.answerDeclarations` | declarations[13] (all false = "none apply") | — | `declarations` row | O2.1 |
| `application.answerDemographics` | per §1002.13 / App. B, may be `declined` | own party only | `applicant_demographics` (restricted) | O2.1 |
| `application.affirmJointIntent` | — | own party; before that party's credit order (`SM_O21_JOINT_INTENT_GATE`) | joint-intent artifact | O2.1 |
| `application.inviteParty` | role, contact | — | party + conversation created | O2.1 / UI |
| `verification.connect` | vendor (truv_income · plaid_assets · irs_ives · carrier_connect) | post-intent or `fee_paid_by=sm` | `verification.received{kind}` on webhook | O3.3, O3.4 |
| `document.upload` | document_class?, file | — | `document.classified` (O3.1) | O3.1 |
| `explanation.submit` | subject_ref, text, attestation | — | `documents{class=explanation_letter|inquiry_explanation}` | O3.1, O3.2 |
| `disclosure.acknowledgeReceipt` | disclosure_id | `consents{esign, active}` for the class | `disclosure.le.received` / `disclosure.cd.received` / `disclosure.companion.received` with `receipt_evidence=esign_confirmed` | O2.2, O2.3, O6.2 |
| `intent.record` | — | LE `received|deemed_received` (`intent_records.valid` computed) | `intent.to_proceed.received`; opens `REGZ_1026_19E2_INTENT_FEE_GATE` | O2.4 |
| `lock.request` | quote_id, period_days, float_down_elected? | `SM_O61_COMPLIANCE_PASS_LOCK_GATE`; `SM_QUOTE_VALIDITY_GATE` | `locks.requested → pending_mlo_approval` (`SM_LOCK_MLO_APPROVAL_SLA_30MIN`) → `lock.executed` (`REGZ_1026_19E3IVD_LOCK_REVISED_LE_3BD` opens) | O2.4 |
| `lock.requestExtension` | days | `locks.status=confirmed` | `lock.extended` | O2.4 |
| `counteroffer.respond` | decision ∈ {accept, decline} | `REGB_1002_9_COUNTEROFFER_90` running | `counteroffer_accepted` / `denied` path | O2.6 |
| `application.withdraw` | reason? | any non-terminal | `application.withdrawn` | O2.6 |
| `mi.selectPlan` | plan ∈ {bpmi_monthly, single, split, lpmi} | `mi_certificates.status=quoted` | `plan_selected` (LE revised) | O5.6 |
| `valuation.scheduleAccess` | slot | `valuation_orders.status=assigned` | `inspection_scheduled` | O5.1 |
| `rov.request` | comparables[], narrative | `appraisals.review_status=accepted`, before `FNMA_B4_1_3_12_ROV_CLOSING_GATE` | `rov.requested` | O5.2 |
| `insurance.submitEvidence` | policy doc / carrier connection | — | `insurance.evidence.received` | O5.5 |
| `closing.selectSlot` | slot, closing_type_preference | decision sub-status `clear_to_close`; `SM_O72_RON_STATE_AUTH_GATE` | `closing.scheduled` | O7.2 |
| `closing.captureEsignConsent` | — | `SM_O72_ESIGN_CONSENT_CLOSING_GATE` | `signing_sessions.consent_captured` | O7.2 |
| `rescission.exercise` | — | rescission `running` | `rescinded` path (H-8/H-9) | O6.3 |
...
| `offer.respond` | decision ∈ {yes, not_now, never} | `refi_opportunities.status=offered` | `lead.created`+conversion / `refi.opportunity.declined` / marketing consent revoked | O1.1, O1.2 |
| `refi.request` | — | serviced loan | `refi_opportunities.requested → offer_ready` | O1.1 |
| `human.request` | reason? | always | `human.transfer.requested` | 4.x, O1.3 |
...
| `credit.softpull.received` / `credit.report.received` | prequal / application numbers refresh; inquiry `ExplanationCard`s if O3.2 flags |
...
| `du.findings.received` (not shown), `decision.issued{conditional_approval|counteroffer|denial|noia}` | badge + `NoticeCard` with the O2.6 document |
...
- `applicant_demographics` is write-once from the borrower's own card and never read back into the UI (restricted table with access logging — O2.1).
...
- Retention of UI-owned tables follows the owning record's class (`sm_lead_36m` for pre-application conversations; `fnma_loan_file_life_plus_4y` once an application exists — O12.3).
...
POST /v1/borrower/documents                  → upload (multipart) → O3.1
...
`origination.ai_mlo_intake ∈ {assisted, autonomous}` (renders/omits `terms_review`, `pending_mlo`, `pending_mlo_approval` states) · `origination.preapproval_program` (default on) · `closing.enote_default` (O7.1) · `case.ai_path` · `theme ∈ {dark, light}` · `voice.in_app` · connector vendor toggles per O3 open questions · `jurisdiction_rules` (state copy: CO pre-use notice, UT/CA chatbot disclosure, TX 50(a)(6), NY CEMA, MA borrower-interest).
