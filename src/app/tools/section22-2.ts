...
/** The partner's recorded VantageScore 4.0 approval evidence (B3-5.1-01 09/02/2026 'eligible for use by approved lenders'): from the input or `applications.vantagescore_4_approval_ref`. */
const vs4Approval = (i: ToolInput, a: Record<string, unknown>): string | null => str(i, "vantagescore_4_approval_ref") || (typeof a.vantagescore_4_approval_ref === "string" && a.vantagescore_4_approval_ref ? a.vantagescore_4_approval_ref : null);
...
async function order(i: ToolInput, ctx: CommandContext, rt: ToolRuntime, order_type: OrderType, supersedes: CreditReport | null): Promise<Record<string, unknown>> {
  ...
  const o = validateOrder({ application_id: app, borrower_ids, order_type, score_model, app_score_model, requested_model_codes: (i.requested_model_codes as Partial<Record<Repository, string>> | undefined) ?? null, ...(Array.isArray(i.repositories) ? { repositories: i.repositories as Repository[] } : {}),
    permissible_purpose: str(i, "permissible_purpose") || supersedes?.permissible_purpose || "", certification_ref: str(i, "certification_ref") || supersedes?.certification_ref || "", borrower_authorization_ref: str(i, "borrower_authorization_ref"), subscriber_code: str(i, "subscriber_code"), vantagescore_4_approval_ref: vs4Approval(i, a),
...
export const TOOLS_22_2: readonly ToolDef[] = defineTools("22.2", "verification", refusing([
  ...
  { name: "orderRefresh", kind: "act", handler: compute(async (i, ctx, rt) => {
      ...
      const o = validateOrder({ application_id: app, borrower_ids: [...relied.borrower_ids], order_type: "soft_refresh", score_model: relied.score_model, app_score_model: relied.score_model, permissible_purpose: str(i, "permissible_purpose"), certification_ref: str(i, "certification_ref"), borrower_authorization_ref: str(i, "borrower_authorization_ref"), subscriber_code: str(i, "subscriber_code"), vantagescore_4_approval_ref: vs4Approval(i, appRecord(rt, app)), trid_received: true, fee_handled: true, ordering_agent: `${ctx.actor.kind}:${ctx.actor.id}`, attempt: Number(i.attempt ?? 1) });
