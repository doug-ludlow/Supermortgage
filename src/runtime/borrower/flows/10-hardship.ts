...
async function tick(deps: FlowDeps, nowIso: string): Promise<void> {
  // 11.1 / 13.1: the counter job over every delinquent loan (src/runtime/delinquency.ts) — once per loan-day; 35.3's `delinquency_counters` cycle (35.9's runner) is the same body and the whole-book pass yields to it once the registry projects the row (D13)
