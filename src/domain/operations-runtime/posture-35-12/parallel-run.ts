...
export function parseIncumbentFile(csv: string): IncumbentRow[] {
  ...
  if (lines.length < 1) throw new RangeError("the incumbent file needs a header row (one row per loan follows; a book of zero loans is a header alone — edge cases)");
  const header = lines[0]!.split(",").map((h) => h.trim().toLowerCase());
  const known = new Set(Object.values(COLS).flat()); const unknown = header.filter((h) => !known.has(h));
  if (unknown.length) throw new RangeError(`the incumbent file carries columns this process does not read (${unknown.join(", ")}); a trial balance holds loan numbers and figures only — no borrower name, SSN or address column (35.12: no borrower PII in any table of this process)`);
...
async function fileDocument(d: PostureDeps, run: RunRow, asOf: string, i: ReconcileInput): Promise<{ id: string; csv: string }> {
  ...
  parseIncumbentFile(csv);   // refuses an unknown column before the file is stored
...
export async function reconcileDay(d: PostureDeps, i: ReconcileInput): Promise<ReconcileResult> {
  ...
  d.deferWrite(async (q) => {
    ...
    await q.query(`INSERT INTO parallel_runs (parallel_run_id, environment, incumbent_servicer, opened_on, planned_end_on, loan_count, action, as_of_date, comparisons, matched, mismatched, mismatch_cents, incumbent_file_document_id, report_document_id, by, decision_id, loan_ids) VALUES ($1, $2, $3, $4::date, $5::date, $6, 'day_reconciled', $7::date, $8, $9, $10, $11, $12, $13, $14, $15, $16::uuid[])`,
      [r.parallel_run_id, r.environment, r.incumbent_servicer, r.opened_on, r.planned_end_on, r.loan_count, asOf, comparisons, matched, mismatched, mismatchCents, file.id, report.id, by, decision_id, r.loan_ids]);
    for (const o of opened) await q.query(`INSERT INTO parallel_run_diffs (diff_id, parallel_run_id, as_of_date, loan_id, field, ours, theirs, delta_cents, action, by, decision_id) VALUES ($1, $2, $3::date, $4, $5, $6, $7, $8, 'opened', $9, $10)`, [o.diff_id, r.parallel_run_id, asOf, o.loan_id || null, o.field, o.ours, o.theirs, o.delta_cents, by, decision_id]);
  });
...
export async function closeRun(d: PostureDeps, i: CloseInput): Promise<CloseResult> {
  ...
  d.deferWrite(async (q) => { const decision_id = await decisionFor(q, "parallel_run", r.parallel_run_id); await q.query(`INSERT INTO parallel_runs (parallel_run_id, environment, incumbent_servicer, opened_on, planned_end_on, loan_count, action, as_of_date, outcome, reason, by, decision_id, loan_ids) VALUES ($1, $2, $3, $4::date, $5::date, $6, 'closed', $7::date, $8, $9, $10, $11, $12::uuid[])`, [r.parallel_run_id, r.environment, r.incumbent_servicer, r.opened_on, r.planned_end_on, r.loan_count, closed_on, outcome, reason || null, by, decision_id, r.loan_ids]); });
