...
export const TOOLS_3_7: readonly ToolDef[] = defineTools("3.7", "escrow", [
  ...
  { name: "releaseDisbursement", kind: "act",
    // 35.7 rule 2: the second officer of PAYEE_CHANGE_DUAL (a payee change > $10,000.00 or a new payee) is an approval record by a distinct officer (roles.approve) — the surfaces inject `approvals` from it; the expression mirrors the guard below
    dualControl: { role: "officer", threshold: (i) => isRelease(i) && releaseApproval({ amount_cents: cents(i.amount_cents), payee_instruction_changed_on: i.payee_instruction_changed_on ? D(str(i, "payee_instruction_changed_on")) : null, release_on: i.release_on ? D(str(i, "release_on")) : D(new Date().toISOString().slice(0, 10)), new_payee: flag(i, "new_payee") }).dual_approval_required },
