...
import {
  anticipatedSalesProceeds, cashToClose, communitySecond, declareAsset, emdCheck, evaluateDeposit, ipcBandBps, largeDepositThreshold, lcorCashBack, lcorCures, miCoverageBps, monthlyPI, otherCosts, otherFinancedReserves, perDiemInterest, prepaidInterest, ratioMilliPct, reserveTolerance, reservesRequired, retirementUse, saleProceedsGate,
...
test("22.4-T3: (large deposit sourcing) Given qualifying income $8,200.00 and a $9,000.00 deposit on Sept 14, 2026 with $6,500.00 printed as a transfer from a verified brokerage account and $2,500.00 unexplained, when evaluated, then `unsourced_cents = 250,000 ≤ 410,000` → `large_deposit = false` and `usable_cents` is not reduced; given the whole $9,000.00 unexplained, then `usable_cents` falls by 900,000 cents and `funds_to_close.computed` shows shortfall $1,856.82 on the fixture worksheet.", async () => {
  ...
  // The single structured question per deposit goes through 22.1's needs list.
  ...
  // B3-4.2-02: where the borrower lacks the documentation, the written explanation is acceptable only with the lender's written rationale in the file — the explanation alone substantiates nothing (SM policy); with the rationale memo the deposit is `sourced` and both documents sit in source_evidence_document_ids
  const letterOnly = evaluateDeposit({ deposit_id: "dep-1", asset_id: "chk-a", posted_on: D("2026-09-14"), amount_cents: 900000n, description_on_statement: "DEPOSIT", sources: [{ cents: 900000n, kind: "sale_of_asset", written_explanation_document_id: "doc-lox-moto" }] }, { transaction: "purchase", threshold_cents: 410000n });
  assert.equal(letterOnly.unsourced_cents, 900000n); assert.equal(letterOnly.large_deposit, true); assert.equal(letterOnly.status, "unsourced"); assert.equal(letterOnly.reduction_cents, 900000n); assert.equal(letterOnly.explanation_with_rationale, false); assert.equal(letterOnly.unsubstantiated_source_cents, 900000n);
  const withRationale = evaluateDeposit({ deposit_id: "dep-1", asset_id: "chk-a", posted_on: D("2026-09-14"), amount_cents: 900000n, description_on_statement: "TRANSFER FROM SCHWAB …7781", sources: [{ cents: 650000n, kind: "transfer_verified_account", readily_identifiable: true }, { cents: 250000n, kind: "sale_of_asset", written_explanation_document_id: "doc-lox-moto", rationale_memo_document_id: "doc-rationale-dep-1", rationale: "title and purchaser unavailable; the $2,500.00 is 30% of monthly income, DTI 38% and a clean 24-month credit profile support the borrower's explanation (B3-4.2-02)" }] }, { transaction: "purchase", threshold_cents: 410000n });
  assert.equal(withRationale.unsourced_cents, 0n); assert.equal(withRationale.status, "sourced"); assert.equal(withRationale.explanation_with_rationale, true); assert.deepEqual(withRationale.source_evidence_document_ids, ["doc-lox-moto", "doc-rationale-dep-1"]); assert.match(withRationale.rationale!, /B3-4\.2-02/);
  const r2 = await h.run("evaluateDeposits", { asset_id: "chk-a", total_monthly_qualifying_income_cents: 820000n, deposits: [{ deposit_id: "dep-1", posted_on: "2026-09-14", amount_cents: 900000n, description_on_statement: "TRANSFER FROM SCHWAB …7781", sources: [{ cents: 650000n, kind: "transfer_verified_account", readily_identifiable: true }, { cents: 250000n, kind: "sale_of_asset", written_explanation_document_id: "doc-lox-moto", rationale_memo_document_id: "doc-rationale-dep-1", rationale: "reasonable judgment on the available documentation, DTI and credit profile (B3-4.2-02)" }] }] });
  assert.equal(r2.usable_cents, 2624018n); assert.equal((r2.deposits as DepositResult[])[0]!.status, "sourced"); assert.equal(h.ofType("asset.deposit.sourced").at(-1)!.payload.explanation_with_rationale, true);
});
...
test("22.4-T5: (IPC limits) Given price $457,800.00, appraised $460,000.00, CLTV 90.00% and seller-paid items of $5,000.00 plus an interested-party-funded buydown subsidy of $8,640.00, when tested, then `max_financing_concessions = 2,746,800 cents` and `ipc.limit.ok`; given a $30,000.00 seller credit, then excess $2,532.00 is reclassified, adjusted price $455,268.00, LTV 90.496% → 24.6 coverage 30%, band 3% ($13,658.04), and 21.5 receives a changed circumstance.", async () => {
  ...
  assert.equal(ok.ok, true); assert.equal(ok.max_financing_concessions_cents, 2746800n); assert.equal(ok.financing_concessions_cents, 1364000n); assert.equal(ok.band_bps, 600); assert.equal(ok.closing_cost_cap_cents, null); assert.equal(ok.binding_cap, "band");
  assert.equal(h.ofType("ipc.limit.ok").length, 1); assert.equal(h.timer("FNMA_B3_4_1_02_IPC_LIMIT_GATE")!.status, "satisfied");
  // B3-4.1-02 second cap: financing concessions ≤ the borrower's closing costs incl. prepaids, counting the items the interested party pays — J $8,712.00 + the $8,640.00 buydown = $17,352.00 ≥ $13,640.00, so the band maximum and ipc.limit.ok are unchanged
  const capped = await h.run("testIpcLimits", { sales_price_cents: 45780000n, appraised_value_cents: 46000000n, loan_amount_cents: 41200000n, cltv_milli_pct: 90000, borrower_closing_costs_cents: 871200n });
  assert.equal(capped.ok, true); assert.equal(capped.closing_cost_cap_cents, 1735200n); assert.equal(capped.max_financing_concessions_cents, 2746800n); assert.equal(capped.effective_limit_cents, 1735200n); assert.equal(capped.binding_cap, "closing_costs"); assert.equal(871200n + 864000n, 1735200n); assert.ok(1735200n >= 1364000n);
  // a $30,000.00 seller credit against $8,712.00 of closing costs (no buydown): the closing-cost cap binds before the band — $21,288.00 must be treated as a sales concession
  const overCosts = testIpcLimits({ sales_price_cents: 45780000n, appraised_value_cents: 46000000n, loan_amount_cents: 41200000n, occupancy: "principal_residence", cltv_milli_pct: 90000, borrower_closing_costs_cents: 871200n, items: [{ ipc_id: "ipc-3", application_id: PURCHASE.app, payer_party_id: "seller", payer_role: "seller", kind: "financing_concession", amount_cents: 3000000n, disclosed_on_settlement: true, counts_toward_limit: true, evidence_document_ids: [], status: "declared" } satisfies IpcItem] });
  assert.equal(overCosts.ok, false); assert.equal(overCosts.closing_cost_cap_cents, 871200n); assert.equal(overCosts.effective_limit_cents, 871200n); assert.equal(overCosts.excess_cents, 2128800n); assert.equal(overCosts.binding_cap, "closing_costs"); assert.equal(3000000n - 871200n, 2128800n);
  assert.equal(evaluateGate("22.4.ipcLimit", { sales_price_cents: 45780000n, appraised_value_cents: 46000000n, loan_amount_cents: 41200000n, borrower_closing_costs_cents: 871200n, items: overCosts.iterations.length ? [{ ipc_id: "ipc-3", application_id: PURCHASE.app, payer_party_id: "seller", payer_role: "seller", kind: "financing_concession", amount_cents: 3000000n, disclosed_on_settlement: true, counts_toward_limit: true, evidence_document_ids: [], status: "declared" }] : [] }).open, false);
...
test("22.4-T8: (reserves with other financed properties) Given the purchase fixture (1-unit principal residence) with two financed rentals (UPB $250,000.00 and $160,000.00) and DU reserves $8,000.00, when computed, then `reserves_required = 800,000 cents` and `other_financed_reserves = 0`; given the same borrowers on a second-home subject, then `reserves_required = 1,620,000 cents`; given verified reserves $14,580.00, then 23.1's 90% test passes (`tolerance_90pct_ok`) but `SM_RESERVES_VERIFIED_GATE` remains closed until $16,200.00 is verified; given $14,500.00, then `B3_2_10_RESERVES_90PCT` resubmission is required.", async () => {
  // B3-4.1-01: the other-financed-property add-on applies only when the subject is a second home or investment property — on the 1-unit principal-residence fixture reserves stay at DU's $8,000.00
  const principal = reservesRequired({ occupancy: "principal_residence", units: 1, transaction: "purchase", qualifying_pitia_cents: 300000n, du_reserves_required_cents: 800000n, other_financed_upb_cents: [25000000n, 16000000n] });
  assert.equal(principal.months, 0); assert.equal(principal.formula_cents, 0n); assert.equal(principal.other_financed_upb_cents, 41000000n); assert.equal(principal.financed_property_count, 3); assert.equal(principal.pct_bps, 0); assert.equal(principal.other_financed_cents, 0n); assert.equal(principal.other_financed_applies, false); assert.equal(principal.required_cents, 800000n); assert.equal(principal.basis, "du_findings"); assert.match(principal.other_financed_reason, /second home or investment property/);
  assert.equal(otherFinancedReserves({ occupancy: "principal_residence", financed_property_count: 3, other_financed_upb_cents: 41000000n }).cents, 0n);
  // the same borrowers on a second-home subject: 2 × PITIA $6,000.00 < DU $8,000.00 → base $8,000.00; 2% × $410,000.00 = $8,200.00 → $16,200.00
  const calc = reservesRequired({ occupancy: "second_home", units: 1, transaction: "purchase", qualifying_pitia_cents: 300000n, du_reserves_required_cents: 800000n, other_financed_upb_cents: [25000000n, 16000000n] });
  assert.equal(calc.months, 2); assert.equal(calc.formula_cents, 600000n); assert.equal(calc.other_financed_upb_cents, 41000000n); assert.equal(calc.financed_property_count, 3); assert.equal(calc.pct_bps, 200); assert.equal(calc.other_financed_cents, 820000n); assert.equal(calc.other_financed_applies, true); assert.equal(calc.required_cents, 1620000n); assert.equal(calc.basis, "b3_4_1_01_multiple_financed");
  assert.equal(reservesRequired({ occupancy: "investment", units: 1, transaction: "purchase", qualifying_pitia_cents: 300000n, du_reserves_required_cents: 800000n, other_financed_upb_cents: [25000000n, 16000000n] }).other_financed_cents, 820000n);
  ...
  // the purchase fixture (principal residence, harness default): no add-on — required $8,000.00, sufficient with $14,580.00 verified, so the gate is satisfied on the first run
  const hp = harness(PURCHASE, mst("2026-11-10", "09:00"));
  hp.upstream("du.findings.received", { submission_number: 3, final: true, reserves_required_cents: 800000n }, mst("2026-11-10", "09:00"), { kind: "agent", id: "underwriter" });
  const p = await hp.run("computeReserves", { ...inputs, verified_reserves_cents: 1458000n }); assert.equal(p.reserves_required_cents, 800000n); assert.equal(p.other_financed_cents, 0n); assert.equal(p.sufficient, true); assert.equal(p.gate_open, true); assert.equal(hp.timer("SM_RESERVES_VERIFIED_GATE")!.status, "satisfied");
  assert.equal(hp.ofType("reserves.computed").at(-1)!.payload.other_financed_applies, false); assert.equal(hp.ofType("reserves.shortfall").length, 0);
  // the second-home variant through the bus: $16,200.00 required
  const a = await h.run("computeReserves", { ...inputs, occupancy: "second_home", verified_reserves_cents: 1458000n });
  ...
  const b = await h.run("computeReserves", { ...inputs, occupancy: "second_home", verified_reserves_cents: 1620000n });
  ...
  const c = await h.run("computeReserves", { ...inputs, occupancy: "second_home", verified_reserves_cents: 1450000n });
...
test("22.4-T9: (Community Seconds CLTV) Given an OHFA-type DPA second of $13,734.00 on the purchase fixture, when computed, then CLTV 92.996% ≤ 105%, `SFC 118` is emitted, the IPC band becomes 3%, and 22.5 receives `deferred_5y_or_more = true`; given the provider is the property seller, then `subordinate_financing.declared{ineligible_provider}` and the loan is restructured.", async () => {
  ...
  // (412,000 + 13,734) ÷ 457,800 = 425,734 ÷ 457,800 = 0.929956… → 92.996 % ≤ 105 % (457,800 × 0.93 = 425,754 sits twenty dollars above the numerator)
  assert.equal(r.cltv_milli_pct, 92996); assert.equal(ratioMilliPct(41200000n + 1373400n, 45780000n), 92996); assert.equal(41200000n + 1373400n, 42573400n); assert.ok(42573400n < 45780000n * 93n / 100n); assert.equal(r.cltv_ok, true); assert.equal(r.cltv_cap_milli, 105000);
...
test("22.4 worked figures: purchase cash to close $43,997.00 = $45,800.00 + $8,712.00 − $5,000.00 − $5,000.00 − $515.00; closing funds $51,140.18 ($26,240.18 + $14,900.00 + $10,000.00), reserves $45,143.18 ≥ $8,000.00; shortfalls $2,856.82 / $1,856.82; IPC $27,468.00 / $13,640.00 / $2,532.00 / $455,268.00 / $13,658.04 / $16,341.96; reserves $8,200.00 → $16,200.00; LCOR $5,769.60 / $5,600.00 / $169.60 / $559,830.40 / $559,830.00 / $3,401.59; prepaid interest $935.47 and $1,785.48", () => {
  ...
  // R6: DU $8,000.00; rentals $250,000.00 + $160,000.00 = $410,000.00 × 2 % = $8,200.00 → $16,200.00 on a second-home subject (principal residence: no add-on, $8,000.00 — B3-4.1-01); $14,580.00 = 90.00 % (no resubmission; gate closed); $14,500.00 → resubmission.
  const rsv = reservesRequired({ occupancy: "second_home", units: 1, transaction: "purchase", qualifying_pitia_cents: 300000n, du_reserves_required_cents: 800000n, other_financed_upb_cents: [25000000n, 16000000n] });
  assert.equal(rsv.other_financed_upb_cents, 41000000n); assert.equal(rsv.other_financed_cents, 820000n); assert.equal(rsv.required_cents, 1620000n);
  assert.equal(reservesRequired({ occupancy: "principal_residence", units: 1, transaction: "purchase", qualifying_pitia_cents: 300000n, du_reserves_required_cents: 800000n, other_financed_upb_cents: [25000000n, 16000000n] }).required_cents, 800000n); assert.equal(reserveTolerance(1458000n, 1620000n).tolerance_90pct_ok, true); assert.equal(reserveTolerance(1450000n, 1620000n).tolerance_90pct_ok, false);
