...
export const EVALUATORS_35_12: Record<string, Evaluator> = {
  /** SM_PROD_GO_LIVE_ATTEST_GATE (not_before_gate): the attestation is possible only on or after `opened_on` + 28 calendar days (rule 9: 28 consecutive days; rule 10: GL-00 only after the gate's day). Facts: `opened_on`, `as_of_date` (the ET date of the attempt). */
  "35.12.goLiveGate": (f) => {
    const opened = s(f, "opened_on"); const asOf = s(f, "as_of_date");
    if (!opened || !asOf) return no("SM_PROD_GO_LIVE_ATTEST_GATE: opened_on and as_of_date are required");
    const notBefore = goLiveNotBefore(opened);
