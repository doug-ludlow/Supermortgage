...
export const TOOLS_29_1: readonly ToolDef[] = defineTools("29.1", "secondary", [
  ...
  { name: "prepareMandatoryPackage", kind: "act", handler: compute((i, ctx, rt) => {
      ...
      const r = svc.prepareMandatoryPackage({ amount_cents: cents(i.amount_cents), product_code: str(i, "product_code"), min_ptr: str(i, "min_ptr"), period_days: num(i, "period_days"), ...(optStr(i, "remittance_type") ? { remittance_type: str(i, "remittance_type") as RemittanceType } : {}), at: at(i, "at", ctx), flags: flagsOf(i, rt), officer_authorization: auth, hedge_request_id: optStr(i, "hedge_request_id"), ...(i.hbl_cap_pct === null ? { hbl_cap_pct: null } : typeof i.hbl_cap_pct === "string" || typeof i.hbl_cap_pct === "number" ? { hbl_cap_pct: String(i.hbl_cap_pct) } : {}) });   // absent → the C2-2-01 default (10); explicit null → no concentration limit (non-TBA product)
