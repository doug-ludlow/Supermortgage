 ...
 * through the REAL bus tools over the hosted runtime's HTTP surface, phase by phase, on a FixedClock: Columbus, OH (Franklin
 * County); an organic "still looking" lead (20.3 worked example 2: Thu Oct 15, 2026) with the price range and the down payment
 * as lead facts; the application opened Mon Oct 19 with the property to be determined (21.1 worked example 2: web channel, two
 * applicants, joint intent 18:52 / 20:31 EDT); the signed contract at 1187 Oakwood Ave arriving through 22.1 (ingest → classify
 * → FAKE extraction) and 32.2's `application.confirmField{purchase_contract}` — the address and the $457,800 price as the fifth
 * item, the $412,000 loan amount at 20:44 EDT as the sixth (`application.trid_received`; LE due Thu Oct 22); 20.4 worked example B
 * (HomeReady waiver → 6.375 %, P&I $2,570.34, lender credit $515.00); the LE Thu Oct 22; intent Fri Oct 23; the 30-day lock Mon
 * Oct 26 (expires Wed Nov 25 — 21.4 worked example 3); 22.x identity / credit (representative score 705) / assets; 24.1 the
 * traditional appraisal ordered Fri Oct 23 (after the intent — see the gaps below), received Thu Oct 29 at $460,000 → 24.2
 * `value_used = lower_of_two` $457,800 and the Reg B copy Fri Oct 30 (24 README INT-O5-3); 23.1 DU Mon Oct 26 (Approve/Eligible,
 * DTI 45.44 %, LTV 89.99) → 23.2 → 23.3 the conditional approval Mon Oct 26 (valid until Nov 25) and clear to close Fri Nov 6; 24.6 BPMI 25 % at 0.38 % → $130.47 and the
 * HPA dates Oct 1, 2034 / Dec 1, 2035 / Jan 1, 2042 (24.6 worked example 1); 24.4 the title commitment on the 2021 ALTA form, the
 * CPL and the wire verification; 26.2 the RON closing Wed Nov 18 10:05 ET; 25.2 CD v1 Thu Nov 12 e-delivered Fri Nov 13 →
 * earliest consummation Tue Nov 17 (worked example 4) and the seller's CD; 26.1 the OH purchase eNote set; 26.2 the eNote signed
 * 10:31 EST = consummation (no rescission on a purchase); 26.3 the wet-state funding the same day (13 × $71.96 = $935.48 prepaid
 * interest; net wire $410,339.52) → `loan.funded{2026-11-18}`; 30.2 boarding through POST /v1/applications/{id}/fund with the
 * purchase snapshot (note $412,000 at 6.375 %, MI active, escrow $615.00 + MI $130.47, deposit $1,240.00 — 26.3 worked example 5).
 *
 ...
 * the fixture stops short and the gap is named in the phase's doc comment (`GAP:`). Nothing here asserts the sections' figures —
 * purchase-lifecycle.test.ts does; a phase fails loudly on any non-200 answer. The application's borrowers carry contact e-mails
 * so the borrower API's one-time code links them to their parties; `linkParties` runs between the application row and 20.3's
 * conversion so the 32.x flows find the parties on `application.received`.
 *
 * GAPS — where the spec's purchase path has no tool (or the tool refuses the spec's timing) the fixture takes the nearest
 * spec-conforming path and says so here; nothing is faked past a gap:
 ...
 *      state-only row; `POST /v1/applications` takes the property only at creation). `isTbd` / the record's Property pane keep
 *      reading the TBD row; the test asserts the contract row and the six-item event instead (see signContract()).
 *   2. 21.1 `confirmPrefill{value}` for `property_address` does not stamp `intake.property_address` / `property_state` (the item is
 *      recorded with `source = borrower_confirmed_prefill`); the test asserts the item and notes the null.
 *   3. 24.1 `readDuOffer` cannot parse 23.1's FAKE findings (`value_acceptance_offer` is an object) — the spec's "DU offer first,
 *      then the method" order is unreachable, so the appraisal is ordered before DU (23.1 runs Mon Oct 26).
 *   4. 24.1 R2 refuses an SM-borne order before `intent.to_proceed.received` (SM_VALUATION_ORDER_BEFORE_ITP), so the order is
 *      Fri Oct 23 09:30 after the 09:14 intent, not the 24 README's Tue Oct 20.
 *   5. `purchase_contracts.document_id` has a foreign key to `documents` that only the upload route writes; a bus-ingested
 *      contract (22.1 ingestDocument with a non-uuid id) confirms with `document_id = null`.
 *   6. 26.3 wet-state purchase: the pre-signing wire / `disbursement_authorization` sequence of the spec is not reproduced —
 *      `authorizeFunding` needs `conditions.passed` including the executed package, so the fixture funds after consummation
 *      (same day) through the same conditions → advance → wire → agent receipt → `confirmDisbursement` path the refinance takes.
 *   7. 25.2 / 22.4 — the $8,712.00 total closing costs are decomposed illustratively (the owner's policy carries H's remainder);
 *      the spec gives only the totals ($43,997.00 cash to close).
 *   8. 29.4 / 30.1 delivery and purchase are not driven for the purchase journey (the refinance journey covers the same tools on
 *      the same loan-id grammar); the journey ends at 30.2 boarding and 30.4's hand-off.
 *   9. 20.4 worked example B quotes BPMI at 0.58 % (the quote's projected payment) while 24.6 / the CD carry the elected
 *      25 % plan at 0.38 % = $130.47; both figures are the spec's own and the fixture keeps each where its section puts it.
 */
...
export class PurchaseJourney {
  ...
  async eventsOf(where: string, params: unknown[]) { return this.o.db.query<{ type: string; loan_id: string | null; application_id: string | null; sequence: string; payload: Record<string, unknown> }>(`SELECT type, loan_id, application_id, sequence::text, payload FROM loan_events WHERE ${where} ORDER BY loan_events.sequence`, params); }   // the table column, not the text-cast output column of the same name (which would order lexicographically past 999 events)
  ...
  readonly A = "Casey Purchaser"; readonly B = "Riley Purchaser";
  ...
  /** Purchase, $412,000 on a $457,800 contract (LTV 89.995 → 90.00 %), score 705 Classic FICO, Franklin County OH, BPMI standard 25 %, HomeReady-eligible FTHB, 30-day lock, Purchase Ready Mon Dec 7, 2026. The 0.58 %/yr BPMI is 20.4's illustrative estimate; 24.6's insurer quote (0.38 %) is what the CD carries. */
  ...
  /** 23.2's purchase findings (Tue Oct 20): the catalog's verification messages the interpretation opens PTD conditions from — assets, reserves ($8,000.00 — 23.1 worked example 4), the EMD, the contract and the rest. */
  ...
  /** Validity components: credit 120 days from Oct 19 (Feb 16, 2027), the lock Wed Nov 25, the appraisal 4 months from its Oct 26 effective date, DU close-by Dec 7 → the approval is valid until Nov 25, 2026. */
  readonly VALIDITY = { credit_expires_at: "2027-02-16", lock_expires_at: "2026-11-25", valuation_expires_at: "2027-02-26", du_close_by_date: "2026-12-07" };
  ...
  /** Every item passes; MI evidenced by the 24.6 commitment, homeownership education by the HomeView certificate of Fri Oct 30 (23.2 rule 2). */
  readonly CTC_FACTS = () => Object.fromEntries(this.CTC_CODES.map((c) => [c, { status: "pass", ...(c === "CTC_INSURANCE_FLOOD" ? { evidence_ref: "doc-hoi-binder-1" } : c === "CTC_MI" ? { evidence_ref: this.miCertificateId } : c === "CTC_EDUCATION" ? { evidence_ref: "doc-homeview-cert-1" } : {}) }]));
  ...
  readonly MI_QUOTES = () => [{ mi_company_code: "06", plan: "bpmi_monthly", coverage_pct: 25, coverage_option: "standard", rate_bps: 38, renewal_type: "constant", refundable: false, quoted_at: EDT("2026-10-22", "11:00"), expires_at: EDT("2027-01-20", "11:00"), quote_id: `MIQ-06-${this.R}` }, { mi_company_code: "33", plan: "bpmi_monthly", coverage_pct: 25, coverage_option: "standard", rate_bps: 40, renewal_type: "constant", refundable: false, quoted_at: EDT("2026-10-22", "11:00"), expires_at: EDT("2027-01-20", "11:00"), quote_id: `MIQ-33-${this.R}` },
    { mi_company_code: "06", plan: "bpmi_monthly", coverage_pct: 12, coverage_option: "minimum", rate_bps: 30, renewal_type: "constant", refundable: false, quoted_at: EDT("2026-10-22", "11:00"), expires_at: EDT("2027-01-20", "11:00"), quote_id: `MIQ-06-min-${this.R}` }];
  ...
  /** 26.3 worked example 5's funding-condition facts (wet state, purchase — rescission not applicable, no payoffs): the executed package reviewed 11:40 ET Wed Nov 18, every other item passing or n/a. */
  ...
  async seedPricing(): Promise<void> {
    ...
    clock.set(EDT("2026-09-01", "12:00")); await this.tool({}, "20.4", "buildFeeItems", { op: "cost_schedule", cost_schedule_id: "cs-oh-purchase-traditional-2026-09", partner_id: this.PARTNER_ID, state: "OH", transaction_type: "purchase", valuation_method: "traditional", items: this.COST_ITEMS, effective_from: "2026-09-01" }, OFFICER);
    await this.publishSheet("2026-10-15");
  }
  /** The day's best-efforts sheet, 06:35 ET (20.4; the 21.4 pricing port reads it). */
  async publishSheet(date: string): Promise<void> { this.clock.set(ET(date, "06:35")); await this.tool({}, "20.4", "publishRateSheet", { rate_sheet_id: `rs-p-${date}`, partner_id: this.PARTNER_ID, source: "pe_whole_loan_api", published_at: ET(date, "06:35"), expires_at: ET(date, "17:00"), prices: this.PRICES }, PRICING); }
  /**
  ...
  async openLead(): Promise<string> {
    const { leadId, R } = this; const clock = this.clock; const scope = {};
    clock.set(EDT("2026-10-15", "11:05"));
    ...
    clock.set(EDT("2026-10-15", "11:05")); await this.tool(scope, "20.3", "deliverDisclosure", { op: "start", lead_id: leadId, interaction_id: `i-p-${R}`, channel: "web_chat", ai: true });
    await this.tool(scope, "20.3", "deliverDisclosure", { lead_id: leadId, interaction_id: `i-p-${R}`, notice_id: `n-disc-p-${R}` });
    clock.set(EDT("2026-10-15", "11:07")); await this.tool(scope, "20.3", "authenticate", { lead_id: leadId, method: "otp_email", evidence: { destination: this.o.borrowerEmail } });
    clock.set(EDT("2026-10-15", "11:08"));
    ...
    clock.set(EDT("2026-10-15", "11:12")); await this.tool(scope, "20.3", "captureConsent", { lead_id: leadId, kind: "credit_authorization", authorization_id: `auth-p-${R}`, authorization_kind: "soft_prequal", text_version: "soft-prequal-2026-09", channel: "web_chat", end_user: "partner", evidence: { ip: "203.0.113.9", user_agent: "fixture", session_id: `i-p-${R}` } });
    await this.tool(scope, "20.3", "orderSoftPull", { lead_id: leadId });
    clock.set(new Date("2026-10-15T11:12:30-04:00").toISOString()); await this.tool(scope, "20.3", "orderSoftPull", { lead_id: leadId, op: "receive", report_id: `rpt-p-${R}`, representative_score: 712 });
    return leadId;
  }
  /**
  ...
  async openApplication(): Promise<string> {
    const { leadId } = this; const clock = this.clock;
    clock.set(EDT("2026-10-19", "18:40"));
    ...
    clock.set(EDT("2026-10-19", "18:41"));
  ...
  async interview(): Promise<void> {
    ...
    clock.set(new Date("2026-10-19T18:41:05-04:00").toISOString()); await this.tool(scope, "21.1", "discloseAI", { session_id: `S-P-${R}`, utterance_id: `utt-p-${R}`, state: "OH" });
    clock.set(EDT("2026-10-19", "18:42")); await this.tool(scope, "21.1", "captureField", { field: "credit_request", transaction_type: "purchase", occupancy: "primary", property_state: "OH", identity_verified: true });
    clock.set(EDT("2026-10-19", "18:45")); await this.tool(scope, "21.1", "captureField", { field: "name", value: this.A, borrower_id: "B1" });
    clock.set(EDT("2026-10-19", "18:50")); await this.tool(scope, "21.1", "captureField", { field: "ssn", value: "123-45-4120", borrower_id: "B1" });
    clock.set(EDT("2026-10-19", "18:52")); await this.tool(scope, "21.1", "affirmJointIntent", { borrower_id: "B1", method: "web_checkbox", evidence_id: `ji-a-${R}` });
    clock.set(EDT("2026-10-19", "19:00")); await this.tool(scope, "21.1", "captureField", { field: "income", value: "820000", borrower_id: "B1" });
    ...
    clock.set(EDT("2026-10-19", "20:15")); await this.tool(scope, "21.1", "captureField", { field: "citizenship_status", value: "non_permanent_resident", borrower_id: "B2" });
    clock.set(EDT("2026-10-19", "20:31")); const ji = await this.tool(scope, "21.1", "affirmJointIntent", { borrower_id: "B2", method: "web_checkbox", evidence_id: `ji-b-${R}` });
    assert.equal(ji.output["all_borrowers"], true);
  }
  /**
   * 32.3 C1, Mon Oct 19 20:30–20:44 EDT: the signed contract arrives as a document (22.1 `ingestDocument{declared_class purchase_contract}` →
   * `classifyDocument` → `extractFields` by the FAKE extractor — the fields are the contract's own list) and the borrower confirms the extraction
   * through 32.2's `application.confirmField{path: purchase_contract}` (the `borrower-app` agent): `purchase_contracts` written, the $457,800 price
   * as the value item and 1187 Oakwood Ave as the fifth item (`purchase_contract.confirmed`); the $412,000 loan amount at 20:44 is the sixth →
   * `application.trid_received 2026-10-19 20:44 EDT`, LE due Thu Oct 22 (21.2 worked example 2b).
   *
   * NOTE (21.1): a confirmed extraction goes through 21.1 `confirmPrefill{value}` (32.2's `captureSix` for `source ≠ borrower`), which records the
   * six-item and its hash but — unlike `captureSixItem` / `offerPrefill` — does not stamp `property_address` / `property_state` on the intake
   * record; the item is submitted, the record's address stays null (the state was known from the lead).
   *
   * GAP (subject property row): no origination tool writes `application_properties` for a to-be-determined purchase once the contract names the
  ...
  async signContract(): Promise<void> {
    const scope = { app: this.appId }; const clock = this.clock; const R = this.R; const doc = this.contractDocumentId;
    clock.set(EDT("2026-10-19", "20:30"));
    ...
    clock.set(EDT("2026-10-19", "20:31"));
    ...
    clock.set(EDT("2026-10-19", "20:40"));
    ...
    clock.set(EDT("2026-10-19", "20:44")); const sixth = await this.tool(scope, "21.1", "captureField", { field: "loan_amount_sought", value: "41200000", borrower_id: "B1" });
  ...
  async verifyAndOrderCredit(): Promise<string> {
    const scope = { app: this.appId }; const clock = this.clock; const R = this.R;
    clock.set(EDT("2026-10-19", "20:50")); await this.tool(scope, "22.6", "verifyIdentity", { borrower_id: "B1", borrower_ids: ["B1", "B2"], scheduled_note_date: "2026-11-18" }, FRAUD_RISK);
    clock.set(EDT("2026-10-20", "09:00")); await this.tool(scope, "22.6", "verifyIdentity", { borrower_id: "B2", borrower_ids: ["B1", "B2"], scheduled_note_date: "2026-11-18" }, FRAUD_RISK);
    ...
    clock.set(EDT("2026-10-20", "09:02")); await this.tool(scope, "21.4", "checkFeeGate", { command: "order_credit_report", fee_kind: "credit_report", amount_cents: "7500", vendor_invoice_cents: "6850", op: "impose", fee_item_id: `fee-credit-report-p-${R}`, method: "card_token", checked_at: EDT("2026-10-20", "09:02") }, PRICING);
    clock.set(EDT("2026-10-20", "09:05"));
  ...
  /**
   * 22.1 / 22.4 (Tue Oct 20 → Mon Oct 26): the paystub and two bank statements, the declared assets (checking $31,240.18 / savings $14,900.00 /
   * 401(k) $38,000.00 — 22.4 R8 worked example 1), the $5,000.00 EMD as the seller's IPC of record and the R7 limit test (min(price, appraised)
   * × 3 % at 90 % CLTV = $13,734.00 ≥ $5,000.00), then the funds-to-close worksheet ($43,997.00 to close; $51,140.18 usable; reserves $45,143.18).
   */
  async assetsAndFundsToClose(): Promise<Record<string, unknown>> {
    const scope = { app: this.appId }; const clock = this.clock; const R = this.R;
    clock.set(EDT("2026-10-20", "10:30"));
    await this.tool(scope, "22.1", "ingestDocument", { document_id: `doc-pay-p-${R}`, source_channel: "borrower_upload", sha256: `sha-doc-pay-p-${R}`, subject_borrower_id: "B1", applicant_borrower_ids: ["B1", "B2"], page_count: 2 }, VERIFICATION);
    await this.tool(scope, "22.1", "classifyDocument", { document_id: `doc-pay-p-${R}`, doc_class: "paystub", confidence: 0.98 }, VERIFICATION);
    await this.tool(scope, "22.4", "declareAssets", { assets: [
      { asset_id: `chk-p-${R}`, asset_type: "checking", borrower_ids: ["B1"], declared_balance_cents: "3124018", institution_name: "Huntington Bank", account_last4: "4477", holder_names: [this.A] },
      { asset_id: `sav-p-${R}`, asset_type: "savings", borrower_ids: ["B1", "B2"], declared_balance_cents: "1490000", institution_name: "Huntington Bank", account_last4: "4485", holder_names: [this.A, this.B] },
      { asset_id: `ret-p-${R}`, asset_type: "retirement", borrower_ids: ["B1"], declared_balance_cents: "3800000", institution_name: "Fidelity", account_last4: "9012", holder_names: [this.A] }], borrower_names: [this.A, this.B] }, VERIFICATION);
    clock.set(EDT("2026-10-26", "14:00"));
    ...
    const ipc = await this.tool(scope, "22.4", "testIpcLimits", { sales_price_cents: "45780000", appraised_value_cents: "45780000", loan_amount_cents: "41200000", occupancy: "primary" }, VERIFICATION);
    assert.equal(ipc.output["ok"], true, JSON.stringify(ipc.output).slice(0, 400));
    const ws = await this.tool(scope, "22.4", "buildFundsToCloseWorksheet", { stage: "pre_cd", transaction: "purchase", sales_price_cents: "45780000", appraised_value_cents: "45780000", loan_amount_cents: "41200000", total_closing_costs_cents: "871200", down_payment_cents: "4580000", emd_cents: "500000", seller_credits_cents: "500000", lender_credit_premium_cents: "51500", reserves_required_cents: "800000",
      // 22.4-T6's fixture assets: the 401(k) is usable for both but must be liquidated to count at closing (none liquidated), so its $38,000.00 lands in reserves with the $7,143.18 left after closing.
      assets: [{ asset_id: `chk-p-${R}`, usable_cents: "2624018", usable_for: "both", status: "verified" }, { asset_id: `sav-p-${R}`, usable_cents: "1490000", usable_for: "both", status: "verified" }, { asset_id: `gift-p-${R}`, usable_cents: "1000000", usable_for: "both", status: "verified" }, { asset_id: `ret-p-${R}`, usable_cents: "3800000", usable_for: "both", status: "verified", liquidation_required_for_closing: true, liquidated_cents: "0" }] }, VERIFICATION);
    return ws.output;
  }
  /**
   * 24.1 / 24.2 (24 README INT-O5-3): no DU offer yet → traditional URAR (UAD 3.6) ordered Fri Oct 23 09:30 EDT with the AMC under the SM-borne
   * $650.00 fee (inside the Franklin County band), the appraiser assigned the same day (SM_APPRAISER_LICENSE_GATE), inspection Mon Oct 26,
   * the report Thu Oct 29 at $460,000 (effective Oct 26) → 24.2 ingests it, submits to UCDP (both GSEs successful, CU 1.9) and reviews it: accepted, `value_used = lower_of_two` = $457,800.00,
   * the Reg B copy package built and e-delivered Fri Oct 30 (≥ 3 business days before Nov 18).
   *
   * Two departures from 24 README's "ordered Tue Oct 20", both the tools' own: (a) 24.1 R2 (`valuation.order_before_itp = false`) refuses even an
   * SM-borne order before a documented intent to proceed, so the order follows Fri Oct 23's intent; (b) the order is placed before DU's findings
   * land because 24.1's `readDuOffer` reads `du.findings.received.value_acceptance_offer` as a string (value_acceptance | value_acceptance_pd |
   * none) while 23.1's FAKE findings carry `{offered, property_value_cents}` — an order placed after the findings throws in the parse (the refinance
   * journey orders before DU for the same reason). DU therefore runs Mon Oct 26 here (see duSubmitAndInterpret).
   */
  async appraisal(): Promise<void> {
    const scope = { app: this.appId }; const clock = this.clock; const R = this.R;
    clock.set(EDT("2026-10-23", "09:30"));
    ...
    clock.set(EDT("2026-10-23", "10:00")); const assigned = await this.tool(scope, "24.1", "verifyAppraiserLicense", { order_id: this.valuationOrderId, appraiser: this.APPRAISER, property_state: "OH", assigned_at: EDT("2026-10-23", "10:00") }, VALUATION);
    assert.equal(assigned.output["status"], "assigned", JSON.stringify(assigned.output));
    clock.set(EDT("2026-10-23", "10:30")); await this.tool(scope, "24.1", "scheduleInspection", { order_id: this.valuationOrderId, scheduled_at: EDT("2026-10-23", "10:30"), scheduled_for: EDT("2026-10-26", "14:00") }, VALUATION);
    clock.set(EDT("2026-10-26", "16:00")); await this.tool(scope, "24.1", "scheduleInspection", { op: "complete", order_id: this.valuationOrderId, completed_at: EDT("2026-10-26", "16:00") }, VALUATION);
    clock.set(EDT("2026-10-29", "14:20"));
    const rcv = await this.tool(scope, "24.1", "trackOrder", { op: "receive", order_id: this.valuationOrderId, report_document_id: `doc-appraisal-p-${R}`, uad_version: "3.6", effective_date: "2026-10-26", received_at: EDT("2026-10-29", "14:20"), lender_client_name: this.PARTNER_NAME, partner_name: this.PARTNER_NAME, fee_invoice_cents: "65000" }, VALUATION);
    assert.equal(rcv.output["accepted"], true, JSON.stringify(rcv.output).slice(0, 400));
    this.appraisalId = `APR-P-${R}`;
    const ingested = await this.tool(scope, "24.2", "ingestReport", { appraisal_id: this.appraisalId, version_no: 1, package: this.PACKAGE, appraised_value_cents: "46000000", effective_date: "2026-10-26", appraiser_party_id: this.APPRAISER.party_id, valuation_order_id: this.valuationOrderId, received_at: EDT("2026-10-29", "14:20"), pdf_document_id: `doc-appraisal-p-${R}` }, VALUATION);
    ...
    clock.set(EDT("2026-10-29", "15:05")); await this.tool(scope, "24.2", "submitUcdp", { appraisal_id: this.appraisalId, version_no: 1, package_hash: `sha256:appraisal-p-${R}-v1` }, VALUATION);
    clock.set(EDT("2026-10-29", "15:07")); const polled = await this.tool(scope, "24.2", "pollFindings", { appraisal_id: this.appraisalId, version_no: 1 }, VALUATION);
    ...
    clock.set(EDT("2026-10-29", "16:00"));
    ...
    clock.set(EDT("2026-10-30", "09:00"));
  ...
  /** 23.1 DU (casefile → credit association → request → submission → findings Approve/Eligible) and 23.2's interpretation 12 minutes later (SM_DU_CONDITIONS_SLA_4H) — Mon Oct 26 09:00 EDT here, after the appraisal order (see appraisal()); the spec's Tue Oct 20 is two business days earlier. */
  ...
  async conditionalApproval(du: { submission_id: string; interpretation_id: string | null; request_hash: string }): Promise<void> {
    const scope = { app: this.appId }; this.clock.set(EDT("2026-10-26", "11:00"));
  ...
  /** 23.3 Fri Nov 6 15:00 EST (28.1: PTD cleared Fri Nov 6): the CTC checklist and clear to close. */
  async clearToClose(): Promise<void> {
    const scope = { app: this.appId }; this.clock.set(EST("2026-11-06", "15:00"));
  ...
  /** 20.4 worked example B on the Tue Oct 20 sheet (09:10 EDT): HomeReady waiver → 6.375 %, P&I $2,570.34, lender credit $515.00; then 21.2's H-24 (with the $130.47 BPMI) — a caller that delivers the LE its own way stops here. */
  async quoteOnly(): Promise<void> {
    const scope = { app: this.appId }; const clock = this.clock;
    await this.publishSheet("2026-10-20");
    clock.set(EDT("2026-10-20", "09:10")); const q = await this.tool(scope, "20.4", "solvePassThrough", { inputs: this.QUOTE_INPUTS, quote_id: this.QUOTE, purpose: "lead_quote", partner_id: this.PARTNER_ID, lead_id: this.leadId }, PRICING);
    ...
    clock.set(EDT("2026-10-22", "10:00")); const h24 = await this.tool(scope, "21.2", "renderH24", this.LE_RENDER(), DISCLOSURE); this.leDataHash = h24.output["data_hash"] as string;
  }
  /** The quote, the H-24 and the LE delivered Thu Oct 22 10:00 EDT through the runtime's 21.2 bridge (MLO-approved; e-signed 12:30 → effective receipt Oct 22, inside the Thu Oct 22 deadline). */
  ...
  async recordIntent(): Promise<void> {
    this.clock.set(EDT("2026-10-23", "09:14"));
  ...
  async quoteForLock(): Promise<string> {
    await this.publishSheet("2026-10-26"); this.clock.set(EDT("2026-10-26", "10:05"));
  ...
  async executeLockAndCommit(): Promise<void> {
    const scope = { app: this.appId }; this.clock.set(EDT("2026-10-26", "10:19"));
    ...
    const c = await this.tool(scope, "21.4", "requestCommitment", { lock_id: this.lockId, at: EDT("2026-10-26", "10:20") }, PRICING); this.commitmentId = c.output["commitment_id"] as string; this.commitmentExpiresOn = String(c.output["expires_on"] ?? "");
  }
  /**
   * 24.6 worked example 1 (first half): two insurers' quotes Thu Oct 22 (standard 25 % at 0.38 %/yr and the 12 % minimum at 0.30 % with its
   * $1,545.00 LLPA), the borrower elects monthly BPMI at standard coverage → $130.47 (`mi_certificates{plan_selected}`; LTV 89.99 → 90 %).
   */
  async miQuotesAndElection(): Promise<void> {
    const scope = { app: this.appId }; const clock = this.clock;
    clock.set(EDT("2026-10-22", "11:00"));
  ...
  /**
   * 24.6 worked example 1 (second half): the delegated order Mon Oct 26 (DU Approve/Eligible), the commitment Tue Oct 27 with the certificate
   * number; Fri Oct 30 the initial amortization schedule from the final note terms (P&I $2,570.34), the HPA dates (cancellation Oct 1, 2034 /
   * termination Dec 1, 2035 / midpoint Jan 1, 2042), the terms verification and the §4903 initial disclosure → `docs_ready`.
   */
  async miOrderAndDisclosure(): Promise<{ cancellation_date: string; termination_date: string; midpoint_termination_date: string; pi_cents: string }> {
    const scope = { app: this.appId }; const clock = this.clock; const R = this.R;
    clock.set(EDT("2026-10-26", "11:30")); await this.tool(scope, "24.6", "submitMiOrder", { order_type: "delegated", du_reliance: true, du_casefile_id: this.casefileId, du_recommendation: "Approve/Eligible" }, CLOSER);
    clock.set(EDT("2026-10-27", "09:00")); this.miCertificateNumber = `MGIC-${R}`;
    ...
    clock.set(EDT("2026-10-30", "10:00"));
  ...
  /**
   * 24.4 (24 README INT-O5-3: "title on the 2021 ALTA form, seller-side judgments cleared Nov 10, CPL/wire verification Mon Nov 16"): the agent
   * vetted Mon Nov 2, the commitment ordered the same day, the commitment received Tue Nov 10 (2021 ALTA Loan Policy, ALTA 8.1 committed, no
   * blocking exceptions → `reviewed`), the settlement agent's wire instructions verified Tue Nov 10 (vendor match + ALTA Registry callback),
   * the CPL naming the partner and Supermortgage received Mon Nov 16; the consummation gates evaluated.
   */
  async title(): Promise<void> {
    const scope = { app: this.appId }; const clock = this.clock; const R = this.R;
    clock.set(EST("2026-11-02", "09:00"));
    ...
    clock.set(EST("2026-11-16", "11:00"));
  ...
  async scheduleClosing(): Promise<void> {
    this.clock.set(EST("2026-11-09", "10:00"));
   ...
   * the same day → `earliest_consummation_date = 2026-11-17` (Sat 14, Mon 16, Tue 17); the seller's CD from the settlement agent on the day of
   * consummation (REGZ_1026_19F4_SELLER_CD_GATE). `deliveries: false` stops after `renderCd`.
   */
  async closingDisclosure(opts: { receipts?: boolean; deliveries?: boolean } = {}): Promise<string> {
    const scope = { app: this.appId }; const clock = this.clock; const R = this.R;
    clock.set(EST("2026-11-12", "10:00"));
    ...
    await this.tool(scope, "25.2", "renderCd", { disclosure_id: this.cdDisclosureId, cd_version: 1, transaction_type: "purchase", state: "OH", required_consumer_ids: ["B1", "B2"],
      ...
      parties: { borrowers: [this.A, this.B], seller_name: "Morgan Vendor", creditor_name: "Partner Bank, N.A.", creditor_nmlsr_id: "123456", mlo_name: "Ada Okonkwo", mlo_nmlsr_id: "987654", settlement_agent_name: "Buckeye Title Agency LLC", settlement_agent_license_id: "OH-TA-2210" },
    ...
    clock.set(EST("2026-11-13", "09:14"));
    for (const consumer of ["B1", "B2"]) {
      await this.tool(scope, "25.2", "deliverDisclosure", { disclosure_id: this.cdDisclosureId, consumer_id: consumer, channel: "esign_portal", at: EST("2026-11-13", "09:14"), esign_consent_id: `ESIGN-P-${consumer}`, ...(consumer === "B1" ? { gate_run: { run_id: "RUN-CD-P-1", open: true, apr_verdict: "pass", blocked_channels: [] } } : {}) }, DISCLOSURE);
      if (opts.receipts !== false) await this.tool(scope, "25.2", "recordReceipt", { disclosure_id: this.cdDisclosureId, consumer_id: consumer, evidence: "esign_confirmed", at: EST("2026-11-13", "09:30"), evidence_document_id: `DOC-ESIGN-P-${consumer}` }, DISCLOSURE);
    }
    if (opts.receipts !== false) { const wp = await this.tool(scope, "25.2", "computeEarliestConsummation", { disclosure_id: this.cdDisclosureId }, DISCLOSURE); assert.equal(wp.output["earliest_consummation_date"], "2026-11-17"); }
    return this.cdDisclosureId;
  }
  /** REGZ_1026_19F4_SELLER_CD_GATE: the settlement agent's seller CD and the creditor's copy, received the day of consummation. */
  async sellerCd(): Promise<void> {
    this.clock.set(EST("2026-11-18", "09:00"));
    await this.tool({ app: this.appId }, "25.2", "scheduleCorrectedCd", { op: "seller_cd", document_id: `DOC-SELLER-CD-${this.R}`, received_at: EST("2026-11-18", "09:00"), provided_by: "settlement_agent" }, DISCLOSURE);
  }
  /**
   * 26.1 Mon Nov 16 (26 README INT-O7-3: "documents released Mon Nov 16"): the note terms for a Nov 18 disbursement (P&I $2,570.34; first payment Jan 1,
   * 2027; maturity Dec 1, 2056), the doc-gen gates, the closing snapshot, the OH purchase eNote set (3200e, the Ohio mortgage 3036, the final 1003 —
   * no H-8: not rescindable), the SMART Doc, QC, the release to the settlement agent — then 26.2's RON session Wed Nov 18 (26.2 worked example 2's
   * clock): 10:05 open, both signers proofed (credential analysis + KBA), the eNote signed 10:29 / 10:31 = `closing.consummated{note_date
   * 2026-11-18}`, the mortgage 10:40 and acknowledged 10:42, the Authoritative Copy tamper-sealed 10:44 and registered 10:52 (due Thu Nov 19).
   */
  async closeAndSign(opts: { snapshot?: Record<string, unknown> } = {}): Promise<void> {
    ...
    clock.set(EST("2026-11-16", "10:00"));
    ...
    clock.set(EST("2026-11-18", "09:30"));
    ...
    clock.set(EST("2026-11-18", "10:05"));
    ...
    for (const [party, hhmm, correct] of [["B1", "10:08", 5], ["B2", "10:12", 4]] as const) { clock.set(EST("2026-11-18", hhmm)); await this.tool(scope, "26.2", "monitorSession", { op: "identity", closing_id: this.CLOSING_ID, party_id: party, method: "credential_analysis_kba", credential_type: "driver_license", credential_analysis_result: "pass", kba_attempts: [{ questions: 5, correct, seconds: 68, at: EST("2026-11-18", hhmm), notary_party_id: this.NOTARY.party_id }], notary_party_id: this.NOTARY.party_id, vendor: "Proof" }, CLOSER); }
    clock.set(EST("2026-11-18", "10:15"));
    ...
    for (const [party, hhmm] of [["B1", "10:20"], ["B2", "10:22"]] as const) await this.tool(scope, "26.2", "monitorSession", { op: "sign", closing_id: this.CLOSING_ID, closing_document_id: `DOC-1003-P-${R}`, kind: "final_1003", signer_party_id: party, signed_at: EST("2026-11-18", hhmm), signature_method: "esign_ron", required_note_signers: ["B1", "B2"] }, CLOSER);
    await this.tool(scope, "26.2", "monitorSession", { op: "sign", closing_id: this.CLOSING_ID, closing_document_id: `DOC-ENOTE-P-${R}`, kind: "enote", signer_party_id: "B1", signed_at: EST("2026-11-18", "10:29"), signature_method: "esign_ron", required_note_signers: ["B1", "B2"] }, CLOSER);
    const signed = await this.tool(scope, "26.2", "monitorSession", { op: "sign", closing_id: this.CLOSING_ID, closing_document_id: `DOC-ENOTE-P-${R}`, kind: "enote", signer_party_id: "B2", signed_at: EST("2026-11-18", "10:31"), signature_method: "esign_ron", required_note_signers: ["B1", "B2"] }, CLOSER);
    ...
    clock.set(EST("2026-11-18", "10:44")); await this.tool(scope, "26.2", "validateAuthoritativeCopy", { op: "seal", closing_id: this.CLOSING_ID, seal_hash: seal, signing_completed_at: EST("2026-11-18", "10:31"), authoritative_copy_ref: `EV-P-${R}`, tamper_sealed_at: EST("2026-11-18", "10:44") }, CLOSER);
    clock.set(EST("2026-11-18", "10:46")); const v = await this.tool(scope, "26.2", "validateAuthoritativeCopy", { closing_id: this.CLOSING_ID, authoritative_copy: copy }, CLOSER); assert.equal(v.output["gate_open"], true, String(v.output["reason"]));
    clock.set(EST("2026-11-18", "10:52")); const reg = await this.tool(scope, "26.2", "registerENote", { closing_id: this.CLOSING_ID }, CLOSER); assert.equal(reg.output["accepted"], true);
  }
  /**
   * 26.3 worked example 5 (wet state, purchase, Wed Nov 18): the funding opened 08:00 ET on the closing date (not rescindable → funds on the
   * note date; `funding_type = wet`, per diem 412,000 × 6.375 % ÷ 365 = $71.96, prepaid Nov 18–30 = 13 days = $935.48, first payment Jan 1,
   * 2027, LPI Dec 1), the worksheet (gross − prepaid interest − $1,240.00 escrow deposit + $515.00 lender credit = $410,339.52) reconciled to
   * the settlement statement; after the 11:40 execution review every condition passes (rescission and payoffs n/a on a purchase), the funding
   * is authorized 11:52, the advance approved, the wire prepared / released under dual control / accepted (IMAD), the agency confirms receipt,
   * and the agency's disbursement 13:15 → `loan.funded{disbursement_date 2026-11-18}`.
   *
   * Timing note (not a gap): 26.3's wet-state rule releases the wire BEFORE the session on the pre-signing subset (SM_O73_WET_FUNDS_AT_TABLE_GATE,
   * 08:55 ET in the worked example) and issues the disbursement authorization after the execution review; the tools carry both (`funding.stage`,
   * `notifySettlementAgent{op: disbursement_authorization}`), but `authorizeFunding` requires `conditions.passed` — every item passing, the executed
   * package included — so this fixture runs the whole 26.3 sequence after the 11:40 review on the same day; the money figures are the example's.
   */
  ...
  async openFunding(): Promise<void> {
    ...
    clock.set(EST("2026-11-18", "08:00"));
  ...
  async disburse(): Promise<void> {
    ...
    clock.set(EST("2026-11-18", "11:40")); const conditions = await this.tool(scope, "26.3", "evaluateFundingConditions", { funding_id: F, facts: this.FUNDING_FACTS(EST("2026-11-18", "11:40")) }, FUNDER); assert.equal(conditions.output["passed"], true, JSON.stringify({ blocking: conditions.output["blocking_codes"], pending: conditions.output["pending_codes"] }));
    clock.set(EST("2026-11-18", "11:52")); await this.tool(scope, "26.3", "requestWarehouseAdvance", { funding_id: F, conditions: conditions.output, rescission: { status: "not_applicable" }, fraud_hold: { fraud_hold: false }, ptf: { ptf_cleared: true }, cash_to_close: { worksheet: { reconciled_to_cd: true, sufficient: true } }, gifts: [] }, FUNDER);
    await this.tool(scope, "26.3", "requestWarehouseAdvance", { funding_id: F, op: "advance_approved", advance_id: `ADV-P-${R}` }, FUNDER);
    clock.set(EST("2026-11-18", "12:00")); const wireId = `W-P-${R}`;
    ...
    clock.set(EST("2026-11-18", "12:10")); await this.tool(scope, "26.3", "prepareWire", { funding_id: F, op: "release", wire_id: wireId, bank_ref: "BK-P-1", released_at: EST("2026-11-18", "12:10") }, APPROVER);
    await this.tool(scope, "26.3", "prepareWire", { funding_id: F, op: "accept", wire_id: wireId, imad: "20261118B1QGC01R000412", accepted_at: EST("2026-11-18", "12:11") }, FUNDER);
    clock.set(EST("2026-11-18", "12:30")); await this.tool(scope, "26.3", "notifySettlementAgent", { funding_id: F, op: "agent_receipt", funds_received_by_agent_at: EST("2026-11-18", "12:30") }, FUNDER);
    clock.set(EST("2026-11-18", "13:15"));
  ...
  /** 24.6 R10 / SM_MI_ACTIVATE_1BD: activation requested at the note date (Nov 18 13:30), the insurer confirms Thu Nov 19 with the effective date Nov 18 → `active` (30.2's OB-009 reads it). */
  async activateMi(): Promise<Record<string, unknown>> {
    const scope = { app: this.appId }; const clock = this.clock;
    clock.set(EST("2026-11-18", "13:30")); await this.tool(scope, "24.6", "requestActivation", { note_date: "2026-11-18", at: EST("2026-11-18", "13:30") }, CLOSER);
    clock.set(EST("2026-11-19", "09:00")); const active = await this.tool(scope, "24.6", "requestActivation", { op: "confirm", activation_effective_date: "2026-11-18", certificate_number: this.miCertificateNumber, confirmed_at: EST("2026-11-19", "09:00") }, CLOSER);
  ...
  /**
   * 30.2 worked example 2: POST /v1/applications/{id}/fund boards ONE servicing loan from the record — 26.3's `loan.funded` on the log, 26.1's
   * note hash, 26.2's consummation and MIN — with the purchase snapshot for what the record does not carry: the OH note ($412,000 at 6.375 %,
   * the late-charge terms 26.1 computed), the final CD (P&I $2,570.34; escrow $615.00 + MI $130.47; deposit $1,240.00; 13 days' prepaid interest),
   * 30.3's initial analysis (county taxes $520.00 + hazard $95.00 = $615.00/month; deposit $1,240.00), LTV 90 %, the active MI certificate
   * (coverage 25 %, monthly BPMI, the §4903 initial disclosure), the Columbus property (appraised $460,000; original value $457,800), the hazard
   * binder effective Nov 18. `snapshotOverrides` replace whole top-level fields (30.2's correction rule).
   */
  async board(snapshotOverrides: Record<string, unknown> = {}): Promise<string> {
    this.clock.set("2026-11-18T20:20:00.000Z");   // 15:20 EST Wed Nov 18 — 30.2 worked example 2's `loan.funded` hour
  ...
  async openServicingHandoff(): Promise<void> {
    this.clock.set("2026-11-18T20:25:00.000Z");
  ...
  /** Every phase in order — the whole purchase to boarding (the test settles the flows between them). */
  async phases(): Promise<{ name: string; run: () => Promise<unknown> }[]> {
    ...
    return [
      ...
      { name: "verifyAndOrderCredit", run: () => this.verifyAndOrderCredit() }, { name: "quoteAndLe", run: () => this.quoteAndLe() }, { name: "miQuotesAndElection", run: () => this.miQuotesAndElection() }, { name: "recordIntent", run: () => this.recordIntent() }, { name: "appraisal", run: () => this.appraisal() },
      { name: "quoteForLock", run: () => this.quoteForLock() }, { name: "requestLock", run: () => this.requestLock() }, { name: "executeLockAndCommit", run: () => this.executeLockAndCommit() }, { name: "duSubmitAndInterpret", run: async () => { du = await this.duSubmitAndInterpret(); return du; } },
      { name: "conditionalApproval", run: () => this.conditionalApproval(du!) }, { name: "miOrderAndDisclosure", run: () => this.miOrderAndDisclosure() }, { name: "assetsAndFundsToClose", run: () => this.assetsAndFundsToClose() }, { name: "title", run: () => this.title() }, { name: "clearToClose", run: () => this.clearToClose() }, { name: "scheduleClosing", run: () => this.scheduleClosing() },
      { name: "closingDisclosure", run: () => this.closingDisclosure() }, { name: "closeAndSign", run: () => this.closeAndSign() }, { name: "sellerCd", run: () => this.sellerCd() }, { name: "fund", run: () => this.fund() }, { name: "activateMi", run: () => this.activateMi() }, { name: "board", run: () => this.board() }, { name: "openServicingHandoff", run: () => this.openServicingHandoff() },
    ];
  }
}
