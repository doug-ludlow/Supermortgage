...
export const TOOLS_32_2: readonly ToolDef[] = defineTools(PROCESS, BORROWER_APP, [
  ...
  cmd("consent.capture", "write", async (i, ctx, rt) => {
    ...
    never("AFFIRMATION_METHOD", "01 §3.5 as relaxed by 32.17 rule 20 (2026-09-13): esign / credit_authorization / autodraft_authorization / tcpa are affirmed by checkbox_with_text + typed name on a ConsentCard, or by single_tap on a card that shows the statement (the goal, Show me my rate); ai_disclosure_ack a single tap", (i) => ["esign", "credit_authorization", "autodraft_authorization", "tcpa_voice", "tcpa_sms"].includes(str(i, "kind")) && str(i, "method") !== "" && !["checkbox_with_text", "single_tap"].includes(str(i, "method")), "this consent kind is affirmed by checkbox_with_text and a typed name, or by one tap on the statement")] }),
  ...
  }, { guardrails: [never("LEVEL_REQUIRED", "02 §2 credit.authorize: L2 (soft) / L1 (hard — 32.17 rule 18: the borrower's authorization and a session, no identity gate); 32.14 DELTA-13 / 20.3 rule 2: L1 for a soft pull with the consumer's own entry of name, address, DOB and SSN (`consumer_entered_identity`, stated by the API)", (i) => str(i, "kind") === "hard_pull" ? !levelAtLeast(i, "L1") : !(levelAtLeast(i, "L2") || (i.consumer_entered_identity === true && levelAtLeast(i, "L1"))), "a soft pull needs an L2 session (or L1 with the consumer's own identity entry) and a hard pull a session"),
  ...
  cmd("application.setGoal", "write", async (i, ctx, rt) => {
    ...
    // 32.16 §2.0: an organic application opened at the account door carries the door's placeholder (limited_cash_out / primary, 0057's NOT NULL enums) — the goal the borrower tapped is the real one; the row follows 21.1's record so every reader of `applications.transaction_type` (the flows' purchase path, the Record, boarding) sees the same fact
    { const tt = str(i, "transaction_type"); const occ = str(i, "occupancy"); if (["purchase", "limited_cash_out", "cash_out"].includes(tt) && ["primary", "second_home", "investment"].includes(occ)) defer(rt, async (q) => { await q.query(`UPDATE applications SET transaction_type = $2::transaction_type, occupancy = $3::occupancy_type WHERE id = $1 AND channel = 'organic' AND loan_id IS NULL`, [application_id, tt, occ]); }); }
