...
const p39: ToolDef[] = defineTools("3.9", "escrow", [
  { name: "evaluateInterestEligibility", kind: "act", handler: compute((i) => { if (!i.facts) throw new RangeError("evaluateInterestEligibility needs facts {state, origination_date, …}"); const ex = ioeExemption(i.facts as Parameters<typeof ioeExemption>[0]); return { eligible: ex === null, exemption: ex }; }) },
