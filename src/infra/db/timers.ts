...
interface TimerRow extends Record<string, unknown> {
  id: string; code: string; subject_kind: string; subject_id: string; loan_id: string | null; armed_at: string; armed_by_event_id: string; anchor_date: string;
...
function rowToInstance(r: TimerRow): TimerInstance {
  return {
    ...
    ...(r.loan_id ? { loanId: r.loan_id } : {}), ...(r.due_date ? { dueDate: r.due_date as PlainDate } : {}), ...(r.due_at ? { dueAt: Date.parse(r.due_at) } : {}),
...
export class PgTimerRepository {
  ...
  async save(instances: readonly TimerInstance[], q: Queryable = this.db): Promise<void> {
    for (const i of instances) {
      await q.query(
        `INSERT INTO timers (id, code, subject_kind, subject_id, loan_id, armed_at, armed_by_event_id, anchor_date, due_date, due_at, status, satisfied_at, satisfied_by_event_id, breached_at, cancelled_reason, note)
        ...
        [i.id, i.code, i.subject.kind, i.subject.id, i.loanId ?? null, i.armedAt, i.armedByEventId, i.anchorDate, i.dueDate ?? null, i.dueAt !== undefined ? new Date(i.dueAt).toISOString() : null,
          i.status, i.satisfiedAt ?? null, i.satisfiedByEventId ?? null, i.breachedAt ?? null, i.cancelledReason ?? null, i.note ?? null]);
    }
  }
