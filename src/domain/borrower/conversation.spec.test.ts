...
const SCENES: readonly Scene[] = [
  ...
  // 32.18 rule 7 on a purchase: the home card re-sent for the estate and the clean-energy lien the contract card never asked — the two facts in words, with the contract's address the card still lacks (a to-be-determined purchase has no property row before this)
  { when: /own the land|leasehold|PACE|clean-energy/i, calls: propose("refi.home.confirm", { fields: [{ path: "property_address", value: "9 Saguaro Way, Phoenix, AZ 85018" }, { path: "estate_type", value: "fee_simple" }, { path: "existing_clean_energy_lien", value: "no" }] }), text: readBack("Noted: you own the land and there is no PACE or clean-energy loan on the house. That's saved — say if it's not right.", "Got it. The home card here has the two questions about the house itself.") },
  ...
  { when: /mark (all )?the declarations|declarations (as|are) (all )?no/i, calls: propose("declarations.occupancy", { option_id: "yes_no_prior" }), text: "Those answers are yours to give, not mine to enter. The declarations card here lists them; tap the one that fits." },   // 32.3 R5: the sequence opens with 5a.A — a no-command card, still never taken in words
...
test("conversation: the refinance persona goes from sign-up to a boarded loan by talking — a card only where §2.3 requires one, every stated fact written by the turn (32.17 rule 21), the FAKE vendors and reviewers in between", { skip }, async () => {
  ...
  const identity = await b.pending("identity.confirm.title"); assert.deepEqual((identity.props["fields"] as { path: string; value: string; source: string }[]).map((x) => [x.path, x.value, x.source]), [["legal_name", b.name, "stripe_identity"], ["date_of_birth", b.dob, "stripe_identity"], ["current_address", f.address, "stripe_identity"], ["residency_basis", "", "borrower"], ["monthly_rent_cents", "", "borrower"], ["months_at_address", "", "borrower"]]);   // 32.3 E5: the residence basis and the months are asked on the same card (T5)
  later(1); await b.tap(identity, fieldsEvidence(identity, { residency_basis: "own", months_at_address: "72" }));
  ...
  later(1); r = await b.say("It is my main home, at 100 N Central Ave in Phoenix, and I live there. I own the land, and there is no PACE or clean-energy loan on it."); await assertModelReply(b, r, { proposedInto: "refi.home.confirm", text: /100 N Central Ave/ });
  ...
  const occ = await b.pending("declarations.occupancy"); assert.equal(occ.props["proposal"], undefined, "no proposal reached the declarations card (5a.A, the first of the sequence)");
  later(1); await b.tap(occ, { option_id: "yes_no_prior", evidence: { option_id: "yes_no_prior", tapped_at: clock.now() } });   // 32.3 R5: 5a.A on the card before the list; no command
  const lien = await b.pending("declarations.clean_energy_lien"); later(1); await b.tap(lien, { option_id: "no", evidence: { option_id: "no", tapped_at: clock.now() } });   // 5a.E on its own card, no command
...
test("conversation: the purchase persona goes from sign-up to a boarded loan by talking — the preapproval, the contract, the LE, the lock, underwriting's items, the appraiser hand-off, the CD, the RON signing, the same-day funding and boarding", { skip }, async () => {
  ...
  const identity = await b.pending("identity.confirm.title"); later(1); await b.tap(identity, fieldsEvidence(identity, { residency_basis: "own", months_at_address: "72" }));
  const ssn = await b.pending("identity.ssn.title"); later(1); await b.tap(ssn, fieldsEvidence(ssn, { ssn: b.ssn }));
  ...
  const occ = await b.pending("declarations.occupancy"); later(1); await b.tap(occ, { option_id: "yes_no_prior", evidence: { option_id: "yes_no_prior", tapped_at: clock.now() } });   // 32.3 R5: 5a.A first
  const lien = await b.pending("declarations.clean_energy_lien"); later(1); await b.tap(lien, { option_id: "no", evidence: { option_id: "no", tapped_at: clock.now() } });   // then 5a.E, its own card
  ...
  // 32.18 rule 7: the Nov 2 run's document named the estate and the clean-energy lien as missing (the contract card asks neither), so the home card was re-sent as the ask that collects them — answered in words, written by the turn
  later(1); r = await b.say("We own the land outright, and there is no PACE or clean-energy loan on the house."); await assertModelReply(b, r, { proposedInto: "refi.home.confirm", text: /own the land/ });
  const homeFacts = await b.written("refi.home.confirm"); assert.ok(String(homeFacts.props["flow_key"]).includes(":gap:"), "the re-sent home card (the emission's flow_key), not 32.3 R1's refinance card");
