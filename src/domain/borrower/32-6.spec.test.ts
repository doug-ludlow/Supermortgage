...
const events = (appId: string, type?: string) => db.query<{ type: string; occurred_at: string; sequence: string; payload: Record<string, unknown> }>(`SELECT type, occurred_at, sequence::text AS sequence, payload FROM loan_events WHERE application_id = $1 AND ($2::text IS NULL OR type = $2) ORDER BY loan_events.sequence`, [appId, type ?? null]);
