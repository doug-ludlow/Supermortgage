...
**§1024.33(c)(1)**: "during the 60-day period beginning on the effective date of transfer," a payment received by the transferor "on or before the applicable due date (including any grace period allowed under the mortgage loan instruments)" may not be treated as late "for any purpose" and no late fee may be imposed (comment 33(c)(1)-1). The statute (12 U.S.C. 2605(d)) says "before the due date"; Reg X's "on or before ... including any grace period" is broader and governs the build. Comment 33(c)(1)-2: a transferee's compliance with §1024.39 during the 60 days is not treating a payment as late. **(c)(2)**: beginning on the effective date of transfer and without any 60-day or due-date limit, a transferor that receives a payment incorrectly must "promptly" either "transfer the payment to the transferee servicer for application" or "return the payment to the person that made the payment and notify such person of the proper recipient" — forwarding/return is still required after day 60; only the (c)(1) late-treatment protection ends. **(d)**: compliance preempts state notice laws (state notices to insurers/taxing authorities may still be required).
...
**Discrepancies vs blueprint**: (1) the "~5 yrs" retention is policy — Reg X §1024.38(c)(1) requires one year post-transfer; the baseline's `respa_5y` class is adopted as conservative; (2) blueprint omits the 30-day exception and the (b)(2) exclusions, which matter for master-servicer-only changes (no notice) versus subservicer changes (notice); (3) the 60-day rule is "on or before the due date including grace," not "60-day late-fee grace" — it protects misdirected payments only, and the (c)(2) forwarding/return duty is not limited to the 60 days.
...
- After day 60, misdirected payments are posted as of Supermortgage receipt (no (c)(1) protection); the transferor's §1024.33(c)(2) forwarding/return duty has no 60-day or due-date limit, so `SM_1024_33C2_FORWARD_PROMPT_1` still arms on every `payment.received{received_by='transferor'}` (+1 servicer business day from `transferor_received_at`, satisfied by `payment.posted`); a one-time courtesy waiver is allowed by policy (decision 3).
...
- Payment received by transferor on day 61+: no (c)(1) protection; the (c)(2) forwarding/return duty still applies and `SM_1024_33C2_FORWARD_PROMPT_1` arms as usual; courtesy waiver by policy.
