...
export function applySatisfiedOverrides_3_8(reg: TimerRegistry): void {
  ...
  o("FNMA_B101_WAIVER_REVOKE_BEFORE_TRIAL_GATE", { satisfied: "`escrow.waiver.trial_gate.cleared{basis∈{escrow_established, exception_documented}}`",
    why: "§3.8 timer table: `lossmit.trial_plan.offer_prepared` → escrow established (or exception documented: current on T&I AND the Flex Mod is offered under the D2-3.2-06 disaster-related-hardship path — `flex_mod_disaster_hardship=true`; a standard Flex Mod trial on a waived loan always requires escrow first); satisfied by `escrow.account.established` or `escrow.waiver.exception_documented` — recorded as `escrow.waiver.trial_gate.cleared{basis}` naming which fact cleared the gate (B-1-01: 'unless the borrower is current on the payments for taxes … and the mortgage loan modification is a Fannie Mae Flex Modification in accordance with Evaluating or Soliciting a Borrower with a Disaster-Related Hardship for a Fannie Mae Flex Modification in D2-3.2-06')." });
