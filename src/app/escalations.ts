...
export class EscalationService {
  ...
  open(input: EscalationInput, by: Actor): Escalation {
    const e: Escalation = { id: randomUUID(), kind: input.kind, ownerRole: input.ownerRole ?? DEFAULT_ROLE[input.kind], openedAt: this.clock.now(), openedBy: `${by.kind}:${by.id}`, payload: input.payload ?? {}, status: "open",
      ...(input.loanId ? { loanId: input.loanId } : {}), ...(input.caseId ? { caseId: input.caseId } : {}), ...(input.batchId ? { batchId: input.batchId } : {}), ...(input.severity ? { severity: input.severity } : {}), ...(input.slaTimerId ? { slaTimerId: input.slaTimerId } : {}) };
    this.opened.push(e);
    this.events.append({ type: "escalation.created", ...(e.loanId ? { loanId: e.loanId } : {}), aggregate: { kind: "escalation", id: e.id }, actor: by, payload: { escalation_id: e.id, kind: e.kind, owner_role: e.ownerRole, severity: e.severity ?? null, ...e.payload } });
    return e;
  }
...
export class PgEscalationRepository {
  ...
  async save(e: Escalation, q: Queryable = this.db): Promise<void> {
    await q.query(`INSERT INTO escalations (id, kind, loan_id, case_id, batch_id, severity, owner_role, sla_timer_id, opened_at, opened_by, completed_at, completed_evidence_document_id, payload)
      ...
      [e.id, e.kind, e.loanId ?? null, e.caseId ?? null, e.batchId ?? null, e.severity ?? null, e.ownerRole, e.slaTimerId ?? null, e.openedAt, e.openedBy, e.completedAt ?? null, e.evidenceDocumentId ?? null, toJson(e.payload)]);
  }
