...
test("32.11-T6: Given **Yes**, then the compressed application asks income (fresh statement), hard-pull authorization, declarations and demographics again, and does not re-ask address (ConfirmCard from `servicing_record`) or identity documents; `application.trid_received` fires on the sixth confirmation.", { skip }, async () => {
  ...
  // the declarations again as 32.3 R5's sequence (5a.A first; the list after 5a.E; "Something applies" → SQ-05; the last tap asserts the fourteen answers) — never the old one-tap list
  const occ = cards.find((c) => c.kind === "ChoiceCard" && c.copy_key === "declarations.occupancy" && c.status === "pending")!; assert.ok(occ, `5a.A pending: ${keys.join(" ")}`); assert.equal(occ.command_ref, null); assert.equal(occ.props["asked_again"], true); assert.equal(occ.props["flow_key"], `declarations.occupancy:${(occ.props["declarations_seq"] as P)["key"]}`);
  assert.ok(!cards.some((c) => c.copy_key === "declarations.title"), "the list waits for 5a.A and 5a.E"); assert.ok(cards.some((c) => c.kind === "DemographicsCard" && c.status === "pending"));
  ...
  // 32.18 rule 7: the compressed home card asks the estate and the clean-energy lien too (the servicing record has neither)
  assert.deepEqual(home.props["required_paths"], ["property_address", "estate_type", "existing_clean_energy_lien"]); assert.ok(homeFields.some((f) => f.path === "estate_type" && f.value === "") && homeFields.some((f) => f.path === "existing_clean_energy_lien" && f.value === ""), "both asked, unanswered");
  clock.set(MST("2026-12-02", "10:07")); await confirm("refi.home.confirm", fieldsOf("refi.home.confirm").map((f) => (f.path === "estate_type" ? { ...f, value_confirmed: "fee_simple", source: "borrower" } : f.path === "existing_clean_energy_lien" ? { ...f, value_confirmed: "no", source: "borrower" } : f)));
