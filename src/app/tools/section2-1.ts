...
export async function postReceivedPayment(i: ToolInput, ctx: CommandContext, rt: ToolRuntime): Promise<unknown> {
  ...
  // 35.5 rule 7: a lockbox item's receipt set was posted at ingest on its received_on (credit as of receipt, §1026.36(c)(1)) — the posting run reuses it
  // rather than posting a second one; an item the ingest parked on the clearing account's suspense (6.5, unidentified) and a person later identified
  // carries `receipt_parked_account`: its receipt here moves that parked credit to the loan's suspense (the same rule-8 shape, from the parked account)
  const priorReceipt = typeof pay.receipt_entry_set_id === "string" && pay.receipt_entry_set_id ? pay.receipt_entry_set_id : null;
  const parked = pay.receipt_parked_account as { custodial_account_id?: string; account?: string } | undefined;
  const receipt = priorReceipt && parked?.custodial_account_id
    ? post(`receipt ${id}`, [{ account: cust(parked.custodial_account_id, (parked.account as CustodialAccount | undefined) ?? "suspense_unapplied"), amountCents: amount, ruleRef: "2.1:r8:receipt" }, { account: loanAcct("suspense_unapplied"), amountCents: -amount, ruleRef: "2.1:r8:receipt" }])
    : priorReceipt ? { id: priorReceipt }
    : post(`receipt ${id}`, [{ account: cust(custodial.clearing, "clearing_cash"), amountCents: amount, ruleRef: "2.1:r8:receipt" }, { account: loanAcct("suspense_unapplied"), amountCents: -amount, ruleRef: "2.1:r8:receipt" }]);
  // the sets a reversal (2.3's return, 2.1 rule 9) mirrors: the ingest's receipt too when this posting only released it
  const receiptSetIds = priorReceipt && receipt.id !== priorReceipt ? [priorReceipt, receipt.id] : [receipt.id];
  ...
  if (plan.installments.length || plan.curtailment_cents > 0n) {
    ...
    const setIds = [...receiptSetIds, alloc.id, split.id];
    ...
    ctx.events.append({ type: "payment.posted", loanId, aggregate: { kind: "payment", id }, actor: ctx.actor, payload: { payment_id: id, loan_id: loanId, outcome: plan.outcome, amount_cents: s(amount), received_on: receivedOn, credited_as_of: creditedAsOf, channel: pay.channel ?? null, installments, interest_cents: s(interest), principal_cents: s(principal), escrow_cents: s(escrow), late_charge_cents: s(plan.late_charge_cents), curtailment_cents: s(plan.curtailment_cents), upb_after_cents: s(plan.installments.length ? plan.installments[plan.installments.length - 1]!.upb_after_cents : plan.next.upb_cents), rule_path: plan.rule_path.join(" → "), ledger_entry_set_ids: setIds } });
    return { payment_id: id, outcome: plan.outcome, installments, interest_cents: s(interest), principal_cents: s(principal), escrow_cents: s(escrow), to_suspense_cents: s(plan.to_suspense_cents), entry_set_ids: setIds };
  }
  ...
  rt.store.put("payments", id, { ...pay, status: "held", allocation_outcome: plan.outcome, suspense_item_id: item.id, balance_needed_cents: s(remaining), ledger_entry_set_ids: receiptSetIds }, ctx.actor, ctx.now);
