...
test("35.12-T19: Given any 35.12 route or cycle in a contract test over the fixture book, then the ledger, every `*_cents` column of `loans`, `fees`, `escrow_accounts` and `loan_installments`, and every `notices` row are identical before and after; every event and row this process wrote matches no e-mail, no name field and no 9-digit TIN pattern; every action route recorded a `staff_actions` row and an `agent_decisions` row naming the person and, where the rule requires, the confirmer.", { skip }, async () => {
  ...
  try {
    ...
    // one parties row slipped in outside the doors (synthetic = false): the scan finds it; the environment is rebuilt (the row gone, the seed stands), the manifest recorded, the purge scan carries the scan_id
    const [real] = await db.query<{ id: string }>(`INSERT INTO parties (party_type, legal_name, contact, synthetic) VALUES ('borrower', 'Real Person', '{"email": "real.person@example.test"}'::jsonb, false) RETURNING id::text AS id`);
    const scan = await call("POST", "/ops/api/posture/scans", { environment: "nonprod", kind: "nonprod_real_data" }, "cara", "compliance", "data.scan", [200]); assert.equal(scan.body["real_data_found"], true, "the slipped-in row; the fixture book itself is synthetic throughout");
    await db.query(`DELETE FROM parties WHERE id = $1`, [real!.id]);
    w.clock.set(at(5 * MIN, t0));   // the rebuilt environment's manifest lands after the finding's scan (REBUILD_NOT_RECORDED otherwise); the sessions' idle window is 30 minutes
    const nonprodManifest = await call("POST", "/ops/api/posture/manifests", hardened("nonprod", w.clock.now(), digest), "cid", "ciso", "posture.record", [200]);
    const purged = await call("POST", `/ops/api/posture/scans/${scan.body["scan_id"]}/purged`, { environment: "nonprod", rebuilt_manifest_id: nonprodManifest.body["manifest_id"] }, "cara", "compliance", "data.scan", [200]); assert.equal(purged.body["purged_scan_id"], scan.body["scan_id"]); assert.equal(purged.body["real_data_found"], false);
