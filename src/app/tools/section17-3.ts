...
import { receiveMisdirectedPayment, disposeMisdirectedPayment, type Instrument } from "../../domain/transfers/ops-17-2.ts";
  ...
  { name: "runOutboundDqGate", kind: "write", guardrails: [noBalanceEdit, officerAttestation, partnerCredentials], handler: compute((i, ctx, rt) => {
      switch (i.op ?? "dq") {
        ...
        case "receipt": {   // SM_XFER_OUT_POST_T_EVENT_GATE: a payment received on/after T for a listed loan is a misdirected payment (direction out) — the posting command is refused; no payment.received, no investor event; the row rides the next servicer-BD forwarding file (D32)
          ...
          // 32.12 backend delta (additive): with the RESPA facts on the input (`respa_effective_date`, the installment's `due_date` + `grace_days`, the transferee block) the receipt is classified by 17.2's §1024.33(c) rule — `payment.received{loan.status=transferred_out, protected}` + `payment.misdirected.received{protected}` on the loan (SM_1024_33C2_FORWARD_PROMPT_1 arms) and, forwarding being the default, `misdirected_payment.forwarded` + `payment.misdirected.forwarded` (ops-17-2 receiveMisdirectedPayment / disposeMisdirectedPayment); `protected` is written on the misdirected_payments row the late-charge and delinquency runs read
          let classification: Record<string, unknown> | null = null;
          if (row && str(i, "respa_effective_date")) {
            const tb = obj(i, "transferee"); const instrument = (str(i, "instrument") || (channel === "ach" ? "ach" : channel === "wire" ? "wire" : "check")) as Instrument;
            const input = { payment_id: str(i, "payment_id"), loan_id: str(i, "loan_id"), received_on: receipt.received_on, due_date: optDate(i, "due_date") ?? receipt.transfer_date, grace_days: Number.isNaN(num(i, "grace_days")) ? 15 : num(i, "grace_days"), respa_effective_date: date(i, "respa_effective_date"), amount_cents: amt, instrument, forwardable: i.forwardable !== false, forward_block_reason: (i.forward_block_reason as string | undefined) ?? null, transferee: { name: String(tb.name ?? ""), remittance_address: String(tb.remittance_address ?? ""), tollfree: String(tb.tollfree ?? "") }, disposed_on: optDate(i, "disposed_on") };
            const rc = receiveMisdirectedPayment(ctx.events, input, ctx.actor);
            rt.store.put("misdirected_payments", str(i, "payment_id"), { protected: rc.classification.protected, credited_as_of: rc.classification.credited_as_of, day_of_window: rc.classification.day_of_window, window_end: rc.classification.window_end, forward_by: rc.classification.forward_by, respa_effective_date: input.respa_effective_date }, ctx.actor, ctx.now);
            let disposed: string | null = null;
            if (i.forwardable !== false && i.dispose !== false) { const d = disposeMisdirectedPayment(ctx.events, rc.classification, { loan_id: str(i, "loan_id"), disposed_on: optDate(i, "disposed_on"), forward_reference: str(i, "forward_reference") || null }, ctx.actor); disposed = d.id; rt.store.put("misdirected_payments", str(i, "payment_id"), { forwarded_at: ctx.now, forward_reference: str(i, "forward_reference") || null, disposition: "forwarded" }, ctx.actor, ctx.now); }
            classification = { protected: rc.classification.protected, credited_as_of: rc.classification.credited_as_of, day_of_window: rc.classification.day_of_window, window_end: rc.classification.window_end, forward_by: rc.classification.forward_by, disposition: rc.classification.disposition, disposed_event_id: disposed };
          }
          if (row) { const { forwarding_file_date, ...cols } = row; rt.store.put("misdirected_payments", str(i, "payment_id"), { ...cols, ...(classification ? { protected: classification["protected"] } : {}) }, ctx.actor, ctx.now);
            ctx.events.append({ type: "transfer.misdirected_payment.recorded", loanId: str(i, "loan_id"), aggregate: batchAgg(i), actor: ctx.actor, payload: { batch_id: str(i, "batch_id"), payment_id: str(i, "payment_id"), loan_id: str(i, "loan_id"), direction: "out", received_at: receipt.received_on, amount_cents: String(amt), instrument: cols.instrument, forwarding_file_date, gate: r.gate, refusal: r.refusal, ...(classification ? { protected: classification["protected"], window_end: classification["window_end"] } : {}) } }); }
          return { ...r, misdirected_payment: row ? { ...row, amount_cents: row.amount_cents } : null, ...(classification ? { classification } : {}) }; }
