...
export const SHAPES = {
  ...
  // 32.16 DELTA-27: the in-app voice turn — the utterance as heard (the FAKE's transcript, confidence, utterance id), the reply with its copy key and deep link, the card a spoken yes attested, the misses on the current card, the spoken reply's audio reference (never audio)
  voice_utterance: { utterance: { ...MESSAGE, transcript: true, vendor: true, confidence: true, utterance_id: true, language: true, low_confidence: true }, reply: MESSAGE, attested: { card_instance_id: true, kind: true, copy_key: true, status: true, manner: true, utterance_id: true, transcript_ref: true, read_back_message_id: true, decision_id: true }, misses: true, spoken: { vendor: true, audio_ref: true, text: true }, routed_to: true, command_executed: true, command: true } satisfies Shape,
  ...
  video_session: { token: true, level: true, party: { party_id: true, first_name: true }, opened_account: true, video_session_id: true, status: true, vendor: true, conversation_url: true, end_reason: true, transcript_ref: true, created_at: true, joined_at: true, ended_at: true, subject: { application_id: true, loan_id: true }, conversation_id: true, vendor_conversation_id: true, replica_id: true, borrower_camera: true, greeting: true, opening_turn: true, fallback_copy_key: true } satisfies Shape,
  video_greeting: { video_session_id: true, ready: true, text: true, reply_message_id: true } satisfies Shape,
