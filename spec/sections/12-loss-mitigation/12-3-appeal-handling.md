...
|---|---|---|---|---|---|---|
| `REGX_1024_41H2_APPEAL_WINDOW_14` | deadline (borrower) | `notice.provided{denial of modification}` with tier ge_90 or (f) period, principal residence (§1024.30(c)(2)) | `provided_at` | 14 `calendar_days` | `lossmit.appeal.received` | lapse → `lossmit.appeal.window.expired` → holds may release if (g)(1)/(f)(2)(i) satisfied |
...
| `REGX_1024_41G1_APPEAL_HOLD` | not_before_gate | `lossmit.appeal.received{principal_residence≠false}` (rule 8: the hold opens on receipt before eligibility is confirmed; §1024.30(c)(2): a non-principal-residence courtesy reconsideration opens no Reg X hold) | — | until appeal denied or window lapses | `foreclosure_holds.closed{kind=lm_appeal_pending}` | 13.x refuses motion/sale |
...
1. **Eligibility:** `eligible = tier ∈ {ge_90} OR first_filing_made_at_receipt=false` (Reg X) AND the appealed determination denied a trial/permanent modification (Flex Mod, including "not evaluated/ineligible by loan data" outcomes). Reg X and D2-2-07 both require principal residence: §1024.30(c)(2) limits §§1024.39–.41 (including the (h) appeal) to loans secured by the borrower's principal residence (§1024.30(b) exempts *servicers* — small servicers, reverse mortgages, qualified lenders — not loans). `eligible = principal_residence AND (tier ∈ {ge_90} OR first_filing_made_at_receipt=false) AND modification denial`; a non-principal-residence appeal is a courtesy reconsideration only (`ineligibility_reason=not_principal_residence`).
...
- `NTC_REGX_41H_APPEAL_INELIGIBLE` (late/non-modification/tier/not a principal residence): reason, what the borrower can do (NoE, new application after becoming current, courtesy reconsideration — the platform's default for a non-principal-residence appeal).
...
- **Decision record:** `{appeal_id, eligibility{tier, first_filing, option_types, principal_residence}, excluded_ids, reviewer_id, new_information_summary, reeval_trace (12.2 schema), recommended_outcome, human_decision, human_edits, notices}`.
