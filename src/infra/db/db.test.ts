...
test("migrations: every file under db/migrations is applied to the test database (751 tables: public + restricted_fl)", { skip }, async () => {
  ...
  assert.equal(t!.c, 751n);   // the count a fresh `db/migrate.sh` run produces through 0123 (public + restricted_fl base tables; 0116 oidc_identities, 0117 lead_tokens, 0118 party_credentials, 0119 agent_turns, 0120 demo_clock, 0121 video_sessions; 0122 and 0123 add columns only)
});
