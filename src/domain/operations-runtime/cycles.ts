...
export const selectors = {
  ...
  default_case_loans: { name: "default_case_loans", select: async (q) => {
    ...
    for (const sql of [`SELECT DISTINCT loan_id AS id FROM regx_ei_windows WHERE closed_at IS NULL`, `SELECT DISTINCT loan_id AS id FROM cases WHERE closed_at IS NULL AND loan_id IS NOT NULL`, `SELECT DISTINCT loan_id AS id FROM claim_candidates WHERE resolved_at IS NULL`])
...
type DefInput = Omit<CycleDef, "runner" | "depends_on" | "serves_timer" | "escalation_role" | "expected_by_rule"> & Partial<Pick<CycleDef, "runner" | "depends_on" | "serves_timer" | "escalation_role" | "expected_by_rule">>;
export const def = (d: DefInput): CycleDef => ({ runner: null, depends_on: [], serves_timer: null, escalation_role: "ops_analyst", expected_by_rule: null, ...d });
...
export const CYCLE_ROWS: readonly CycleDef[] = [
  def({ cycle_code: "cashiering_daily", owner_process: "35.5", owner_agent: "cashiering", unit_scope: "loan", schedule: "daily, every active loan (servicing.ts:133 today restricts it to originated loans — 35.5 lifts that)", period_grammar: "day", selector: selectors.active_loans, receipt_event: "cashiering.daily.run_completed", serves_timer: "NOTE_6A_LATE_CHARGE_GRACE_GATE", expected_by_rule: "same_day 23:59 ET" }),
  ...
  def({ cycle_code: "document_integrity", owner_process: "35.2", owner_agent: "security-records", unit_scope: "global", schedule: "daily 02:30 ET", period_grammar: "day", selector: selectors.global, receipt_event: "document.integrity.run_completed", serves_timer: "SM_DOC_INTEGRITY_DAILY", expected_by_rule: "same_day 23:59 ET" }),
  def({ cycle_code: "projection_verify", owner_process: "35.1", owner_agent: "security-records", unit_scope: "global", schedule: "daily 06:00 ET", period_grammar: "day", selector: selectors.global, receipt_event: "projection.run_completed", serves_timer: "SM_PROJECTION_LAG_DAILY", expected_by_rule: "same_day 23:59 ET" }),
  ...
  def({ cycle_code: "lockbox_ingest", owner_process: "35.5", owner_agent: "cashiering", unit_scope: "account", schedule: "per lockbox per business_days_servicer; the file by 10:00 in lockbox_batches.cutoff_tz", period_grammar: "day", calendar: "business_days_servicer", selector: selectors.lockboxes, receipt_event: "lockbox.ingest.run_completed", serves_timer: "SM_LOCKBOX_FILE_EXPECTED_1BD", expected_by_rule: "10:00 ET" }),
  def({ cycle_code: "ach_file_build", owner_process: "35.5", owner_agent: "cashiering", unit_scope: "global", schedule: "every banking day (business_days_federal) by 14:00 ET", period_grammar: "day", calendar: "business_days_federal", selector: selectors.global, receipt_event: "ach.file_build.run_completed", depends_on: onCashiering(), serves_timer: "SM_ACH_FILE_BUILD_1BD", expected_by_rule: "14:00 ET" }),
  def({ cycle_code: "ach_returns_ingest", owner_process: "35.5", owner_agent: "cashiering", unit_scope: "global", schedule: "every banking day 08:00 ET", period_grammar: "day", calendar: "business_days_federal", selector: selectors.global, receipt_event: "ach.returns_ingest.run_completed", serves_timer: "SM_ACH_RETURN_ACTIONED_1BD", expected_by_rule: "08:00 ET" }),
  ...
  def({ cycle_code: "parallel_run.reconcile", owner_process: "35.12", owner_agent: "compliance-sentinel", unit_scope: "global", schedule: "daily 21:00 ET while a parallel_runs row is open", period_grammar: "day", selector: selectors.open_parallel_runs, receipt_event: "parallel_run.day.reconciled", depends_on: onCashiering(), serves_timer: "SM_PROD_PARALLEL_RUN_DAILY", expected_by_rule: "21:00 ET" }),
