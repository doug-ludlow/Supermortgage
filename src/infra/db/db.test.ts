...
test("migrations: every file under db/migrations is applied to the test database (765 tables: public + restricted_fl)", { skip }, async () => {
  ...
  assert.equal(t!.c, 765n);   // the count a fresh `db/migrate.sh` run produces through 0131 (public + restricted_fl base tables; 0116 oidc_identities, 0117 lead_tokens, 0118 party_credentials, 0119 agent_turns, 0120 demo_clock, 0121 video_sessions; 0122 and 0123 add columns only; 0124 widens a check; 0125 partner_book_imports, partner_book_facts, partner_book_invitations, partner_book_reviews, readiness_checks; 0126 adds a column only; 0127 staff_users, staff_credentials, staff_sessions, staff_actions, staff_access_reviews; 0128 directory_unmasks, directory_exports; 0129 partner_book_daily_reports; 0130 evidence_packs; 0131 adds a column only)
});
