...
export function applySatisfiedOverrides_12_2(reg: TimerRegistry): void {
  ...
  // ---- §1024.30(c)(2) (12.2 timer table, amended): the (c)(1) evaluation clock arms only for a principal residence (null = unknown, treated as one).
  o("REGX_1024_41C1_EVALUATE_NOTIFY_30", { trigger: "`lossmit.application.completed{principal_residence!=false}`", anchorField: "complete_at",
    why: "§12.2 timer table (amended): `lossmit.application.completed` (tier gt_37/ge_90; principal residence — §1024.30(c)(2), 12.3 rule 1: `principal_residence≠false`) → 30 `calendar_days` from `complete_at` (for (c)). The 12.1 `lossmit.application.open/update` handler (src/app/tools/section12.ts) stamps `principal_residence` on the completion event (input flag, else the stored application value, else null); a non-principal-residence loan keeps only the Fannie Mae D2-2-05 / F-2-10 evaluation duties." });
}
