...
describe("32.4 grouped LE package (T3, T4)", () => {
  ...
  it("the Thread renders a single grouped message: 'Your Loan Estimate and 2 related document(s).' as one reference with a chip per document; on the rail each document is its own card (32.16 §2.1: no card in the thread)", () => {
    ...
    const rec = document.querySelector('[data-record-section="record"]')!;
    expect(rec.getAttribute("data-open")).toBe("false"); // everything but the current ask waits behind "Your record" (32.16 §2.2): a card appears when it is needed
    fireEvent.click(rec.querySelector(":scope > h2 > button")!);
    if (docs.getAttribute("data-open") === "false") fireEvent.click(docs.querySelector(":scope > h2 > button")!);
