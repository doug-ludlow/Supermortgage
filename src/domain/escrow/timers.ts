...
export function applyEscrowTimerOverrides(reg: TimerRegistry): void {
  ...
  o("REGX_1024_17I2_POST_EXEMPTION_HISTORY_90", { satisfied: "`escrow.statement.sent{statement_type=post_exemption_history}`", why: "§3.3 timer table: date the account became current (`exemption_ended_on`) + 90 calendar days (§1024.17(i)(2)); satisfied by `escrow.statement.sent` (post_exemption_history)." });
  ...
  o("FNMA_B101_WAIVER_REVOKE_BEFORE_TRIAL_GATE", { evaluator: "3.8.escrowEstablishedOrExceptionDocumented",
    why: "§3.8 timer table: escrow established (or exception documented: current on T&I AND the Flex Mod is offered under the D2-3.2-06 disaster-related-hardship path — `flex_mod_disaster_hardship=true`; a standard Flex Mod trial on a waived loan always requires escrow first) before trial plan offer (B-1-01)." });
