...
export class Runtime {
  ...
  async sweep(nowIso: string = this.clock.now()): Promise<SweepReport> {
    ...
    // 35.7: the roles pass after the FAKE reviewers (a FAKE approval of the day is counted by the daily scan that follows) and before the breach pass (a day's scan receipt never breaches) — errors logged, never thrown
    let roles: RolesSweepReport | null = null;
    try { roles = await rolesSweepPass(this, nowIso, { runId: null }); } catch (e) { this.logger?.error("roles sweep failed", { at: nowIso, error: e }); }
